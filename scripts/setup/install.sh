...
if [ ${#MISSING_DEPS[@]} -gt 0 ]; then
    ...
    if [ "$AUTO_INSTALL_DEPS" = "true" ]; then
        ...
        # Install WireGuard (used for auto-onboarding tunnel back to
        # api.lanagent.net when the agent is behind NAT). Required for any
        # agent that isn't publicly reachable; harmless when not used.
        if ! command -v wg-quick &>/dev/null || ! command -v wg &>/dev/null; then
            info "Installing WireGuard..."
            if command -v apt-get &>/dev/null; then
                apt-get install -y -qq wireguard wireguard-tools &>/dev/null \
                    && ok "WireGuard installed" \
                    || warn "WireGuard install failed (tunnel mode will be unavailable)"
            fi
        fi

        # If the agent will run as a non-root user, grant the minimal sudoers
        # rights needed for the gateway client to bring up the tunnel.
        if [ -n "${RUN_AS_USER:-}" ] && [ "${RUN_AS_USER}" != "root" ] && command -v wg-quick &>/dev/null; then
            sudoers_file="/etc/sudoers.d/lanagent-wg"
            if [ ! -f "$sudoers_file" ]; then
                info "Granting ${RUN_AS_USER} sudoers rights for wg-quick + wg0.conf"
                tmp="$(mktemp)"
                cat > "$tmp" <<EOF
# LANAgent — gateway client uses these to bring up the auto-onboarding tunnel.
${RUN_AS_USER} ALL=(root) NOPASSWD: /usr/bin/wg-quick up wg0
${RUN_AS_USER} ALL=(root) NOPASSWD: /usr/bin/wg-quick down wg0
${RUN_AS_USER} ALL=(root) NOPASSWD: /usr/bin/install -m 600 -o root -g root /tmp/wg0.*.conf /etc/wireguard/wg0.conf
${RUN_AS_USER} ALL=(root) NOPASSWD: /usr/bin/systemctl enable wg-quick@wg0
${RUN_AS_USER} ALL=(root) NOPASSWD: /usr/bin/systemctl disable wg-quick@wg0
EOF
                visudo -cf "$tmp" >/dev/null 2>&1 \
                    && install -m 440 -o root -g root "$tmp" "$sudoers_file" \
                    && ok "Sudoers entry installed at $sudoers_file" \
                    || warn "Sudoers entry validation failed — agent will need to run as root for tunnel"
                rm -f "$tmp"
            fi
        fi
...
# Docker containers can't reach host localhost — resolve before writing .env
OLLAMA_WRITE_URL="${OLLAMA_URL:-http://localhost:11434}"
if [ "$DOCKER_MODE" = "true" ] && [[ "$OLLAMA_WRITE_URL" == *"localhost"* ]]; then
    OLLAMA_WRITE_URL="${OLLAMA_WRITE_URL/localhost/host.docker.internal}"
fi
...
OLLAMA_BASE_URL=${OLLAMA_WRITE_URL}
...
if [ "$DOCKER_MODE" = "true" ]; then
    ...
    # AI detector Python deps (torch + transformers ~250MB CPU-only). Installed
    # in a per-service venv so we don't fight Ubuntu 24's PEP 668 system-package
    # protection. ecosystem.config.cjs auto-detects this venv and falls back to
    # system python3 when it's absent, so this step failing degrades cleanly —
    # the rest of the agent still runs; only AI detection is unavailable.
    AI_DETECTOR_DIR="$PROJECT_ROOT/src/services/ai-detector"
    if [ -f "$AI_DETECTOR_DIR/requirements.txt" ]; then
        info "Installing AI detector Python dependencies (CPU torch, ~250MB)..."

        # apt-install python3-venv + python3-pip if missing. The version-pinned
        # python3.X-venv subpackage matters: on Ubuntu 24 the python3-venv
        # metapackage can be "installed" without actually pulling python3.12-venv,
        # so `python3 -m venv .venv` succeeds at creating the dir but fails to
        # bootstrap pip inside it. Always explicitly request the version-pinned
        # one based on the detected python3 minor version.
        if command -v apt-get &>/dev/null; then
            PY_VER=$(python3 -c 'import sys; print(f"{sys.version_info.major}.{sys.version_info.minor}")' 2>/dev/null)
            ${SUDO:-} apt-get install -y -qq python3-venv python3-pip ${PY_VER:+python${PY_VER}-venv} &>/dev/null \
                || warn "Could not apt-install python venv/pip packages — AI detector may be unavailable"
        fi

        if python3 -m venv --help &>/dev/null 2>&1; then
            if [ ! -d "$AI_DETECTOR_DIR/.venv" ]; then
                python3 -m venv "$AI_DETECTOR_DIR/.venv" 2>&1 | tail -3
            fi

            if [ -x "$AI_DETECTOR_DIR/.venv/bin/pip" ]; then
                # --extra-index-url pulls CPU torch wheels (~200MB) instead of
                # the default CUDA wheels (~2GB) — most LANAgent deployments
                # don't have a GPU and the detector_service auto-detects and
                # uses GPU at runtime if available.
                "$AI_DETECTOR_DIR/.venv/bin/pip" install --upgrade pip -q 2>&1 | tail -3
                if "$AI_DETECTOR_DIR/.venv/bin/pip" install -q \
                        --extra-index-url https://download.pytorch.org/whl/cpu \
                        -r "$AI_DETECTOR_DIR/requirements.txt" 2>&1 | tail -5; then
                    ok "AI detector Python deps installed"
                else
                    warn "AI detector pip install failed — service will be unavailable"
                    info "Retry manually: $AI_DETECTOR_DIR/.venv/bin/pip install -r $AI_DETECTOR_DIR/requirements.txt"
                fi
            else
                warn "venv was created but pip is missing — AI detector unavailable"
            fi
        else
            warn "python3-venv not available — AI detector will be unavailable"
            info "Install manually: apt-get install python3-venv python3-pip"
        fi
    fi
fi
