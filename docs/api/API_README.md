...
| Image Transcode | `POST /imageTools/transcode` | 2 | Image format transcode (avif/png/jpeg/webp) with oversize-passthrough; streams the upload to the agent. See **Image Transcode** below. |
...
### Agent Auto-Onboarding (`POST /agents/register`)

LANAgent instances can self-onboard to the gateway using their existing P2P Ed25519 identity — no admin key, the signature is the auth. The agent's own `src/services/gateway/gatewayClient.js` handles the client side automatically; this section documents the gateway's contract for anyone wiring a non-LANAgent runtime.

```
POST https://api.lanagent.net/agents/register
Content-Type: application/json

{
  "fingerprint": "1fe07ae9013afed4...",        // sha256(publicKey)[:16] hex
  "publicKey":   "MCowBQYDK2VwAyEA...",         // Ed25519 public key, base64-encoded SPKI DER
  "name":        "MyAgent",
  "walletAddress": "0xabc...",                  // BSC/ETH address for payouts
  "serviceUrl":  "https://my-agent.example.com",// optional — gateway probes this for DIRECT mode
  "agentPort":   80,
  "wgPublicKey": "x25519-pubkey-base64",        // required if no serviceUrl, or as TUNNEL fallback
  "version":     "2.25.48",
  "agentId":     null,                           // optional ERC-8004 claim; honored if no clash
  "timestamp":   1779380000,                    // unix seconds, ±5min window
  "nonce":       "random-hex-32",               // single-use, gateway enforces
  "sig":         "ed25519-signature-base64"     // signs canonical JSON of all fields except sig
}
```

Gateway behavior:
- Verifies the signature (`publicKey` must hash to `fingerprint`)
- Enforces ±5min timestamp window + per-nonce replay protection
- Probes `serviceUrl` (if provided) for reachability
- **DIRECT mode** (serviceUrl reachable): records the agent and routes traffic directly
- **TUNNEL mode** (not reachable, or no serviceUrl): allocates a free `10.8.0.X/32` slot (.3–.254; .1 = gateway, .2 = ALICE legacy), appends a WireGuard peer for `wgPublicKey`, hot-reloads via `wg syncconf`, returns the tunnel config the agent needs to bring up its side
- **Idempotent on `fingerprint`** — re-registration updates the existing record and replaces the WireGuard peer block in place (no IP churn, no service interruption)
- Gateway-assigned `agentId` starts at 10001 to stay clear of ERC-8004's 2000–9999 range

Response (DIRECT mode):
```json
{ "success": true, "agentId": 10042, "mode": "direct", "fingerprint": "1fe07ae9..." }
```

Response (TUNNEL mode):
```json
{
  "success": true, "agentId": 10042, "mode": "tunnel", "fingerprint": "1fe07ae9...",
  "tunnel": {
    "endpoint": "api.lanagent.net:51820",
    "serverPublicKey": "x25519-server-pubkey-base64",
    "allocatedIp": "10.8.0.x",
    "gatewayIp": "10.8.0.x",
...
Heartbeat: agents should re-POST to `/agents/register` every ~5 min to refresh `lastSeen`. The `gatewayClient.js` shipped with LANAgent does this automatically.

Error responses:
- `400` — missing required fields, or no serviceUrl + no wgPublicKey
- `401` — signature invalid, fingerprint doesn't match publicKey, timestamp out of window, or replay (nonce already seen)
- `500` — tunnel provisioning failed (gateway-side WireGuard config error)
...
→ { "token": "eyJ...", "apiKey": "gsk_abc123...", "credits": [redacted] }

POST /portal/login
{ "email": "you@example.com", "password": "..." }
→ { "token": "...", "credits": [redacted] }
...
Authorization: Bearer <token>
→ { "credits": [redacted], "apiKeys": [...], "recentPayments": [...] }

GET /portal/packages
→ { "packages": [
    { "id": "starter", "price": 500, "credits": [redacted], "priceDisplay": "$5.00" },
    { "id": "growth", "price": 1500, "credits": [redacted], "priceDisplay": "$15.00" },
    { "id": "pro", "price": 5000, "credits": [redacted], "priceDisplay": "$50.00" }
  ]}
```
...
→ { "token": "eyJ...", "wallet": "0x...", "credits": [redacted] }
...
→ { "credits": [redacted], "newBalance": 500 }

GET /credits/balance
X-API-Key: gsk_your_key
→ { "credits": [redacted], "totalPurchased": [redacted], "totalSpent": [redacted] }
```
...
→ {
    "success": true,
    "summary": { "count": 5, "credits": [redacted], "totalPurchased": [redacted], "totalSpent": [redacted], "totalRefunded": [redacted] },
    "wallets": [
      {
        "wallet": "0x8b1ed1...",
        "credits": [redacted],
...
The api.lanagent.net landing page already ships its branding as an inline SVG favicon, but ad networks need standalone PNG URLs with cross-origin-friendly headers. Three new routes:

```
GET https://api.lanagent.net/logo.png             → 512×512 PNG (default)
GET https://api.lanagent.net/logo.png?size=1024   → 1024×1024 PNG
GET https://api.lanagent.net/logo-512.png         → 512×512 PNG
GET https://api.lanagent.net/logo-1024.png        → 1024×1024 PNG
```
...
### v2.25.52 — Provider routing fix, AI-detector venv install, PR round + MindSwarm cleanup

**Provider hardcoding (`src/api/plugins/websearch.js`)** — the web-search flow had `defaultOrder = ['anthropic', 'openai']` and would *switch the active provider* to whichever came first. With both keys registered, every web search bypassed the agent's `aiProviders.current` (`openai`/gpt-5 on ALICE). Now the order is caller-preference → current provider → `['openai', 'anthropic']` fallback. PR-review code paths intentionally keep anthropic. `capabilityIncrementalScanner.js` similarly switched its `'anthropic'` default for context-limit lookups to a conservative `'huggingface'`.

**Puppeteer (`src/utils/stealthBrowser.js`)** — added `protocolTimeout: 180000` to `pptr.launch()` defaults. Was throwing unhandledRejections on CDP `Network.enable` for slow Cloudflare-gated pages under the default 30s.

**AI-detector install path** — `src/services/ai-detector/requirements.txt` gained `torch`, `numpy`, `transformers` (`detector_service.py` imports them but they were never listed). `scripts/setup/install.sh` now creates a per-service venv at `src/services/ai-detector/.venv` and `pip install`s with `--extra-index-url https://download.pytorch.org/whl/cpu` (CPU wheels, ~200 MB vs ~2 GB CUDA defaults). Explicitly apt-installs the version-pinned `python3.X-venv` package — on Ubuntu 24 the generic `python3-venv` metapackage isn't enough to create a venv with working pip. `ecosystem.config.cjs` resolves the `ai-detector` interpreter to the venv python when present, falls back to system `python3` otherwise (so existing installs without a venv keep their current behaviour until they re-run `install.sh`).

**Gateway (separate repo `api-lanagent-net`, commit `9853d95`)** — single missed `new: true` site at `index.mjs:2128` (inside `/agents/register`) finally migrated to `returnDocument: 'after'`. Was producing continuous mongoose deprecation warnings under the live PID because that endpoint fires on every agent refresh.

**PR #2187 — viz-dashboard URL-hash deep-linking (merged)** — `#viz=agent-brain`-style hash deep-linking + `localStorage` persistence of the last active visualization tab. Hash takes priority on init, then localStorage, then first viz. `hashchange` listener for cross-tab navigation, `removeEventListener` in `destroy()`, try/catch around storage for private-mode safety.

**PR #2188 — `stableJsonStringify` utility (reimplemented)** — deterministic JSON for hashing/diffing/caching. New export at `src/utils/jsonUtils.js`. Sorted keys (default lexical, custom comparator optional), handles circular refs (`'[Circular]'`), BigInt→string, Date→ISO 8601, Buffer→base64, Map→sorted object, Set→sorted array. Matches `JSON.stringify` semantics for undefined (objects drop, arrays nullify). Tests: `tests/unit/jsonUtils.stable.test.js` (14 tests, all pass via `node --test`). The original PR's test mixed CJS into pure-ESM and would never have run — reimplementation drops the broken test.

**PR #2189 — memoryVectorStore caching (reimplemented)** — `MemoryVectorStore.search()` and `findDuplicate()` now cache results in the existing NodeCache. Hash-based cache keys via SHA-256 of the Float32 embedding (16 hex chars vs ~30 KB raw `.join(',')`). Invalidate on actual writes only (`flushBatch`, `deleteMemory`, `deleteMemories`, `rebuildIndex` — not on `addMemory` which only queues). Env flags: `ENABLE_SEARCH_CACHE` (default `true`), `ENABLE_DUP_CACHE` (default `true`), `SEARCH_CACHE_TTL` (default `300` seconds). Tests: `tests/unit/memoryVectorStore.cache.test.js` (7 tests). The original PR's test monkey-patched non-existent method names (`searchMemories`, `isDuplicate`) and never exercised the real cache path.

**PR #2190 — freshping `searchchecks` action (merged)** — new client-side filter over `getChecks()`.
...
// → { success: true, data: [...filtered checks...], count: N, total: M }
```

Both filters are optional and combine (AND). `q` matches case-insensitively against name OR url substring; `status` is exact match against `check_state` (`up`/`down`/`unknown`). No new deps.

**PR #2191 — lpMarketMaker `Idempotency-Key` header (merged)** — every mutating LP market maker route now accepts an idempotency key, via either the `Idempotency-Key` request header or an `idempotencyKey` field in the JSON body. Affected routes:

```
POST /api/crypto/lp/mm/enable
POST /api/crypto/lp/mm/disable
POST /api/crypto/lp/mm/open
POST /api/crypto/lp/mm/close
POST /api/crypto/lp/mm/rebalance
POST /api/crypto/lp/mm/collect
POST /api/crypto/lp/mm/schedule
```

The key is namespaced per-operation (`lpmm:<op>:<key>`) so the same client key on `/open` and `/close` won't collide. Cached response is returned on repeat (default TTL 600 s, configurable via `LP_MM_IDEMPOTENCY_TTL_SECONDS`). Errors are deliberately not cached so transient failures (e.g. RPC timeouts) can be retried by the client with the same key.

```bash
curl -X POST https://alice.example.com/api/crypto/lp/mm/open \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -H "Idempotency-Key: open-2026-05-23-001" \
  -d '{}'
```

**MindSwarm plugin (`src/api/plugins/mindswarm.js`)** — four coupled fixes addressing day-of feedback from the MindSwarm-side dev:

- **First-hello post on auto-register** (`_postFirstHello`) — fires once right after `_setupProfileAfterRegistration` when an agent registers a fresh MindSwarm account. Privacy-safe by construction: agent name + host OS family + `#lanagent #firstpost` hashtags. AI-generated when a provider is available (explicit no-personal-info prompt + length + keyword guards), with a fixed-template fallback. Existing agents (registering via saved credentials) skip this — the path only fires on a genuinely new registration. So new agents arriving at MindSwarm are visible on the social network instead of materializing silently.

- **Token-refresh race fix** (`_getRefreshPromise`) — MindSwarm uses single-use refresh tokens with replay detection. Two parallel pollers each 401'ing at the same minute would each call `/auth/refresh` — the second one's refresh token having already been rotated/invalidated by the first, causing repeated full re-logins. Now mirrors the pattern in MindSwarm's own web frontend (`frontend/src/lib/api.ts:46-59`) — all 401 callers `await` the same in-flight promise; cleared only after the refresh resolves.

- **403 banned/suspended detection** — `_apiRequest` now detects `accountStatus === 'banned' | 'suspended'` (or email-verification-required) responses, drops `accessToken`/`refreshToken`, sets `_engagementConfig.enabled = false`, persists the cleared state, and throws a permanent error that the new `customErrorClassifier` short-circuits past `retryOperation`. Stops the retry-every-5-min-forever pattern against a dead account.

- **Poll cadence backoff** — `agenda.every('5 minutes', 'mindswarm-engagement')` → `'15 minutes'` in `src/services/scheduler.js`. Inside `_engagementCycle`, the heavy `_scanAndReplyToUnrepliedPosts` (which pulls `/users/me/posts` + `/posts/:id/replies` × up-to-10 posts + sub-reply checks per post) is now gated to once per hour via `_lastReplyScanMs`. `/notifications` hits/day per endpoint: 288 → 96. Own-profile sub-reads: ~8.6 k/day → ~720/day.

A full socket.io migration was considered (MindSwarm's backend does run socket.io for messaging) but deferred — the backend's `Notification.createNotification` model helper does push + email but does NOT `io.emit`, so a socket-only architecture would miss the bulk of reply/follow/mention/like notifications until the backend also emits a socket event from that helper.
...
Three fixes prompted by a routine status audit. The [redacted-token] trader was profitable but the daily error log had four `INSUFFICIENT_FUNDS` swap rejections on grid-buys. The status payload showed a misleading trailing-stop value. The LP MM had been silently running for 70+ days with `'0/0'` lifetime fees no matter what.
...
Two latent bugs in `src/api/crypto.js` surfaced while restoring the [redacted-token] token-trader instance after the previous day's strategy-registry rebuild incident.
...
**Same-day rescue:** Recovered the [redacted-token] token-trader live with `POST /api/crypto/strategy/token-trader/configure` (`tokenAddress=0x997a…18e1`, `tokenNetwork=bsc`, `capitalAllocationPercent=50`, `tokenTaxPercent=0`) followed by `POST /api/crypto/strategy/token-trader/restore-state?token=0x997a…18e1` with the cached cost basis from the stale `tokenTraderStatus` block. [redacted-token] had pumped [redacted] while the trader was offline; the first three ticks after restore caught the pump (scale-out at +20% / +25% / +30% levels, [redacted] realized in 7 minutes; lifetime realized PnL advanced). Trailing stop updated [redacted] [redacted]. Verified post-deploy: `heartbeats.started: true`, `tokenCount: 1`, [redacted-token] `regime: PUMP`, `interval: 180s (3m)`, `consecutiveErrors: 0`.
...
Cut the `render` tier price from 5 credits ($0.05) to 3 credits ($0.03) — same as `full`. FlareSolverr cost-to-serve is well under [redacted]/call so the 5cr sticker was almost entirely margin and read expensive next to ScraperAPI ultra_premium ($0.03+/call) and ScrapeGraphAI's stealth-extract ([redacted]/call on Pro). The new scheme keeps `render` semantically distinct from `full` (FlareSolverr-backed; handles real CF JS challenges) but aligned in price, so under a `full → render` escalation pattern the average per-fallback cost converges on $0.03 instead of $0.05.
...
**Operations (one-time):**
- Bootstrapped BETA with on-chain credits. Gateway wallet swapped BNB to SKYNET via PancakeSwap V2 then transferred it to BETA's recipient. BETA verified the tx and credited the account.
...
`POST /imageTools/transcode` — **2 credits.** Stateless format transcode that
offloads heavy AVIF encoding (the libvips/libaom working set) onto the agent host.
The gateway **streams** the upload straight to the agent, so large screenshots
don't buffer on the gateway.

**Transport:** `multipart/form-data` field `image` (≤100MB — preferred for large
inputs), OR `image` as base64 in JSON, OR `url` for sources too large to upload.

**Params:** `target` (required: `avif|png|jpeg|webp`), `quality` (1–100, default 82),
`effort` (0–9, default 2; AVIF), `lossless` (bool), `maxPixels` (oversize-passthrough
threshold; `0`/absent = always encode), `sourceFormat` (optional hint),
`passthroughBytes` (default true; set `false` to get metadata only when a passthrough
fires).

**Oversize passthrough:** when `maxPixels` is exceeded and the source is an
already-compressed raster, the source is returned unchanged with `passthrough:true`
and `outputFormat` = the source format (no re-encode).

```bash
curl -X POST https://api.lanagent.net/imageTools/transcode \
  -H 'X-API-Key: gsk_your_key' \
  -F image=@screenshot.png -F target=avif -F quality=82 -F effort=2 -F maxPixels=8000000
```

Response carries `data.{image,outputFormat,passthrough,sourceFormat,width,height,pixels,bytes}`
plus `creditsCharged`/`creditsRemaining`. Errors are refunded (not billed): `400`
invalid target / undecodable, `402` insufficient credits, `500/502` encode failure /
agent unreachable, `503` capacity reached.
...
- PancakeSwap liquidity: [redacted]
...
**Staking Auto-Claim:**
- Agent auto-claims staking rewards every 6 hours when pending > [redacted] SKYNET
...
{ "chain": "bsc", "toAddress": "0x...", "amount": "100", "token": "0x8b77CC5c6cB3d846608d9d5Dd03fA406BA03b8F1" }
```
...
- `SKYNET` (`0x8b77CC5c6cB3d846608d9d5Dd03fA406BA03b8F1`) — project token
...
- Report fee: [redacted] SKYNET (routes to staking reward pool)
...
To run a second instance on the same server, set in `.env`: `PM2_PROCESS=lan-agent-beta`, `AGENT_PORT=8180`, `MONGODB_URI=***REMOVED***2`, `TELEGRAM_ENABLED=false`, `ENABLE_MQTT=false`. Use PM2 fork mode. Instances auto-discover each other via P2P.
...
- Wallet balances UI shows active token trader token balance (e.g., [redacted-token])

**Phase 1 — Token Contract & Foundation:**
- SKYNET BEP-20 Contract: `0x8b77CC5c6cB3d846608d9d5Dd03fA406BA03b8F1` — 100M fixed supply
- SKYNET/BNB LP: PancakeSwap V2 pair `0x2AC21524188988025E54429a40B83460098eB601`
...
```
http://192.168.x.x/downloads/filename.mp4
```
...
Fixed Nano RPC endpoints (switched to rainstorm.city/api as primary), fixed invalid representative address, added 429 rate limit handling with exponential backoff. Transaction labels now correctly show `trade_sell`/`trade_buy` instead of `defi_yield`/`service_payment`. Fixed token symbol resolution for FHE, [redacted-token], LINK. External service toggles now use toggle switches with confirmation modals. Revenue API rate limit increased to 300/15min. Fixed missing auth on refresh-balances endpoint.
...
#   "tracking": { "peakPrice": [redacted], "trailingStopPrice": [redacted], "scaleOutLevelsHit": [] },
#   "regime": "PUMP"
# }
...
- **POST** `/api/keys`
  ...
  **Response**:
  ```json
  {
    ...
    "key": {
      "id": "key_id",
      "key": "la_<full-key-shown-once>",  // Full key shown only once!
...
- **GET** `/api/health` - Quick health status check (no auth required)
  ...
  ```json
  {
    "status": "ok",
    "health": "healthy",
    "reportAgeSeconds": 2012,
    "timestamp": "2026-05-22T15:21:59.991Z",
    "uptime": 449.10668615
  }
  ```

  - `status`: `"ok"` whenever the latest diagnostic report is not `critical` *or* is older than 2× the diagnostics schedule interval (default 12h). `"unhealthy"` only when a fresh report has `overallHealth=critical`.
  - `health`: `"healthy" | "warning" | "critical" | "unknown" | "stale"`. Returns `"stale"` when `reportAgeSeconds > 2 × autoRunInterval` (default 12h) — a single stale critical reading no longer pins the endpoint to unhealthy indefinitely.
  - `reportAgeSeconds`: age of the most recent diagnostic report in seconds, or `null` if no report exists yet.
...
**📁 Production Log Structure** (`$DEPLOY_PATH/logs/`):
...
curl -X POST https://api.lanagent.net/service/websearch/crypto \
  ...
  -H 'Content-Type: application/json' \
  -d '{"symbol": "ETH"}'
# → { "success": true, "result": "ETH Price: [redacted]\n24h Change: -2.15%" }
