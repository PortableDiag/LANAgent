...
**Latest (v2.25.45):** Sync from genesis — ALICE PR triage round (#2158-#2164, 7 PRs from the auto-improve loop). All 7 closed; 5 manually re-implemented with corrections, 2 partially-applied with the unsafe/dead pieces dropped. Per-PR closure comments on each. New plugins shipped: **`oauthmanager`** (full OAuth 2.0 with PKCE, refresh, RFC 7009 revoke; persistent token storage via `PluginSettings` with AES-256-GCM encryption at rest — PR was in-memory only and would lose every session on restart); **`xueqiu`** (Xueqiu/雪球 Chinese stock data — quote/search/hotPosts/hotStocks with ticker normalization for A-shares/HK/US); **`googlesecops`** (Google SecOps Detection Engine integration — listRules/getRule/searchDetections with bearer-token auth and pagination). Core capability: **`BasePlugin.getPluginConfig()`** — sanitized config snapshot with two redaction paths (explicit `safeConfigKeys` allow-list for sensitive plugins, or recursive deny-pattern scrub catching `/key|secret|token|password|credential|apikey|privatekey|seed|mnemonic|webhook/i` at any depth). Underpins the `getPluginConfig` action wired into the `_ai_template.js` scaffold so every future AI-generated plugin gets safe redaction automatically — fixes the same `this.config` exposure defect previously rejected as #2102. Bug fix: blockchain RPC retry tuning in `payment.js` (`getTransaction`/`getReceipt` 2→5 retries with bounded backoff). Trivial: `/api/external/image/health` endpoint with supportedFormats. Genesis-only items skipped per standing policy: `BaseStrategy.calculateAverageTradeDuration`/`calculateMaximumDrawdown`/`calculateSharpeRatio` + `StrategyRegistry.generatePerformanceReport` (from PR #2158). v2.25.44 base: Sync from genesis covering v2.25.41 through v2.25.44 — four versions of work bundled into one public release. (v2.25.44 — Production log hygiene pass) Five fixes: (1) `agent.notify` `message.substring is not a function` TypeError — `cryptoMonitor.js` was calling `agent.notify({title, message, priority})` while every other caller passes a string. Fixed both call sites to pass formatted strings AND hardened `agent.notify` to coerce object/non-string inputs so a future caller passing the wrong shape can't break notifications again. (2) ExpressVPN `connect "uk"` regression — `expressvpnctl` no longer accepts country names, now requires region slugs (`uk-london`, `usa-los-angeles-1`). Added 1h-cached `getRegionList()` and `resolveRegion()` with prefix/alias/substring resolution to `vpn.js`. (3) Plugin missing-credentials spam — 17 paid-API plugins (aviationstack, currencylayer, fixer, ipgeolocation, ipstack, mediastack, messagebird, sinch, numverify, numbersapi, weatherstack, zapier, googlecloudfunctions, checkly, here, anime, jsonplaceholder) were spamming `[ERROR]` for missing API keys on every boot; downgraded to `logger.warn` when cause is missing credentials, and extended `apiManager`'s credential-error detection to recognize bespoke patterns. Result: errors.log `[ERROR]` count dropped from ~22+ per boot to 0 plugin-init noise. (4) Empty `Balance verification failed:` — `swapService.js` now logs `error.message || error.code || error.reason || String(error)` plus token/network context. (5) Removed 7 leftover `console.error('DEBUG: ...')` calls leaking to `pm2-errors-0.log` from `capabilityIncrementalScanner`. (v2.25.43 — API-mode account registration + reapption) Account-registration strategies now support `mode: 'api'` alongside the existing browser path — skips Puppeteer and talks JSON directly to the site's signup endpoint, far more reliable for SPAs. `reapption` strategy wired up (calls `/api/auth/challenge` then `/api/auth/register`; semantic challenge questions answered by the agent's own AI). (v2.25.42 — social-media auto-post unwedge + privacy hardening) Auto-post dedup window shrunk from last 8 posts to last 3 (≈36h at 2/day) in `twitter.js` and `mindswarm.js`, with fail-open if the filter would still zero everything. GPT-5 empty-completion fix: bumped `maxTokens` to 1500 (MindSwarm) and 3000 (Twitter) so reasoning tokens don't eat the entire budget. Permanent diagnostic dump on AI failures (stringified `result` in skip-log). Privacy: extended `autoPostFilter.SENSITIVE_COMMIT_PATTERNS` to match `session.*\d{4}-\d{2}-\d{2}|docs\(session` so commits touching both `src/` and `docs/sessions/` no longer surface session-report subjects in auto-post context. Also: `scripts/deployment/deploy-files.sh` md5-verify + return-code check on remote extract (silent partial-deploy bug). (v2.25.41 — self-mod new-plugin routing) New `featureClassifier.js` service decides whether a github-discovered feature should be added as a new plugin file or modify an existing one. `selfModification.js` routes new-plugin tasks through a skeleton-scaffold path instead of trying to retrofit an unrelated file. v2.25.40 base: ALICE PR triage round (#2138–#2157, 20 PRs from the auto-improve loop). 4 merged, 1 salvaged manually, 15 closed with rationale. Merged: `ytdlpCookieJar` mkdir retry (#2151), `flareSolverr` custom-headers passthrough (#2152), `MqttDevice` status-change observability hook (#2154), `SSHConnection` double-start guard (#2155). Salvaged: new `GET /api/external/social/supported-sites` endpoint (#2147) returning the loaded extractors (`twitter`, `ytdlp`) with capabilities and Phase-1-verified host list — lets clients probe coverage without trial-against-`/probe`. Implemented directly rather than merging because the PR also stripped the route file's Phase-1/2/3 tier matrix header docblock. Closes broke into five categories: unsupported retryOperation option names that are silently ignored (#2138 `retryCondition`, #2141 `randomize` — `customErrorClassifier`/no jitter respectively), dead code with no callers/routes (#2139, #2153), sorts/lookups against fields or methods that don't exist (#2140 `MCPServer.priority`, #2149 `TaskScheduler.scheduleJob`), breaking changes (#2146 `GeneratedSong` model→wrapper, #2157 `encrypt`/`decrypt` sync→async on 30+ sync call sites), and wrong-shape implementations (#2142 module-load change stream, #2143 vendored upstream WebXR lib, #2144 opens hardcoded auto-post filter, #2145 `DailyPnL` cache invalidation gap, #2148 caches what Node's loader already caches, #2150 ollama retry-wrap breaks streaming/long timeouts, #2156 `new Function` ACE vector). Each PR got a comment explaining the call. v2.25.39 base: Agenda order-of-init fix. Plugins registered Agenda handlers in `initialize()`, but `apiManager.initialize()` runs at agent bootstrap line 127 while `scheduler.initialize()` runs at line 249 — so the `if (!this.agent.scheduler) return` guard silently bailed and handlers never registered (`update-pr-review-models` was 125 days stale, `review-prs` missing entirely, three `system-admin-*` jobs and two `sinch-scheduled-*` handlers missing). Fix: framework hook in `agent.js` after `scheduler.initialize(this)` iterates `apiManager.apis` and calls `plugin.defineSchedulerJobs()` if the method exists. Migrated `prReviewer.js` and `systemAdmin.js`; `sinch.js` already had a method with that name and got picked up automatically. Verified post-deploy: all jobs scheduled with valid `nextRunAt`, no stale rows. v2.25.38 base: Dashboard AI Providers page was stuck showing the gpt-4o / claude-3.5 era despite the daily `model-update` Agenda job running cleanly. Two combined problems: provider `availableModels` lived only in the in-memory provider instance (every restart dropped the dashboard back to `[]` until the next 3 AM job run), and Anthropic's `getAvailableModels()` was hardcoded to a fixed 5-model list that ignored `this.availableModels` entirely. Fix: `providerManager.syncModelsWithDatabase()` now pulls every `ModelCache` row at startup and calls `provider.updateAvailableModels(cache.models)`; Anthropic prefers `this.availableModels` when populated, falls back to the hardcoded list (mirrors the existing HuggingFace pattern). Also bumped scheduler `defaultModels.openai` from `'gpt-4o'` to `'gpt-5'`. After this, dashboard shows current model lists immediately after restart. v2.25.37 base: End-to-end MindSwarm ad automation. New `createAd` command on the `mindswarm` plugin orchestrates the full lifecycle — resolves an image (`imageUrl` | `imageFilePath` | `imagePrompt` for AI generation via `imageGenerationService` → `uploadMedia`), submits the ad, reads `/ads/settings`, then either (free mode: `ad_free_mode:true` or `daily_rate:0`) calls `/pay` with empty body or (paid mode) sends native crypto via `transactionService.sendNative` to the platform wallet and submits the txHash for on-chain verification. New `editAd` command wraps MindSwarm's `PATCH /api/ads/:adId`. v2.25.36 base: Quiet-the-noise pass for non-alice deployments — diagnostics + four background jobs (vpn-wireguard-watchdog, check-emails, cryptoMonitor.getAddressBalance, Telegram status test) now shape-aware. v2.25.35 base: Per-agent admin endpoints + dual-record agent bug fix + payment audit-trail / sanitizer date fix. New `createAd` command on the `mindswarm` plugin orchestrates the full lifecycle — resolves an image (`imageUrl` | `imageFilePath` | `imagePrompt` for AI generation via `imageGenerationService` → `uploadMedia`), submits the ad, reads `/ads/settings`, then either (free mode: `ad_free_mode:true` or `daily_rate:0`) calls `/pay` with empty body or (paid mode) sends native crypto via `transactionService.sendNative` to the platform wallet and submits the txHash for on-chain verification. Native-only for paid mode today (eth/bnb/matic); USDT/USDC daily rates exist server-side but a `sendToken` ERC-20 path isn't wired yet. New `editAd` command wraps MindSwarm's `PATCH /api/ads/:adId` — owner can edit `title` / `description` / `linkUrl` / `duration` / `placement` / `targetTags`; `_id` / `impressions` / `clicks` / `payment` history preserved; real field changes on approved/active ads re-run approval (instant under `ad_auto_approve`, otherwise drops to `pending_review`); `imageUrl` intentionally non-patchable. Two bug fixes that came with the work: `_submitAd` now properly requires `imageUrl` (server validation), `_payForAd` accepts empty body in free mode (was hardcoded to require six crypto fields). Two hotfixes during the live ALICE rollout: lazy-init `imageGenerationService` with `agent.providerManager`, and magic-byte sniffer for the upload (fal-ai returns JPEG even on PNG-request). First live ad active for ALICE advertising api.lanagent.net. v2.25.36 base: Quiet-the-noise pass for non-alice deployments. Beta VPS went unreachable after hitting 99% disk; cleanup recovered the host but a fresh diagnostic still flagged `health: critical` with 8 failed tests. None were real failures — the diagnostic suite and four background jobs all hard-coded the alice shape (PM2 + port 80, WireGuard installed, Gmail creds present, full RPC quota) and spammed errors on any leaner deployment. `selfDiagnosticsEnhanced.apiBaseUrl` now uses `WEB_PORT`/`AGENT_PORT`; Process Status detects `/.dockerenv`; Telegram returns `warning` when `TELEGRAM_BOT_TOKEN` isn't set; `vpn-wireguard-watchdog` probes for `wg-quick` + `/etc/wireguard/wg0.conf` and short-circuits when absent; `check-emails` guards on `emailPlugin.gmailUser`; `cryptoMonitor.getAddressBalance` wraps in the existing `contractService.withRpcFallback`. v2.25.35 base: Per-agent admin endpoints + dual-record agent bug fix + payment audit-trail / sanitizer date fix. New `GET /api/external/admin/wallets` and `GET /api/external/admin/payments/recent` (auth: `X-Admin-Key` header matched against `AGENT_ADMIN_KEY` env var, absent → 503, mismatch → 401) let the gateway admin panel fan out and surface `ExternalCreditBalance` rows that don't live in the gateway's own DB — solves the silent failure mode where a "wallet purchased credits" Telegram notification couldn't be traced to a wallet visible anywhere in the admin UI because the wallet lived on the agent, not the gateway. Companion gateway repo (`api-lanagent-net`, separate commits `9ae04d5` + `778667e`): `GET /admin/api/agent-wallets` fan-out endpoint hitting each enabled agent's `/api/external/admin/wallets` in parallel with the shared key (5s timeout each, graceful per-agent failure); new "Per-agent wallets" section on `/admin/wallets` page; Refunded column added to existing wallets table — data was already in `CreditBalance.totalRefunded` but never displayed (e.g. `0xce8efb55…`: balance 238 = totalPurchased 1794 − totalSpent 1966 + totalRefunded 410, books balance perfectly, the column was just hidden); fixed missing blue bars on `/admin/scrapes` Top services / Top agents (`.bar-fill` was a `<span>` with no `display:block`, so the inline `width:X%` was ignored). `ExternalPayment` schema gained `currency` / `creditsIssued` / `bonusCredits` / `promotion` / `usdValue` (all optional / backwards-compatible); `credits.js` populates them on every credit-purchase record so the row alone is enough to reconstruct the purchase (auto-replenishment records had been logging `amount=255940` with everything else `undefined`). `responseSanitizer` middleware was flattening every Date in every external response to `{}` because it walks objects via `Object.entries(val)` and Date instances pass `typeof === 'object'` but `Object.entries(date)` returns `[]` — fix passes `Date` and `Buffer` through unchanged before the generic object branch (wide-scope, every external response that contained a date was affected, not just admin/wallets). The session-anchoring fix: `loadAgentModel()` in `src/core/agent.js:295-329` was hardcoded to query `name:'LANAgent'` regardless of `process.env.AGENT_NAME` (the comment was self-incriminating: `Always try to load the LANAgent record first, regardless of AGENT_NAME`); meanwhile `providerManager` / web UI / scheduler / bugFixing / selfHealing / git plugin / mindswarm plugin all wrote to `AGENT_NAME`. On multi-instance installs this produced two `agents` documents per agent, both updated continually by different code paths. Production drift between the ALICE record and the LANAgent ghost spanned ~14 fields including `aiProviders.configurations` (ALICE had `gab` and `huggingface` configs the ghost didn't), `mediaGeneration.image.provider` (`huggingface` vs `openai`), `erc8004.status` (`active` with `agentId 2930` / `txHash 0xf85bc0c…` / `linkedWallet 0xc0C0D08…` vs `none`), `serviceConfigs.*.lastCheckTime` (real timestamps vs `null`), and others. Pre-fix audit confirmed zero fields existed only on the ghost — ALICE had every field plus 464 more, so no merge was needed. Fix uses `process.env.AGENT_NAME || 'LANAgent'` consistently. Verified post-restart that ALICE receives heartbeat updates and the LANAgent ghost stays frozen at its pre-fix mtime; ghost record then deleted via `mongosh deleteOne`; second restart confirms no new ghost recreated. Crypto wallet / ERC-8004 registration data untouched (lives on ALICE record + separate `cryptowallets` collection). v2.25.33 base — Crypto strategy hardening pass — three fixes prompted by a routine status audit. (1) TokenTrader `INSUFFICIENT_FUNDS` swap failures on native-paired buys: the price-impact branch in `executeTokenTrader` switched to native path when native had lower impact (almost always true for BSC tokens via WBNB) without checking the wallet's actual BNB — so the trader dispatched 0.6 BNB swaps when the wallet had 0.09 BNB. Fix gates the switch on `requiredNative + 0.005 (gas reserve) <= actualNative` and falls back to the stablecoin path with a `Native path lower impact ... falling back to stablecoin path` warn log when insufficient. Second nearby bug: the prior balance-check branch read `marketData.prices?.[net]?.nativeBalance` (doesn't exist; native balance is at `marketData.balances?.[net]?.native`), so it always read 0 — fixed for clean logging. (2) TokenTrader trailing-stop display: `state.trailingStopPrice` was always set with `pumpTrailingStopPercent` (4.5%) for both PUMP and MOON regimes, but `_analyzePump` ignored that value and triggered on its own inline 12% wide stop. Status payloads showed a stop that never actually fired in PUMP. Fix: stored value now reflects regime-applicable stop (12% PUMP / 4.5% MOON / 5% high-gain MOON / 8% standard); `_analyzePump`'s inline calc unchanged so behavior is preserved, only the displayed value moves. (3) LP MM 70 days of zero fee accounting: `lpManager.collectFeesV3` returned only `{txHash, success}`, never the actual amounts collected; `lpMarketMaker.collectFees` updated `lastFeeCollectAt` but never incremented `totalFeesCollectedSKYNET`/`totalFeesCollectedWBNB`. Fix: `collectFeesV3` now returns `{txHash, success, amount0, amount1}` with `tokensOwed0/1` formatted to ether-units (snapshot just before collect tx); `collectFees` reads those, increments lifetime totals in one `saveState`, logs `+X SKYNET, +Y WBNB (lifetime: …)` per collection. Backfill probe via on-chain Collect events found 0 historical events for the active position (single mint at block 89,508,147, no Decrease, no Collect, `tokensOwed=0`, `feeGrowthInside last=0`) — `0/0` lifetime totals were factually correct, just for a different reason than the audit assumed. Sub-finding fixed in same session: `rebalancePosition` correctly updates `state.tokenId` on the new position but never refreshed `openedAt`, so the displayed value stuck on the very first position's mint time even after rebalances rotated through new tokenIds — added `openedAt: now` to the `saveState` call alongside the new tokenId, with comments documenting why `rebalanceCount` (strategy-lifetime telemetry) and `rebalancesLast24h` (circuit-breaker rolling window) intentionally stay rolling. v2.25.30 base: ALICE-authored capability upgrades — 23 open self-modification PRs (#2093–#2115) triaged one by one. 12 merged with corrections; 11 closed with substantive defect explanations on each. Public-facing additions: `GET /api/avatar/gallery` now supports `owner`, `createdAfter`, `createdBefore` query filters with route-level cache (5 min TTL) and 3× retry; `GET /api/avatar/health` liveness probe; `GET /api/scammer-registry/report-history/:address` returning the chronological audit trail of every `ScammerRegistered` event for an address. Internal additions: `NetworkDevice.lifecycleStatus` field + mark helpers, `BaseProvider.getDetailedTokenUsageStats`, `Email.batchProcessEmails` chunking + `getEmailsByConversation` pagination, `DevelopmentPlan.filterItems` `creationDateRange`, `selfDiagnostics` adaptive interval at startup, `gravatarHelper.fetchGravatarProfile` retry, BitNet provider retries on transient failures, `AutoAccount.archiveInactiveAccounts` + Agenda `auto-account-archive` daily job (replaces the original PR's broken in-process timer), `selfModLock` `lockExpiring` 5-min-before-timeout warning. Notable closures: #2099 hallucinated NASA endpoint (`api.nasa.gov/planetary/data` returns 404 — verified via curl); #2102 template change that would expose `this.config` credentials via the external gateway across every future ALICE-generated plugin; #2112 deletes the working IP rate limiter to call non-existent service methods (every faucet claim would 500); #2106 calls non-existent `TaskScheduler.scheduleJob` and uses in-process timers that lose state on restart (use Agenda); #2104 mutating `cache.options.stdTTL` only affects future writes and the module-level `setInterval` runs at import time. v2.25.29 base: ALICE auto-post lockout fix — Twitter/X and MindSwarm posting stopped on 2026-05-01 because the dedup filter checked the last 8 posts against only 7 topic categories, so once recent posts covered them all every candidate item got stripped (`hasContent: false`, silent `.debug()` skip). Self-perpetuating because posting halt meant the recent-8-posts window never advanced. Fixed by expanding the topic catalog from 7 to 12 (`healing`, `skynetEcon`, `shipped`, `capabilities`, `providers`) and converting the silent `.debug()` skips to `.info()` with diagnostic context. SkynetAPIBot Telegraf `handlerTimeout` bumped from 90s default to 5 min so `/dl` doesn't fire spurious "unexpected error" replies during 60–180s gateway-side downloads; persistent chat-action keepalive every 4s; threaded replies via `reply_to_message_id`; status message updates as work progresses then auto-deletes once the file lands. v2.25.28 base: external media-download fixes — `/api/external/service/ytdlp/download` was returning `file.path: "[redacted]"` (paying customers paid 3 cr, agent downloaded the file, but no fetchable URL came back) because the responseSanitizer correctly redacts `/root/lanagent-deploy/downloads/<file>` internal paths. Fixed by minting a 60-min download token URL before sanitizer runs — applies to ytdlp/ffmpeg/imageTools/pdf when `result.file.path` or string `result.file` is present. `/api/external/youtube/download` was 502'ing with "ytdlp.execute is not a function" because `apiManager.apis.get('ytdlp')` returns a wrapper `{ instance, enabled, ... }` not the plugin instance — fixed by unwrapping `.instance`. v2.25.27 base: gateway repo workflow established + today's gateway-side fixes pulled into source control. The gateway code (`api.lanagent.net` portal, admin console, Stripe webhook, scrape proxies) is a separate repo at `PortableDiag/api-lanagent-net` (private), local at `/media/veracrypt3/Websites/LANAgent_Website/api-lanagent-net/`. Multiple sessions had been editing files directly on the production host with `ssh "sed -i …"` patches; the local repo had fallen six commits behind. This release rsyncs prod→local, commits the drift, adds a proper `deploy.sh` (whitelisted rsync + pm2 restart + health check), and persists memory notes so this doesn't recur. Bundled gateway commit `0a0c5a0` covers: `RequestLog.create` added at every credit-debiting endpoint (was only in the legacy proxy helper — `/scrape`, `/scrape/batch`, `/agents/:agentId/:service` all silently debited credits without logging, which is why customer dashboard charts and admin Top Services were empty for `web-scraping`); `agentUrl` → `agentName` fix in admin scrapes Top Agents aggregation; HTML5 responsive scrolling tables (`html, body { overflow-x: hidden }` + every `<table>` wrapped in `.table-wrap`); `.row-flex` mobile-wrap rule for Grant Credits admin-actions card; portal "Enter your email first" amber warning; promo strikethrough on credit packages and subscription cards (`~~$25~~ $12.50/mo first month`); dashboard usage chart label updates with range + JWT-first auth + better empty-state copy; `express.urlencoded` middleware fix for admin login form. Follow-up commit `28e132e` fixes a JS template literal escape issue: `\d` and `\s` were being silently stripped from regex sources at template evaluation time, so the promo strikethrough regexes never matched anything until the backslashes were doubled. v2.25.26 base: operational hardening pass — no API contract changes from v2.25.25. Bumps so all four surfaces (genesis, public PortableDiag/LANAgent, beta.lanagent.net, ALICE 192.168.0.52) report a matching version. Genesis-side: one-line comment fix in `src/api/external/routes/scraping.js` (TIER_COSTS comment misattributed the render cut to v2.25.21 instead of v2.25.25). Public repo: synced v2.25.25 from genesis and scrubbed 46 mentions of genesis-only crypto strategy classes (ArbitrageStrategy, DollarMaximizerStrategy, NativeMaximizerStrategy, RuleBasedStrategy, TokenTraderStrategy, VolatilityAdjustedStrategy, ArbitrageScan) from public's `feature-progress.json` plus 2 wholly-proprietary feature entries; reformatted to consistent 2-space indentation. Public-shipped strategies (BaseStrategy, DCAStrategy, GridTradingStrategy, MeanReversionStrategy, MomentumStrategy, StrategyRegistry) untouched. Beta agent: deployed v2.25.25 image to `beta.lanagent.net` after recovering 3GB mid-build by stopping the running BETA container to free its writable layer (Beta VPS is 25GB total and was at 86% pre-deploy; first two builds failed at the unpack-to-overlayfs step). Fixed beta's `origin` git remote which had the GitHub PAT inserted twice in the URL, breaking every scheduled self-modification fetch. Production (192.168.0.52 / ALICE): v2.25.25 code was already live (TIER_COSTS render: 3 confirmed) but `package.json` was stuck at v2.25.11 — v2.25.26 deploys via the full `deploy.sh` sync to make sure `package.json` lands. v2.25.25 base: render-tier price cut from 5cr to 3cr (matches `full`); FlareSolverr cost-to-serve is well under $0.001/call so the 5cr sticker was almost entirely margin and read expensive next to ScraperAPI ultra_premium ($0.03+/call) and ScrapeGraphAI's stealth-extract ($0.034 on Pro). New scheme keeps render distinct from full semantically (FlareSolverr-backed; CF JS-challenge handler) but aligned in price; under a `full → render` escalation pattern, average per-fallback cost converges on $0.03. Headline price story is now "1¢ to 3¢ per scrape" (was 1¢ to 5¢). v2.25.24 base: Per-page audit of every admin body for the same class of mobile-overflow issues. One remaining offender found: the "Grant credits" admin-actions row on user-detail and wallet-detail pages used `.row-flex` with inline `width:160px` / `flex:1` declarations that overrode responsive rules and overflowed on phone widths. Fixed by making `.row-flex` wrap at ≤880px and forcing its inputs to a full row with `!important` overrides. All other admin pages clean. v2.25.23 base: admin dashboard mobile follow-up — Promotions and Scrapes pages were still rendering as wide multi-column layouts on phone screens because two inline `grid-template-columns` declarations overrode the responsive rules. Promotions page no longer scrolls horizontally on mobile (sidebar form drops below the table at ≤880px); Scrapes page no longer puts Top Services and Top Agents side-by-side on mobile (charts stack at ≤880px). Two new responsive grid classes added (`.grid-2`, `.grid-aside`) so future layouts can declare intent without inline overrides. v2.25.22 base: gateway admin dashboard responsive overhaul — Recent Payments table no longer crammed (Payments card now spans 2/3 width on desktop, full width on tablet, stacked on mobile; long emails truncate with hover-title), top nav collapses into a hamburger drawer on screens ≤880px, all wide tables scroll horizontally inside their cards instead of overflowing the page. v2.25.21 base: gateway and BETA agent recovery. Fixed silently-broken admin login (gateway was missing `express.urlencoded` so the HTML form's email field never reached the handler — every login fell into the anti-enumeration tarpit). Reconnected gateway↔BETA after a 24-day stale-API-key outage caused by the April 11 mongo ransomware wipe (6,917 401s on `/credits/balance`, BETA excluded from the routing pool). Bootstrapped BETA with $5 of on-chain SKYNET credits via the gateway wallet → PancakeSwap V2 swap → SKYNET transfer → BETA `/credits/purchase` verification. Reworked `pickBestAgent` for strict primary/fallback by reputation: ALICE always wins when online and funded; BETA only sees traffic when ALICE is offline or out of credits (was weighted-random which gave BETA ~0.5% probability even when ALICE was healthy). Stood up backup tooling: `/root/backup-beta.sh` + nightly cron on BETA (14d retention) and a local pull script `scripts/deployment/backup-beta.sh` writing full-restore tarballs to `_BackUps_/` (30d retention). v2.25.20 base: PR review pass on 10 AI-generated PRs (#2083–#2092). v2.25.19 shipped BYOK on the SKYNET API Telegram bot (`/setkey`, `/mykey`, `/clearkey`, AES-256-GCM at rest, validated against gateway before storing). v2.25.17 shipped the generic social-media downloader (`POST /social/download` MP4, `POST /social/audio` MP3 on the gateway), with Cloudflare-gated hosts (Rumble, BitChute) routed through long-lived FlareSolverr sessions for `cf_clearance` cookies + matching User-Agent + `--impersonate chrome-131` (TLS fingerprint must match the issuer), and a per-host cookie-jar pipeline for Instagram/Facebook with operator-managed admin endpoints at `/api/admin/cookies`. See [CHANGELOG.md](CHANGELOG.md) for full history.
...
- **SKYNET Telegram Bot** (`@SkynetAPIBot`) — public API showcase, 30+ commands, auto-credit purchase, on-chain event broadcasting to @skynet_events, token transfer tracking to @skynet_tracker, MindSwarm rich embeds, cashtag price tooltips, **BYOK** (`/setkey gsk_…` or paste in chat — encrypted at rest, validated against gateway, source message auto-deleted) so users hitting the daily free-tier cap can bill their own gateway account
