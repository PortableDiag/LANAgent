...
**Latest (v2.25.14):** ALICE PR review pass — 18 auto-generated PRs (#2065-#2082) reviewed one by one against the standing rule "implement what you can safely; salvage good ideas with bad implementations rather than discarding". 1 merged (**ipstack `getTimezones`** batch command — mirrors `fetchGeolocationBatch`, reuses existing helpers), 1 salvaged (**`repoInfo.js` git-remote fallback observability** — replaced silent `} catch {}` with a `logger.debug` line so the no-origin-remote path is visible). 16 closed with explanatory comments covering breaking changes, would-crash-on-import code, success-stub returns, on-chain double-spend hazards, misused utilities, redundant duplicates, and description-vs-diff mismatches. v2.25.13 added the email + support layer to `api.lanagent.net` (usage-threshold emails at 80/90/100% on sub buckets and credit-pool balance, password-reset + email-verification + API-key create/revoke + cancellation transactional emails, support@ inbox monitored by a Telegram-notifying poller with 7-day account context). v2.25.12 introduced VPN auto-rotation on block + Scraper Subscription tiers. See [CHANGELOG.md](CHANGELOG.md) for full history.
