...
**Latest (v2.25.13):** Two infrastructure releases since v2.25.11. **v2.25.12** adds VPN auto-rotation on block to the paid-scrape path: `executeScrapeWithVpnRotation` wraps the scraper and rotates the agent's ExpressVPN exit through a curated 10-location pool (US East/West/Central, Toronto, London, Amsterdam, Frankfurt, Paris, Tokyo, Singapore) when a `stealth`/`full`/`render` scrape returns a block-shaped failure (403/406/429/503/Cloudflare challenge), retrying up to 2× with cache invalidation; `basic` tier skipped to avoid the 3–5s reconnect overhead. Test hook `_testBlock: 'once'|'always'` for E2E verification. **v2.25.13** is a gateway-side release adding an email + support layer to `api.lanagent.net`: usage-threshold notification emails (80/90/100% on subscription buckets and credit-pool balance, fire-once-per-cycle, atomic claim, fire-and-forget); account-management email suite (password reset, email verification, welcome, API-key created/revoked, sub cancellation); `support@lanagent.net` inbox monitored by a `support-poller` PM2 service that pings Telegram with enriched 7-day account context for each ticket. Roadmap to LLM-drafted replies (v3), auto-send for trusted categories (v4), and proactive outreach on internal events (v5) is in [`docs/proposals/ai-support-system.md`](docs/proposals/ai-support-system.md). v2.25.10 PR review pass added Shazam lyrics, LP MM scheduled ops, peer session counters, adaptive external-auth rate limit. v2.25.7 added FlareSolverr-backed render-tier scraping. See [CHANGELOG.md](CHANGELOG.md) for full history.
