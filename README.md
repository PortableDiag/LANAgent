...
**Latest (v2.25.20):** PR review pass on 10 AI-generated PRs (#2083–#2092) — 1 merged (`FeatureRequest.statusHistory`), 5 salvaged manually with corrected implementations (`SystemLog` pagination, `scammerRegistry` compression middleware, `PluginDevelopment.calculateEvaluationAnalytics` rewritten as cross-document static, payment notifications converted to fire-and-forget retry, MCP tool-usage analytics with bounded ring buffer + new `GET /api/tools/usage` endpoint), 4 closed as unimplementable (missing SDK endpoints, type mismatches, missing prerequisites). v2.25.19 shipped BYOK on the SKYNET API Telegram bot (`/setkey`, `/mykey`, `/clearkey`, AES-256-GCM at rest, validated against gateway before storing). v2.25.17 shipped the generic social-media downloader (`POST /social/download` MP4, `POST /social/audio` MP3 on the gateway), with Cloudflare-gated hosts (Rumble, BitChute) routed through long-lived FlareSolverr sessions for `cf_clearance` cookies + matching User-Agent + `--impersonate chrome-131` (TLS fingerprint must match the issuer), and a per-host cookie-jar pipeline for Instagram/Facebook with operator-managed admin endpoints at `/api/admin/cookies`. See [CHANGELOG.md](CHANGELOG.md) for full history.
