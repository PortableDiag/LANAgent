...
Stateless image-format transcode that offloads heavy AVIF encoding (the
libvips/libaom multi-hundred-MB working set) off small caller boxes onto the
agent host.

- **Plugin:** new `transcode` action on `imageTools` — encode to `avif|png|jpeg|webp`
  (sharp; AVIF defaults `quality:82, effort:2, chromaSubsampling:'4:4:4'`), with a
  cheap header-only **oversize-passthrough** (`maxPixels`): huge already-compressed
  rasters are returned unchanged instead of re-encoded. Returns format + dimension
  metadata so callers can drop their own `metadata()` probe. `passthroughBytes:false`
  returns metadata only.
- **External route:** `POST /api/external/service/imageTools/transcode` — multipart
  (≤100MB) / base64 / `url`. Flat **2 credits**, refunded on any failure. New
  in-process **concurrency limiter** (`src/utils/concurrencyLimiter.js`, env
  `IMAGE_TRANSCODE_MAX_CONCURRENT`, default 2) + `sharp.concurrency(4)`; queue
  overflow sheds load (503 + refund).
- Registered in the service catalog (`image-transcode`, 2 credits).

## [2.25.99] - 2026-06-14

### Changed

- Migrated all SKYNET references to the relaunched contract: token
  `0x8b77CC5c6cB3d846608d9d5Dd03fA406BA03b8F1`, LP pair
  `0x2AC21524188988025E54429a40B83460098eB601`. SkynetDiamond address unchanged.

## [2.25.98] - 2026-06-11

### Fixed — render customers were not getting their screenshots
- `takeScreenshot`/`generatePDF` bailed with "Page content did not render" whenever `waitForRealContent` judged a page "thin" — but the challenge-resolution gate runs first, so thin content is almost always a legitimately sparse page (e.g. example.com), not an unresolved challenge. The result: paying **render**-tier customers got `success:true` with **no screenshot** on any low-content page. Now it logs a warning and captures the current DOM anyway. Verified end-to-end through the gateway: render returns a full-page screenshot for both sparse (example.com, 29 KB) and content-rich (Wikipedia, 3.2 MB) pages.
- Gated the v2.25.97 Puppeteer screenshot behind `options.captureScreenshot` so it no longer fires on every Puppeteer scrape (incl. basic/full escalations) — removing the wasted full-page capture + latency/timeout risk; stealth/render screenshots come from the route's existing screenshot path.

## [2.25.97] - 2026-06-11

### Added — full HTML + full-page screenshots from the stealth tier; screenshots now reach ScrapeCache
- `scrapeWithPuppeteer` (stealth tier) now captures the raw HTML (`page.content()`) and a **full-page** screenshot while the page is still open — previously it only extracted text, so stealth scrapes reached ScrapeCache as text-only. Both are best-effort (never fail the scrape) and the screenshot is size-guarded against pathologically tall pages.
- Render/screenshot capture now defaults to **full-page** (the whole scrolled page, not the viewport) unless the caller passes `fullPage:false`. NOTE: render-tier customers who previously got a viewport PNG now get a full-page PNG by default.
- The external scrape route retains `_screenshot`/`_rawHtml` on the response for non-render/non-full tiers so the gateway can ingest the full page + screenshot (stripped before the customer).
- Gateway: `ingestToScrapeCache` now switches to **multipart/form-data** when a screenshot is present (ScrapeCache only accepts screenshots as a file part) and forwards `scraper_engine`/`scraper_tier` metadata. Agent→gateway transfer cap raised to 50 MiB to carry full-page screenshots. Verified end-to-end: an html+screenshot ingest returns `quality_tier: full` with html, screenshot (avif), and auto-WACZ artifacts.

## [2.25.96] - 2026-06-11

### Fixed — feed full HTML (not just text) to ScrapeCache ingest
- The scraper plugin's cheerio path discarded the raw HTML and returned only extracted content, so the gateway's ScrapeCache ingest had no `html` to send for basic-tier scrapes — snapshots landed as `text` tier instead of `full`. `scrapeWithCheerio` now returns `_rawHtml` (FlareSolverr already did). The external scrape route retains `_rawHtml` on the response for non-full tiers so the gateway can ingest the full page; the gateway strips it before returning to the customer (basic-tier callers still only get extracted text). Verified end-to-end: an html-bearing ingest now returns `quality_tier: full` with html + auto-generated WACZ artifacts.
- Known remaining gap: the Puppeteer (stealth tier) path still extracts only — `_rawHtml` not yet captured there.

## [2.25.95] - 2026-06-11

### Fixed — prune dead Ethereum RPC endpoints from the price monitor
- Removed `rpc.ankr.com/eth` (now requires an API key — returns `Unauthorized`) and `1rpc.io/eth` (`eth_call` not whitelisted — returns 403) from `contractServiceWrapper` ethereum RPC list. These were producing recurring WARN-level "Chainlink read for ETH failed" noise in `crypto.log`; the fallback chain covered them so there was no trading impact. Ethereum retains `ethereum-rpc.publicnode.com` + `eth.llamarpc.com`. (The same dead providers remain listed for other chains, which have ample working endpoints; left for a follow-up.)

## [2.25.94] - 2026-06-11

### Fixed — Puppeteer scrape crash on sites with hostile element getters (OpenSecrets)
- `scrapeWithPuppeteer` was aborting with `Cannot read properties of undefined (reading 'warn')` on OpenSecrets pages. Root cause: OpenSecrets' `app.js` overrides the element `innerText` getter with code that throws in a headless context. Three page-context call sites read `innerText` unguarded — `getTextContent(document.body)`, the `a[href]`/`img[src]` extraction `forEach`, and `extractMicrodataFromPage`'s `propElem.innerText` — so one hostile getter rejected the whole `page.evaluate` and failed the scrape. Each is now defensive (try/catch fallthrough to `textContent`/skip), and the jsonld/microdata passes are non-fatal. Verified: OpenSecrets org + member-of-congress pages now return full content via Puppeteer (126 links, 4 KB text).
- `executeScrape` final FlareSolverr escalation no longer gated on `renderTier` — any failed/blocked cheerio/Puppeteer result now falls back to FlareSolverr when available (it's the proven-working path for getter-hostile sites). Still a no-op when FS is down or already used.
- `scrapePage` error log now interpolates `error.message` into the line instead of passing it as a dropped winston metadata arg (the failures had been logging blank).

## [2.25.89] - 2026-06-09

### Added — anti-bot fingerprint hardening

- **Persistent anti-bot cookie jar** (`ScrapeCookieJar` Mongoose
  model). Stores `datadome`, `cf_clearance`, `_abck`, `_px`, etc.
  keyed by hostname. Puppeteer primes saved cookies before
  `page.goto` and harvests after, so once a host's challenge is
  passed once (manual solve, or via a real-browser P2P agent),
  subsequent scrapes replay the verified cookie and skip the
  challenge.
- **Real GPU acceleration enabled when `/dev/dri` is present.**
  `stealthBrowser.js` was forcing software rendering via
  `--disable-gpu` — a near-definitive DataDome bot tell, since real
  users always have hardware GPU vendor strings, not SwiftShader.
  Probe for DRI node and pass `--enable-gpu-rasterization
  --enable-zero-copy --ignore-gpu-blocklist --use-gl=desktop`.
- **`rebrowser-puppeteer-core` swapped in as the Puppeteer base.**
  Puppeteer fork patched against modern CDP-detection (Runtime.evaluate
  timing, isolated-world detection, console.log slowdown tells).
  Wired via `puppeteer-extra.addExtra(rebrowser)`; stealth plugin
  stacks on top.

### Fixed / Removed

- **Removed hardcoded "Intel Iris" WebGL spoof.** Was overriding
  WebGL vendor regardless of real GPU, creating an Intel-announced
  / AMD-behavior fingerprint mismatch on Radeon-based boxes. Stealth
  plugin handles WebGL with more nuance.

### Known limitation

NYT still serves the captcha — the datadome cookie we harvest is
the "tracking" variant, not the "verified-pass" variant. Use the
`scripts/ops/import-scrape-cookies.mjs` one-shot CLI to seed the
jar with a real verified cookie exported from a browser that
manually passed the challenge once.

## [2.25.88] - 2026-06-09

### Added

- **puppeteer-extra + puppeteer-extra-plugin-stealth installed.**
  `src/utils/stealthBrowser.js` already wrapped these (with a fall-back
  to plain puppeteer when missing), but the packages had never actually
  been added to package.json — every browser launch was silently using
  plain Puppeteer with all detection signals wide open. Adding the
  packages flips `[INFO] Stealth Puppeteer loaded` on first launch
  and every scraper that calls `launchBrowser()` inherits the evasions.

### Fixed

- **`fullPage:true` was silently ignored on render-tier screenshots.**
  Client report: customer sent `fullPage:true` to `/scrape` and got a
  viewport-sized 1920×1080 PNG. `executeScrape` wasn't destructuring
  `fullPage` from `req.body` and the screenshot call hardcoded
  `{ fullPage: false }`. Now plumbed end-to-end. `viewport` plumbed
  the same way. Default stays `false` for backward compatibility.
- **Scraper no longer returns `success: true` with captcha JS as
  article body.** The bypass chain logged each layer's failure but
  the final result was still labeled `success: true` with `data.text`
  containing 400 bytes of DataDome's captcha JavaScript payload —
  downstream auto-posters would publish that captcha JS as if it
  were the article. Now returns a clean failure response via
  `cacheFailure` when every bypass layer is exhausted and the
...
- **Twitterbot retry gate was a no-op after v2.25.83.** v2.25.83 routed
  the Twitterbot UA retry through Puppeteer (FlareSolverr silently strips
  UA overrides), but Puppeteer doesn't populate `_rawHtml`. The gate
  required `htmlLen > 0 && htmlLen < 8000 && textLen < 500`, so it never
  fired. Same bug shape as v2.25.84's Wayback fix; applied the same
  pattern: `textLen < 500 && (htmlLen === 0 || htmlLen < 8000)`.
- **Added `looksLikeChallengePage` sniff** for DataDome, Cloudflare's
  "Please complete the security check", Imperva/PerimeterX, and JS-
  object-literal captcha payloads. Both Twitterbot and Wayback gates
  treat challenge pages as stubs regardless of length.
- **archive.ph (a.k.a. archive.today) fallback** added after Wayback.
  Surfaces as `archivePhUrl` on the response.
- **removepaywalls.com fallback** added as final layer. Wraps URLs in
  an iframe pointing at periscope.corsfix.com. Set
  `REMOVEPAYWALL_ENABLED=false` to disable. Surfaces as
  `removepaywallUrl`. Includes wrapper-detection so we don't accept
  the "View this full article!" landing page as content.
- Length-check logic refactored into a shared `isUnusableResult` helper.

### Known limitation

Fresh content (< 1-2 days old) from DataDome-heavy hosts (NYT, similar)
still cannot be scraped from datacenter IPs through any of these
layers: DataDome blocks the direct fetch and the Twitterbot UA,
archive.ph gets rate-limited from datacenter IPs (HTTP 429 / serves us
its own CF CAPTCHA), removepaywalls.com loads the article in an iframe
(we extract only the outer wrapper). Realistic remaining paths are
(a) wait for archives to populate after a few days, or (b) integrate
a paid scraper API with residential proxies.

## [2.25.84] - 2026-06-08

### Fixed

- **Wayback gate didn't trigger after Puppeteer retry** because the
  gate required `_rawHtml > 0` which Puppeteer doesn't populate.
  Now uses textLen as the primary signal.

## [2.25.83] - 2026-06-08

### Fixed

- **v2.25.82's Twitterbot UA retry was a no-op.** Modern FlareSolverr
  hardcodes its `User-Agent` to match the embedded browser's
  fingerprint and silently ignores any `userAgent` field in
  `request.get`. Switched the retry to `usePuppeteer: true` so
  `page.setUserAgent` actually takes effect.

### Added

- **Wayback Machine fallback** for stub-shaped responses. Queries
  the archive.org availability API and fetches the closest snapshot
  when v2.25.82's content-paint check (and the now-Puppeteer
  Twitterbot retry) still come back with a paywall/bot-block stub.
  Snapshot provenance surfaces on the response as `waybackSnapshot`
  (timestamp) and `waybackUrl`. ~5s added latency when triggered.

## [2.25.82] - 2026-06-08

### Added

- **Twitterbot-UA auto-retry on known paywall hosts.** Mirror of
  genesis: new `CRAWLER_FRIENDLY_PAYWALL_HOSTS` set in
  `src/api/external/routes/scraping.js` covering 14 major metered
  paywalls (nytimes.com, washingtonpost.com, wsj.com, ft.com, etc.).
  After a successful scrape, if the host is on the list AND the body
  is paywall-stub-shaped (<8KB HTML AND <500 chars text), retry once
  with `userAgent: 'twitterbot'` and adopt the retry result only if
  it's meaningfully bigger (>2× HTML or ≥500 chars text). Triggered
  by a NYT scrape returning a 1,574-byte stub through the otherwise-
  succeeding render-tier path.
- **`twitterbot` user agent** added to scraper plugin defaults
  (`Twitterbot/1.0`).
...
- **Screenshot/PDF wait for real article content, not just title-clear.**
  v2.25.80's wait resolved the original interstitial bug but uncovered a
  follow-on failure on Jetpack-protected WP sites: after the main
  "Checking your browser" interstitial cleared, a SECOND JS verification
  briefly replaced the body with "Error; please refresh to try again."
  before networkidle settled. Title was already the real article title
  by then, so the title-only check passed. Screenshot then captured the
  error overlay. New helper `waitForRealContent(page, {url, timeoutMs})`
  polls the DOM for a standard article container (`article`,
  `.entry-content`, `main`, `[role="main"]`, `.post-content`,
  `#content`, `#main`, `.article-body`) with both a meaningful painted
  size and ≥200 chars of innerText, with an 800-char body-text fallback
  for SPAs. Called after `waitForChallengeResolution` in
  `takeScreenshot` and `generatePDF`; on timeout, both bail with
  `success: false, contentUnresolved: true` rather than ship a capture
  of the overlay. The HTML extraction path also runs the check but
  proceeds regardless — meta-tag extraction has independent value.

## [2.25.80] - 2026-06-08

### Fixed

- **Screenshot and PDF actions now wait out JS bot-check interstitials
  before capturing** (`src/api/plugins/scraper.js`). `takeScreenshot`
  and `generatePDF` previously did `goto` + immediate capture, so on
  sites with a JS interstitial (wp.com / Jetpack
  "Checking your browser", Cloudflare's managed challenge, Akamai's
  "Access denied", etc.) `networkidle2` would fire while the
  interstitial was still up and we'd ship a screenshot/PDF of the
  bot-check page. Meanwhile the HTML extraction path
  (`scrapeWithPuppeteer`) already did wait, so a render-tier scrape
  could return a real article HTML alongside a fake interstitial
  screenshot. Refactor: extracted the title-based wait-and-recheck
  into a module-level `waitForChallengeResolution(page, {url,
  timeoutMs})` helper used by all three render paths. On unresolved
  challenge, the screenshot/PDF action returns
  `success: false, challengeUnresolved: true` so the upstream ships
  no capture rather than a misleading one.

## [2.25.79] - 2026-06-08

### Fixed

- **Memory percentage calculation at 4 legacy call sites.** All used
  `(mem.used / mem.total) * 100` where `si.mem().used` includes
  buff/cache (Linux footgun) — on a box with `free -h` reporting
  13 GiB available out of 19 GiB total (~30% real usage), the
  metric was reporting 92%. On production this fired 71 spurious
  `memory_cleanup` HealingEvents in 24h. All were correctly
  suppressed by the self-healing service's dry-run gate, but they
  were hiding any real signal and would have triggered unnecessary
  GC + cache-clear cycles if an operator had flipped dry-run off.
  Fixed in `selfHealingService.js` `collectSystemState()`,
  `selfDiagnostics.js` `collectSystemInfo()`,
  `selfDiagnosticsEnhanced.js:740` (a duplicate `collectSystemInfo`
  that was missed when the same fix was applied at :336-338 of
  that file), and `selfModification.js:318` (idle-check). All four
  now use `(total - available) / total` with fallback to
  `mem.active` then `mem.used`, matching the already-correct
  pattern at `selfDiagnosticsEnhanced.js:336-338`.

## [2.25.78] - 2026-06-07

Sync from genesis covering three AI-authored PR features (#2235, #2236, #2237). Each PR had a sound idea but a critical bug that would have broken on first call — implemented correctly here. Genesis intermediate releases v2.25.74 through v2.25.77 covered MindSwarm geo-block handling, Mongoose duplicate-index cleanup, two prior PR follow-ups, an emergency-sell RPC-timeout fast-retry, and a residual-sweep / deposit-scan four-layer fix for the recurring 20-min subagent timeout. The non-PR genesis work in v2.25.74–v2.25.77 either touches infrastructure that's already covered in public from earlier syncs or relies on production-only state and isn't surfaced as new public API — only the three PR features and the supporting test files are mirrored here.

### Added

- **`P2PTransfer` progress instance methods + `GET /api/p2p/transfers/:transferId/progress` endpoint.** Schema methods: `getProgressPercentage()` (clamped 0..100), `getTransferSpeed()` (bytes/sec via `totalSize × progressFraction / elapsed` so it doesn't rely on uniform chunk size — the last chunk is typically smaller and a per-chunk-bytes-times-count approach would over-count), `getETA()` (returns `null` when insufficient data so UI can render "calculating…" rather than misleading 0, `0` when complete), `getProgressInfo()` composite snapshot, and a static `getTransferProgress(id)` lean fetch. The new HTTP endpoint in `src/interfaces/web/p2p.js` is gated by `authenticateToken` and surfaces `CastError` → 400 / not-found → 404.

- **`POST /api/vector-intent/search` accepts a `metadataFilters` body field** with Mongo-style per-field operators: `$eq`, `$ne`, `$in`, `$nin`, `$gt`, `$gte`, `$lt`, `$lte`, `$regex` (+ `$options` for flags like `'i'`). Applied post-vector-search in JS over the already-bounded result set. Complements the existing `filters` arg which only supports flat equality at the LanceDB layer. Multiple operators on the same field AND together; unknown operators fail-closed (reject the row) so a caller typo on `$bogus` doesn't accidentally let everything through. `$regex` catches invalid patterns and treats them as no-match (safer than throwing to the caller). Cache key now includes `metadataFilters`. The filter helper is exported (`applyMetadataFilters`) so unit tests can call it directly without routing every case through the express handler.

- **`GET /api/external/images/:token/metadata`** — returns sharp's decoder-level metadata (format / width / height / channels / depth / density / color space / hasAlpha / hasProfile / isProgressive / orientation / animation frame info) plus EXIF/ICC/IPTC/XMP byte-length descriptors (parsing the actual bytes would need separate decoders and was out of scope), plus token-encoded provenance (filename, agentId, expiresAt as ISO string). Auth-gated by `externalAuthMiddleware`; cross-agent access returns 403, expired/invalid tokens 401, file-no-longer-on-disk 404. Token verification correctly imports `verifyDownloadToken` as a named export (the source PR called a non-existent `verifyToken`). Tests use a real sharp-generated PNG + real `generateDownloadToken` — no module-resolution hacks.

### Tests

- `tests/unit/P2PTransfer.test.js` — 9 cases using hydrated-not-saved Mongoose docs (no DB connection required).
- `tests/unit/vectorIntentRoutes.test.js` — 13 cases covering all operators, AND combinations, missing fields, unknown operators, and result rows lacking metadata.
- `tests/unit/imageGenMetadata.test.js` — 4 cases covering token verify, sharp metadata shape against a real generated PNG, tampered-token rejection, and garbage-input rejection.

## [2.25.73] - 2026-06-06

Sync from genesis covering the cumulative v2.25.70 → v2.25.73 delta plus three AI-authored PRs that were merged with manual follow-up commits to make them actually work.

### Added

- **`SYSTEM_TOKEN_ALLOWLIST` in `CryptoStrategyAgent`** (v2.25.70) — hardcoded address set that short-circuits the residual sweep, deposit handler, and `autoSellUnknownToken`. Protects user-owned, intentionally low-liquidity tokens from instant-blacklist branches and from being reported to the on-chain scammer registry as `airdrop_scam`. Add additional system tokens as `network:lowercase_address` entries.
- **Stage tracking in `CryptoStrategyAgent.execute()`** (v2.25.71) — sets `this._currentStage` at each major step (`starting` → `market-data` → `deposit-scan` → `residual-sweep` → `baseline-check` → `indicators` → `strategy:<name>` → `signal-analysis` → `llm-decide` → `idle`). Read by `SubAgentOrchestrator` on a session timeout, so the failure log line says which step blew the 20-min budget instead of just "Crypto Strategy Agent execution failed".
- **`GET /transactions/history`** (from PR #2231 + follow-up) — paginated, date-and-network-filtered transaction history backed by the `HistoricalTransaction` collection. Response shape: `{ success, transactions: [], pagination: { total, limit, offset, hasMore } }`. `address` and `status` query params are accepted-but-ignored placeholders until the schema is extended.
- **SSH plugin `analytics` and `session-report` actions** (from PR #2232 + follow-up) — wraps the new `SSHConnection.generateFilteredSessionReport(startDate, endDate, aggregationLevel)` model method with optional date range and `hourly`/`daily`/`weekly` aggregation. Defaults to the last 30 days at daily aggregation when the caller omits dates. Reachable via `POST /api/plugin { plugin: "ssh", action: "analytics", connectionId, ... }`.
- **Provider-health tracking and retry/fallback on `GeneratedSong`** (from PR #2234) — adds `getProviderHealth`, `updateProviderHealth`, `resetProviderHealth`, `isProviderUnhealthy`, `getHealthyProviders`, `generateWithRetry`, `updateGenerationStatus`, `getGenerationStats` static methods. Purely additive — call sites in `music.js` not migrated to use them yet (separate follow-up).

### Changed

- **`SubAgentOrchestrator` failure log line carries agent ID, failure-mode tag, current handler stage, and elapsed time** (v2.25.71). Previous form `Agent X execution failed: Session timeout: 20 minutes` gave no clue which step was stuck and once produced a misdiagnosis. New form: `Agent <name> [TIMEOUT|FAILED] [id=<oid>] [stage=<stage>] [elapsed=<sec>s]: <error>`. End-session context persists `stage` and `elapsedMs` so they're queryable from Mongo.
- **`numbersapi` plugin survives upstream outage** (v2.25.72) — `numbersapi.com` has been returning 404 for every endpoint as of 2026-06; the plugin used to throw a hard `[ERROR]` on every boot with a full stack trace × retries, blocking it from loading at all. Now `initialize()` throws a typed `UPSTREAM_UNAVAILABLE` error that `apiManager.loadPlugin` recognizes alongside the existing missing-credentials path — plugin is registered as **disabled** with a clean WARN. `makeRequest()` short-circuits when the upstream is known dead, with a 10-min opportunistic recovery cooldown (one successful response flips the flag back to available).
- **`apiManager.loadPlugin` recognizes two soft-fail shapes** (v2.25.72) — missing credentials (legacy) and `UPSTREAM_UNAVAILABLE` (new). Both register the plugin with `enabled:false` and a `disabledReason` field (`'upstream_unavailable'` or `'missing_credentials'`) so the operator sees both in the UI with the right context.
- **Twitter and MindSwarm auto-posts are hard-validated against repetition + hallucination** (v2.25.73). 10 days of audit showed both surfaces producing the same 5–6 templates on rotation, with light-rewrite repetition the topic-keyword dedup missed and outright hallucinated facts not present in the grounding context. New validators in `src/utils/autoPostFilter.js`: `isBadOpener(text)` matches the actual banned templates ("Just shipped", "Operating with", "Now offering", "I'm now", etc.), `repetitionConflict(candidate, recentPosts, opts)` does 4-gram overlap detection on lossy-normalized text against the 8 most recent posts, and `groundingAnchor(candidate, itemTexts)` requires the candidate to quote at least one numeric token or distinctive word from the items the AI was actually given. Each surface now runs a 3-attempt validate-and-retry loop with rejected drafts fed back to the AI as DO-NOT-REPEAT examples.

## [2.25.69] - 2026-06-04

Bulk sync from the genesis branch covering the cumulative v2.25.56 → v2.25.69 delta. Adds the salvageable output of a 39-PR alice-lanagent self-modification triage round (PRs #2192–#2230 on genesis), plus the cumulative bug-fixes and small features that landed on genesis between v2.25.55 and v2.25.69 — including the v2.25.68 swap-amount decimal-clamp fix that was silently dropping crypto buy-the-dip entries.

### Added

- **Models**
  - `Email.batchProcessByFilter(filter, opts)` — paged bulk-action primitive that delegates per-page work to `batchProcessEmails`, supports markAsRead / markAsProcessed / pageSize / maxDocs.
  - `ExternalAuditLog` pre-save hook — top-level sensitive-key redaction (password / token / authorization / cookie / api-key headers / private_key / mnemonic / encryptedSeed) and UTF-8-safe 10 KiB truncation of request/response bodies before persistence.
  - `ApiKey.hasScope(required, {mode})` + `normalizeScopes` + wildcard/namespace scope matcher (`*`, `repo:*`, `a:b:*`). Pre-save normalization for canonical storage.
  - `SkynetTokenLedger` idempotent `recordPurchase` / `recordSale` accepting `{idempotencyKey, txHash, network}`. Log-first design + unique partial index on `{idempotencyKey, transactionType}` makes the historical log the gate, so two concurrent calls with the same key can't double-mutate the ledger. Sale path uses `findOneAndUpdate({amount:{$gte:n}}, {$inc:{amount:-n}})` for race-safe atomic decrement without requiring a Mongo replica set.
  - `MCPToken` sha256 `lookupKey` field — O(1) indexed `validateToken` lookup instead of scanning every active token with bcrypt. Bcrypt still runs to verify the match. Opportunistic backfill for pre-existing legacy tokens on next validation.
  - `CalendarEvent.toICS({uid, url, includeReminders})` + `CalendarEvent.toICSFeed(query, opts)` — RFC 5545 VEVENT / VCALENDAR export with UTC date formatting, 75-octet UTF-8-safe line folding, ATTENDEE / PARTSTAT / VALARM (with optional REPEAT/DURATION) / RRULE / EXDATE / configurable UID domain via `CALENDAR_UID_DOMAIN` env. NodeCache 60s feed cache.
  - `ExternalPayment` blockchain finality tracking — `finalityRequired` + `finalizedAt` schema fields, `updateConfirmationsByTx(txHash, fields)` with per-chain confirmation defaults (`bsc:15, eth:12, polygon:100, avalanche:20, arbitrum:12, optimism:12, base:12`) and atomic `$max` for monotonic fields.
  - `ModelCache.canonicalizeModels` + `computeHash` (sha256) + `getContentHash` + `contentHash`-based change detection in `updateCache` — replaces fragile `JSON.stringify(a) !== JSON.stringify(b)` comparisons that spuriously flagged "changed" on field-order differences. `compareVersions` lowercases IDs so provider casing changes don't show as added/removed.
  - `DailyPnL.getAggregatedPnL({groupBy, startDate, endDate})` (daily / weekly / monthly / quarterly with corrected ISO `%G-W%V` week format and `ceil(month/3)` quarterly math) + `analyzeTrends({windowSize, metric})` with moving average and percent delta.
  - `DevelopmentPlan` progress (0-100) + milestones schema fields, `updateProgress` + `addMilestone` statics with input validation. Two new entries in `DevelopmentPlan.commands` for NL surface.
  - `P2PPeer.calculateConnectionStability()` + persisted `averageSessionDuration` / `disconnectionFrequency` derived from existing counters.

- **Routes / API**
  - `GET /api/external/admin/payments/summary?since&until&groupBy&includeUsd` — aggregation over ExternalPayment grouped by currency / chain / serviceId, sums of amount / usdValue / creditsIssued / count.
  - `GET /api/external/trust/level/stats` — exposes the existing `trustRegistryService.getTrustStats()` (total + distribution by level + distribution by source).
  - `GET /api/external/catalog/health` — cache + db status (no auth — pure liveness probe).
  - Catalog `GET /` now accepts `q` / `type` / `sort` / `fields` / `limit` / `offset` and emits `X-Total-Count` + `X-Offset` + `X-Limit` headers. Backward compatible when no query params are supplied.
  - ENS `/available/:name` and `/expiry/:name` are now NodeCache-backed (2-min TTL, response carries `cached: true` on hit).
  - MessageBird plugin gains `getMessageStatus({messageId})` action.
  - `POST /api/accounts/bulk-register` and `PATCH /api/accounts/bulk-status` switched to `Promise.allSettled` with per-item structured results `{index, success, account|error}`, summary `{total, succeeded, failed}`, HTTP 207 Multi-Status on mixed outcomes. Cache invalidation only fires when ≥1 item actually succeeded.
  - Vector intent `POST /search` accepts `minSimilarity` body param and filters using the store's existing `similarity` field.

- **Services / utilities**
  - **Swap decimal clamp** (carried from genesis v2.25.68): `safeParseUnits(ethers, amount, decimals)` helper in `swapService.js` clamps JS amount precision to the token's decimal count before `ethers.parseUnits`, preventing the `NUMERIC_FAULT / underflow` that was silently killing USDC dip-buys on Ethereum. All 9 variable-amount callsites converted.
  - `logger.js` — winston `redactFormat` that deep-walks `info.message` and metadata, masks sensitive top-level keys (password / token / cookie / api-key headers / private_key / mnemonic / encryptedSeed), redacts JWT and Bearer tokens via conservative regex. `configureRedaction({keys, patterns, mask, maxStringLength})` for runtime tweaks. Truncation cap 8 KB so stack traces aren't cut mid-frame.
  - `imageGenerationService` — in-flight de-dup Map for concurrent identical generate() calls, NodeCache short-TTL result cache for repeat-prompt requests. `settings.idempotency.{enabled, ttlSeconds}` knobs.
  - `registryClient` — per-beat heartbeat ACK watchdog. After N missed beats in a row (default 2), force `ws.terminate()` to break out of half-open WebSocket instead of waiting for TCP FIN.
  - `selfModLock.renew(service, {extendMs})` + `acquire(service, {keepAlive: true | config})` — lease renewal for long-running operations that exceed the 30-minute lock timeout. `release()` auto-stops keep-alive.
  - `mcpTransport` — `AbortSignal`-driven request cancellation. `request(method, params, {timeout, signal, cancelMethod})` overload (legacy numeric arg still works). `AbortError` and `TransportClosedError` exports. `close()` rejects pending with `TransportClosedError` and cleans up abort listeners.
  - `intentIndexer.generatePooledEmbedding(text, {maxCharsPerChunk})` — splits long text on `|` then sentence boundaries, embeds per chunk, mean-pools to a single vector. Avoids the silent ~512-token truncation that loses information for multi-example intents.
  - `repoInfo` cross-host VCS — `parseGitUrl(urlOrSlug)` for SSH / HTTPS / plain slug across github.com / gitlab.com / bitbucket.org / any custom host. `normalizeRepoInput`. `getOriginIdentity` / `getUpstreamIdentity` returning `{host, owner, repo, slug, source}` (source ∈ {'git', 'env', 'fallback'}). Env priority chains `GITHUB_REPOSITORY → AGENT_REPO_SLUG → GITHUB_REPO` for origin and `UPSTREAM_REPOSITORY → UPSTREAM_REPO` for upstream. Legacy `parseGitHubUrl` / `getOriginRepo` / `getOriginSlug` / `isGenesisInstance` unchanged.
  - `scanProgressCache.warmCacheForSession(ScanProgress, sessionScanId)` — pre-loads per-status counts and pending entries in parallel.
  - `skynetPrice` — optional `{minPrice, maxPrice, maxPctChange}` inputs to `getSkynetUsdPrice` flag anomalous swings without dropping the reading (anomalous prices are real data; returning null was the same shape as "oracle failed" and would make consumers fall back to stale caches at exactly the wrong moment). `getSkynetOracleHealth()` returns last-price + capped anomaly history.
  - `flareSolverr` — `fsListSessions` / `fsForceCleanup` (list-then-destroy per session id, since `sessions.destroy` without a session id is not a valid upstream command) / `fsGetStats`.
  - `killSwitch` scheduled maintenance windows — `kill_switch_schedule` array `[{start, end}]` persisted in PluginSettings (survives restarts), middleware blocks on `manual_active || scheduled_active`, `setKillSwitchSchedule(schedule)` validates each window before persisting, `getKillSwitchSchedule()` returns a copy.
  - `metricsUpdater` per-job `_runningCurrent` / `_runningPrev` overrun guards checked on cron entry and inside the methods, cleared in `finally`.
  - `agentAccessor` wrapped in `EventEmitter` — services can subscribe to `initialized` / `destroyed` / `reconfigured` lifecycle events. `getAgentState()` enumerates real plugins via `agent.apiManager.apis`. `setGlobalAgent` / `getGlobalAgent` function exports preserved for backward compat.
  - Telegram interface `createInterface(type, options)` — lazy-loading factory with explicit per-type → export-name mapping (avoids the trial-fallback chain that would silently return the wrong class if any module re-exports another's name). Named exports retained for static-import callers.
  - `log-waterfall.js` (frontend visualization) — rolling per-service error-rate anomaly detection (60s window, 0.2 threshold), header alpha pulsing via sine wave when boosted, red column outline scaled by boost, error-biased drop spawning. Configurable via `anomalyWindowMs` / `errorRateThreshold` / `maxBoost`.
  - **Groq API plugin** — `list_models` / `generate_completion` / `chat_completion` against the Groq OpenAI-compat layer at `https://api.groq.com/openai/v1`. Loads `GROQ_API_KEY` from env or the Settings > Plugins > Credentials UI. Standard BasePlugin shape with NL examples per command.

### Fixed (cumulative v2.25.56–v2.25.69 fixes synced from genesis)

- **Swap precision underflow** (v2.25.68): `ethers.parseUnits` was throwing `NUMERIC_FAULT / underflow` on USDC amounts wider than 6 decimals, silently killing every threshold-triggered buy on Ethereum. Three buys went missing in one morning (05:16 / 05:18 / 05:26 UTC) before the bug was caught. Fix is the `safeParseUnits` helper above.
- **Social post copy** (v2.25.67): removed false "reasoning across N providers / cross-checking" candidate-post line that twitter.js + mindswarm.js were feeding into the LLM as a topic — providerManager only does sequential single-provider fallback, no consensus or cross-check.
- ***arr `/update` 500s spam** (v2.25.66): all 5 *arr plugins log a stack trace every morning when Servarr's upstream update server flaps 5xx; switched the `/update` call to a one-shot axios that falls straight through to the GitHub-releases fallback.
- **MetricsUpdater Mongoose CastError** (v2.25.65): `trends.byType` subdoc had a field named `type: String`, which Mongoose interprets as SchemaTypeOptions and compiled the array as `[String]`. Renamed to `improvementType`.
- **yt-dlp concurrent-invocation race** (v2.25.65): class-level `_inflight` Map de-dups parallel identical downloads onto a single yt-dlp invocation. Catch blocks now log stderr/stdout/exit-code instead of silently returning the error in the response body.
- **HuggingFace `reasoning_effort: 'minimal'`** (v2.25.64): `normalizeAdditionalParams` helper maps `minimal → low` before forwarding to the HF router (which only accepts low/medium/high).
- **Crypto P&L journal/dashboard** (v2.25.63): realized-only P&L history, dashboard double-count fix, journal endpoint corrected.

## [2.25.55] - 2026-05-25

`BugFixingService` kept two independent enable flags — `this.enabled` (top-level, what the runtime + API status payload report) and `this.config.enabled` (the persisted value loaded from `agent.serviceConfigs.bugFixing`). `loadConfig()` only refreshed `this.config.enabled`, so a saved `enabled:false` was lost on restart and the service kept running on the constructor default `true`. `selfModification.js` already syncs the two correctly; `bugFixing.js` was the outlier.

### Fixed

- **`src/services/bugFixing.js` `loadConfig()`** — added `this.enabled = this.config.enabled` after both the merge-from-DB branch and the no-saved-config branch. A persisted disable now actually disables the service after restart. Log line now includes the resolved `enabled` value for post-restart auditing.

## [2.25.54] - 2026-05-24

Operational + audit version bump — no code changes since v2.25.53. Documents three end-of-session observations that are worth pinning for continuity.

### Operational changes (no code, just live config + capital)

- **[redacted-token] TokenTrader `capitalAllocationPercent` 50 → 65** on ALICE via `POST /api/crypto/strategy/token-trader/configure`. Position state and all other settings preserved.
- **Stable-pool allocation adjusted.** Combined with the 65% allocation, next-cycle grid buys scale up under smart-sizing, entering the "watch slippage" zone. First buy under the new sizing is the validation point.

### Audit observations (no fix today — documented for next session)

- **[redacted-token] smart-sizing fix from v2.25.50 had its first real production test today.** Yesterday's 12/12 grid-buy cap-jam cleared in a single 1.6h window (21:33–23:13 UTC): grid sells fired at +1.1–1.8% adaptive triggers; `consecutiveGridBuys` reset 10 → 0. Strategy is back in its natural buy-low/sell-high grind.
- **SKYNET auto-stake activity worth noting:** the `skynetAutoStaker` service fired two surplus-threshold-triggered stakes on 2026-05-23. The new `skynet-vault-compound` Agenda job ran on its 12h cadence, auto-claiming rewards + vault-bounty. All on-strategy per the SKYNET-as-governance-equity memo.
- **Flagged for follow-up**: `skynet-staking-autoclaim` (daily 06:00 UTC cron) and `skynet-lp-staking-autoclaim` (daily 06:30 UTC cron) last ran 2026-05-21 — three days ago — while the new `skynet-vault-compound` job is running cleanly on its 12h cadence. Could be: (a) ran but completed silently with nothing to claim, (b) Agenda issue dropped the schedule, or (c) `lastRunAt` got reset by something. The compound-bounty path still catches rewards, so it isn't urgent, but worth a deeper look next session.

### Logging quirk worth pinning (caught during today's crypto audits)

- The plain-text `crypto.log` file uses **local PT timestamps**, NOT UTC. Its `14:33` corresponds to `21:33Z` in `structured.json` (which uses proper UTC `Z` suffixes). For accurate time-of-event correlation in future log audits, `structured.json` is the source of truth; cross-reference plain-text logs by event content rather than raw timestamp comparison. Not a bug, just an inconsistency in the existing logger config.

## [2.25.53] - 2026-05-23

Gateway raster-logo endpoints for ad-platform embedding. The api.lanagent.net landing page already ships its robot-icon branding as an inline SVG favicon, but ad networks need a standalone PNG URL with cross-origin-friendly headers — most reject SVG, and helmet's default `Cross-Origin-Resource-Policy: same-origin` blocks third-party renderers from embedding the image.

### Added — gateway (separate repo `api-lanagent-net`, commits `dc12635` + `4bb0a47`)

- **`GET /logo.png`** — 512×512 PNG (default). `?size=1024` returns the 1024×1024 variant.
- **`GET /logo-512.png`** — explicit 512 variant.
...
Operational + provider-routing + PR-round day. Major focus areas: stopping the Anthropic-hardcoding leak that was burning credits on a deprecated provider while ALICE was configured for OpenAI; making the AI-detector install actually work on a fresh box; processing the day's PR queue; and a four-fix MindSwarm pass.

### Fixed — provider routing

- **`src/api/plugins/websearch.js`** — `defaultOrder` was hardcoded `['anthropic', 'openai']` and the web-search flow then *switched the active provider* to whichever came first. With both keys registered, anthropic always won — every web search bypassed the agent's `aiProviders.current` (which on ALICE is `openai`/gpt-5). Now: caller preference → current provider → `['openai', 'anthropic']` fallback. PR-review code paths (`prReviewer.js`, `PRReview.js`, web-UI PR settings) intentionally keep Anthropic as their default per the operator's intent. Verified live: `Generating response using provider: openai, model: gpt-5` on next web search, zero new `credit balance too low` errors since deploy.
- **`src/services/capabilityIncrementalScanner.js`** — initial `currentProvider = 'anthropic'` and the error-path fallback `return { provider: 'anthropic', ... }` switched to `'huggingface'` (the conservative context-limit registry entry). This file only labels context-limit lookups, so the safer default avoids misrouting downstream calls if provider detection fails.

### Fixed — Puppeteer / scraping

- **`src/utils/stealthBrowser.js`** — added `protocolTimeout: 180000` to `pptr.launch()` defaults. The default 30s was triggering unhandledRejections on CDP `Network.enable` for slow Cloudflare-gated pages.

### Fixed — gateway (separate repo `api-lanagent-net`, commit `9853d95`)

- **`index.mjs:2128`** — single missed call site from the 2026-05-08 `new: true → returnDocument: 'after'` mongoose migration, inside `/agents/register`. Because that endpoint fires on every agent refresh, the one line was producing a continuous stream of `[MONGOOSE] Warning: the 'new' option for findOneAndUpdate() is deprecated` under the live PID. Fixed and pushed; gateway error log now clean under the post-restart PID.

### Added — AI-detector install path

- **`src/services/ai-detector/requirements.txt`** — added `torch`, `numpy`, `transformers` (`detector_service.py` imports them but they were never listed).
- **`scripts/setup/install.sh`** — new post-npm-install step apt-installs `python3-venv` / `python3-pip` / version-pinned `python3.X-venv` (the generic metapackage isn't enough on Ubuntu 24 — `python3.12-venv` needs to be installed explicitly), creates `src/services/ai-detector/.venv`, then `pip install --extra-index-url https://download.pytorch.org/whl/cpu -r requirements.txt` (CPU wheels keep it ~200 MB instead of the ~2 GB CUDA defaults).
- **`ecosystem.config.cjs`** — `ai-detector` interpreter now resolves to the venv python when it exists, falls back to system `python3` otherwise. Existing installs without a venv keep their current behaviour until they re-run `install.sh`.
- Verified end-to-end on delta and ALICE: `/health` → `{"status":"ok","device":"cpu","cuda_available":false}`. ALICE migrated additively (created the venv, kept `.env` / Mongo / trading state / WireGuard keys untouched).

### Added — PR round (#2187 / #2190 / #2191 merged; #2188 + #2189 reimplemented; #2187 test dropped)

- **#2187 viz-dashboard** (merged) — URL-hash deep-linking (`#viz=agent-brain`) + `localStorage` persistence of the last active tab. Hash takes priority on init, then localStorage, then first viz. `hashchange` listener for cross-tab navigation, `removeEventListener` in `destroy()`, try/catch around storage for private-mode safety. The accompanying `tests/unit/viz-dashboard.test.js` was removed in `ec232969` — it imported a browser script that has no exports as if it were an ES module, so it failed immediately on `node --test`.
- **#2188 `stableJsonStringify`** (closed + reimplemented in `ec232969`) — deterministic JSON for hashing/diffing/caching. Sorted keys (default lexical, custom comparator optional), handles circular refs (`'[Circular]'`), BigInt→string, Date→ISO 8601, Buffer→base64, Map→sorted object, Set→sorted array. Matches `JSON.stringify` semantics for undefined (objects drop, arrays nullify). The PR's own test mixed CJS into pure-ESM (`const { Module } = require('module')`) and would never have run; reimplementation ships with a 14-test suite that does (`node --test tests/unit/jsonUtils.stable.test.js`).
- **#2189 memoryVectorStore caching** (closed + reimplemented in `ec232969`) — cache `search()` and `findDuplicate()` in the existing NodeCache, hash-based cache keys (SHA-256 of Float32 embedding → 16 hex chars instead of raw `.join(',')` ≈ 30 KB), single code path (the PR duplicated whole function bodies in if/else branches), invalidate on real writes only (`flushBatch`, `deleteMemory`, `deleteMemories`, `rebuildIndex` — not on `addMemory` which only queues). Env flags: `ENABLE_SEARCH_CACHE`, `ENABLE_DUP_CACHE`, `SEARCH_CACHE_TTL`. 7-test suite. The PR's test monkey-patched non-existent method names (`searchMemories`, `isDuplicate`) and never exercised the real cache code path.
- **#2190 freshping searchchecks** (merged) — client-side filter over `getChecks()` by name/url substring (case-insensitive) and/or status. Returns `{ success, data, count, total }`. No new deps, standard plugin pattern.
- **#2191 lpMarketMaker idempotency keys** (merged) — `Idempotency-Key` header (or `body.idempotencyKey`) on every mutating route (`/enable`, `/disable`, `/open`, `/close`, `/rebalance`, `/collect`, `/schedule`). Uses the existing NodeCache, per-op namespacing (`lpmm:<op>:<key>`) prevents cross-op collisions, configurable TTL via `LP_MM_IDEMPOTENCY_TTL_SECONDS` (default 600s), errors deliberately NOT cached so transient failures can be retried. Also collapses ~50 lines of try/catch duplication into a `handleIdempotentMutation` helper. Real value for blockchain ops where client retries could double-execute.

### Added — MindSwarm plugin (`src/api/plugins/mindswarm.js`)

Four coupled fixes, two addressing day-of feedback from the MindSwarm-side dev.

- **First-hello post on auto-register** (`_postFirstHello`) — fires once after `_setupProfileAfterRegistration`. Privacy-safe by construction: agent name + host OS family + `#lanagent #firstpost`. AI-generated when a provider is available (explicit no-personal-info prompt + length + keyword guards) with a fixed-template fallback. Failure is non-fatal — never blocks registration. So new agents arriving at the network are *visible* on the social side. Existing agents (already registered, just logging in) skip this — the path only fires when there are no saved credentials.
- **Token-refresh race fix** (`_getRefreshPromise`) — MindSwarm uses single-use refresh tokens with replay detection (since 2026-05-09). Two parallel pollers each hitting 401 at the same minute would each call `/auth/refresh` — the second one's refresh token having already been rotated/invalidated by the first, forcing repeated full re-logins (~49/161 `/notifications` calls were 4xx'ing). Now mirrors the pattern in MindSwarm's own web frontend (`frontend/src/lib/api.ts:46-59`) — all 401 callers `await` the same in-flight promise; cleared only after resolve.
- **403 banned/suspended/email-verification detection** — `_apiRequest` now detects these in the catch path, drops `accessToken`/`refreshToken`, sets `_engagementConfig.enabled=false`, persists, and throws a `permanent: true` error that the new `customErrorClassifier` short-circuits past `retryOperation`. Stops the retry-every-5min-forever pattern against a dead account. (Defensive — ALICE's `alice_lanagent` account is active; this protects us if the account ever gets suspended.)
- **Poll-cadence backoff** —
  - `src/services/scheduler.js`: `agenda.every('5 minutes', 'mindswarm-engagement')` → `'15 minutes'`. `/notifications` hits/day per endpoint: 288 → 96.
  - `_scanAndReplyToUnrepliedPosts` inside `_engagementCycle` is now gated to once per hour via `_lastReplyScanMs`. Was running every cycle and pulling `/users/me/posts` + `/posts/:id/replies` for up to 10 posts + sub-reply checks (~30 sub-reads × 288 cycles ≈ 8.6 k/day on the bot's own profile). Now ~720/day.
- **Why not full socket.io migration** — looked at the MindSwarm backend; the only events emitted via `io.to/.emit` are `new-message`, `post-liked`, `post-created` (self only), `messages-read`, `message-reaction`, `typing`, and `notification` (only on crypto tips). `Notification.createNotification` (the model-level helper that reply/follow/mention/like notifications flow through) does push + email but does not `io.emit`. A socket-only architecture would miss the bulk of notification traffic until the backend also emits from `createNotification`. Deferred to a future round on the MS side.
...
- **ALICE backup taken** at the end of session — `<backup-archive>` (2.8 GB) at `<local-workspace-path>`. Captures wallet addresses and API-key inventory in the manifest for restore-time verification.

## [2.25.51] - 2026-05-22

PR triage round: 21 open ALICE-authored capability-upgrade PRs (#2166–#2186) reviewed one by one. All 21 had architecturally-sound ideas (standard caching, atomicity, idempotency, replay protection patterns). All 21 manually re-implemented on current main rather than rebased — the PR branches were weeks-stale and a clean rebase would have churned 50+ files per PR even though the actual intent was 1 source file each.

For each PR: read the diff, verified imports were already present or needed adding, checked for hallucinated APIs against actual SDK/library docs, dropped or substituted any bad pieces, applied the rest. Each PR will get a close-comment pointing to this commit. No tests applied — the auto-generated test files use a Module._load CJS stubbing pattern that doesn't work cleanly in ESM; production code quality was verified independently of test runnability.

### Added — model atomicity / idempotency / caching statics

- **`src/models/UserPreference.js`** (#2166) — `updatePreferencesPartial(plugin, userId, category, { set, unset, expectedVersion })` static. Compare-and-set via `expectedVersion`; throws `VersionConflict` on mismatch. Snapshots current preferences into `UserPreferenceHistory` before mutating; non-blocking on history write failure. Uses `$set` for nested paths and `$unset` for deletions, never overwrites the whole map.
- **`src/models/Project.js`** (#2167) — `addTaskAtomic` / `addTasksAtomic` / `removeTaskAtomic` / `removeTasksAtomic` statics. All use `$addToSet`/`$pull` against `tasks` array without loading the full document; invalidate `tasks_<projectId>` cache on success.
- **`src/models/PRReview.js`** (#2171) — `PRReviewSettings.updateStatsAtomic(agentId, deltas, historyEntry)` atomic counter increments + bounded `stats.reviewHistory` append via `$push`/`$slice`. `PRReviewHistory.recordOutcome({…, runId})` upserts by `runId` to dedupe in-flight duplicate writes; falls back to `create()` when runId not provided. Counter deltas derived from `action` and `deploymentStatus`. New compound index on `{agentId, prNumber, createdAt}`, new `reviewHistoryMax: 200` setting (capped per-agent).
- **`src/models/PluginSettings.js`** (#2172) — `getCached` now supports stale-while-revalidate. Backwards-compatible: legacy `(name, key, ttl)` signature still works; new options form `(name, key, { ttl, staleWhileRevalidateSeconds })`. Cache entries become `{value, softExpiresAt}` with ±10% jitter. In-flight dedup map prevents cache stampedes on cold load. `post(save/update/delete)` hooks + `clearCache()` also invalidate `inFlight`.
- **`src/models/ExternalPayment.js`** (#2176) — `acquireNextVerified(serviceId)` atomic FIFO claim via `findOneAndUpdate` + sort; `consumeIfUnconsumed(id)` idempotent flip; `markVerifiedByTx(txHash, fields)`. Two new compound indexes for queue scans.
- **`src/models/FeatureRequest.js`** (#2175) — `contentFingerprint` schema field (unique sparse), `pre('validate')` hook computes sha256 over normalized `{title, description, category}`. `upsertByFingerprint(params)` static for race-safe creation via `findOneAndUpdate` + `$setOnInsert`; falls back to `findOne` on E11000 duplicate-key race.
- **`src/models/MCPServer.js`** (#2181) — in-flight dedup + negative caching (30s TTL on null) on `getCached`/`getByName`/`getEnabled` hot paths.
- **`src/models/HealingEvent.js`** (#2183) — `isInCooldownScoped(eventType, targetService, condition, cooldownMinutes)` cache-first DB-fallback scoped cooldown check. `setCooldownScoped(...)` for cache-only arming without DB write.
- **`src/models/P2PPeer.js`** (#2184) — `acceptSeq(fingerprint, seq)` atomic monotonic sequence advancement (returns false on duplicate/out-of-order — replay protection). `tryAdvanceSeq(seq)` instance variant scoped to `_id` that also updates in-memory `lastSeq`.
- **`src/models/DeviceAlias.js`** (#2185) — `resolveAlias` and `resolveAliases` now use positive cache + negative cache (60s TTL) + in-flight dedup. Single-alias version uses per-key in-flight map; batch version uses single $or-query and negative-caches the misses. `setAlias`/`clearCache`/`cleanupExpiredAliases` also clear `aliasNegativeCache`.
- **`src/models/DiagnosticReport.js`** (#2170) — alert throttling: `alertCooldownSeconds` (default 900s), `maintenanceWindow` (HH:MM-HH:MM, handles midnight crossing), `dedupeBy` (`overall`/`summary`/`hash`). LRU-bounded `recentSummariesCache` (100 keys, 1h TTL). `reportEventEmitter.on('statusChange')` now logs unconditionally but suppresses email sends during maintenance window or active cooldown. PR also deleted the `bugFixing` schema block (stale-main artifact) — that deletion was NOT applied; only the alert-throttling changes were.

### Added — utility / service / route upgrades

- **`src/utils/projectContext.js`** (#2186) — fuzzy project-name matching via Damerau-Levenshtein (short tokens) + Jaccard similarity (multi-token phrases) + alias map from `agent.config?.projectContext?.aliases` plus hardcoded defaults. Exact matches still win at score 1; fuzzy matches get discounted scores and the best candidate is returned (recency tiebreak).
- **`src/utils/gravatarHelper.js`** (#2168) — in-flight dedup, negative caching (5min TTL for 404s), simple circuit breaker (opens for 60s after 5 failures). Env-configurable via `GRAVATAR_FAIL_THRESHOLD` / `GRAVATAR_BREAKER_COOLDOWN_MS` / `GRAVATAR_NEGATIVE_TTL_SECONDS`.
- **`src/api/contracts.js`** (#2174) — `getCachedData` upgraded to in-flight dedup + stale-while-revalidate. Cache entries are `{data, expiresAt, swrTtl}`; stale data is served immediately while a background refresh fires. Legacy primitive cache entries are accepted for backwards compat. Optional `?revalidate=true` query param bypasses cache on safe endpoints. Falls back to stale cached data on fetch error.
- **`src/services/rag/documentLoader.js`** (#2173) — deterministic sha256 content fingerprint + stable `documentId` (`source#hash`) added to metadata across TextLoader, PDFLoader (per-page when split), WebLoader, MarkdownLoader, JSONLoader (per-item with `#index=N`), XMLLoader.
- **`src/providers/anthropic.js`** (#2169) — **partial salvage.** PR's headline `response_format: { type: 'json_schema' }` mechanism is hallucinated for Anthropic (their SDK uses tool-use `input_schema` instead). Skipped that piece. Salvaged: when caller passes `options.jsonSchema`, system prompt is augmented with "respond with ONLY valid JSON" guidance; response is parsed with `safeJsonParse` fallback and validated against the schema; `options.strictJson: true` triggers a one-shot retry with a stronger "JSON only" system prompt on parse failure. Result includes `contentJson` field when parsing succeeds.
- **`src/api/external/routes/jobs.js`** (#2177) — `Idempotency-Key` header (or `body.idempotencyKey`) on `POST /create` and `POST /:jobId/fund`. Validated as 1–128 ASCII (`/^[a-zA-Z0-9-_]+$/`). 1h NodeCache stores responses keyed by idempotency key — repeated requests return cached response with `cached: true`. Key threaded through to `agenticCommerceService` methods (ignored if service doesn't accept it — non-breaking). Also added `GET /api/external/jobs/health` liveness probe.

### Added — plugin extensions

- **`src/api/plugins/zapier.js`** (#2182) — `trigger_webhook` command. Direct HTTP POST/PUT/PATCH to a Zap Catch Hook URL (or any webhook URL) without going through the Zapier REST API. Validates URL format and HTTP method.
- **`src/api/plugins/aviationstack.js`** (#2178) — three new commands: `getFlightsByRoute({depIata, arrIata, date, limit, offset})`, `getAirlineFlights({airlineCode, date, limit, offset})`, `getLiveFlightPosition({flightNumber, date})`. The position query returns a slimmed `{latitude, longitude, altitude, direction, speed, updated}` object from the first matching record's `data.live`.
- **`src/api/plugins/prowlarr.js`** (#2179) — `search_advanced` command supporting Prowlarr/Jackett `t=` types (`search`/`movie`/`tvsearch`/`music`/`book`) with media identifiers (`imdbId`/`tmdbId`/`tvdbId`), episode context (`season`/`episode`), category and indexer filters, pagination (`limit`/`offset`), and client-side sorting by `seeders`/`size`/`publishDate`. Type aliases (`tv` → `tvsearch`, `movies` → `movie`) accepted.
- **`src/api/plugins/tokenProfiler.js`** (#2180) — `compare({addressA, addressB, network})` command. Returns side-by-side safety score, top holder %, taxes, honeypot/mintable flags, ownership-renounced status, liquidity info; `winner` field is the higher-scoring address (or 'tie').

### Pieces deliberately NOT applied from PRs

- **#2170 DiagnosticReport** — PR also deleted the `bugFixing` schema block. Not applied — that block is in use by the bug-fixing service on current main; the deletion was a stale-main rebase artifact.
- **#2169 anthropic** — `response_format: { type: 'json_schema' }` was the headline mechanism but is hallucinated for the Anthropic SDK. The post-parse + strict-retry pattern was kept as best-effort JSON output.
- **All test files** — every PR shipped a `tests/unit/X.test.js` file using `Module._load` CJS stubbing patterns that don't work cleanly in ESM. Production code applied; test files skipped.

## [2.25.50] - 2026-05-22

Two crypto-strategy improvements driven by a deep-dive on [redacted-token]'s stuck state today. After 12 consecutive grid buys [redacted-token] was capital-jammed with only ~45% of its allocated stable deployed — the per-buy sizing of `gridTradePercent (5%) of remaining stable` produces geometric shrinkage and hits `maxConsecutiveGridBuys` long before reaching full deployment. Separately, DollarMaximizer had been idle for 14+ hours because its buy thresholds (ETH -4%, BNB -5%, MATIC -6%, further loosened by ~1.35× in downtrend regime to -5.4% / -6.75% / -8.1% adjusted) almost never fire in a calm market.

### Changed

- **TokenTrader smart-sizing** (`src/services/crypto/strategies/TokenTraderStrategy.js`) — the grid-buy case now uses `max(pct_of_remaining_stable, remaining_deployment_capacity / remaining_grid_buys)`. Early buys still use the gentle percent-of-remaining DCA; later buys ramp up to ensure that by the time the consecutive-buy thesis cap fires, total deployment has reached `maxStableDeploymentPercent`. Two new config defaults: `minGridBuyUsd: 8` (skip uneconomical micro-buys regardless of percentage math), `maxStableDeploymentPercent: 90` (cap on positionCostBasis / totalPool — reserves 10% for emergency-sell scenarios). The 12-buy thesis-validation cap is unchanged. Per the deployment-cap math, smart sizing produces ~$23/buy across 12 buys for [redacted-token]'s current pool size, deploying the full [redacted] of remaining capacity (vs the old behavior of $14/buy at buy #12 with [redacted] stable still sitting idle when the cap fired).
- **DollarMaximizer threshold tightening** (`src/services/crypto/strategies/DollarMaximizerStrategy.js`) — asset buy thresholds tightened: ETH `-4 → -2.5`, BNB `-5 → -3`, MATIC `-6 → -4`. Sell-side and stop-loss thresholds unchanged. Combined with the downtrend regime multiplier (~1.35×), adjusted buy triggers are now -3.4% / -4.05% / -5.4% in downtrend (vs the previous -5.4% / -6.75% / -8.1%). The existing `minProfitGasMultiplier: 2` guard continues to block any trade whose realized profit doesn't beat gas cost by ≥2×, so tighter thresholds cannot trade at a loss.

### Operational

- ALICE prod restarted (PID `3834195`, version reported as 2.25.50 by PM2). Verified `/api/crypto/strategy/token-trader/status` now surfaces `minGridBuyUsd: 8` and `maxStableDeploymentPercent: 90` in its `settings` block. Verified DM logs now report `take-profit at +2.5%` (was +2.5% on ETH previously — sell side was already at this level) and `stop-loss at -3%` (unchanged). Behavior under new sizing won't be observable until a grid sell fires and resets `consecutiveGridBuys` from 12 back to 0; at that point the smart-sizing math takes over.

### Not done (deferred to next session — pickup-ready specs added)

- **Item 4a/4b** (SIDEWAYS-regime peak-anchored scale-out, plus a new `RangeTraderStrategy` class) — discussed and authorized this session but deferred. Reasoning: (a) [redacted-token]'s current price ([redacted] vs peak [redacted]) is at 89.6% of peak, so a range-anchored sell rule wouldn't fire today even with the override in place — no observable behavior change; (b) #4b is a substantial new strategy class needing careful design; (c) deploying these alongside the just-shipped smart-sizing and DM tightening would confound any performance-impact observations from #1 + #2. Better to let #1 + #2 run for a day, observe [redacted-token]'s first post-sell cycle under the new sizing, then tackle #4a/#4b informed by data.
- **Pickup-ready specs for both items added to `docs/sessions/SESSION-SUMMARY-2026-05-22.md` Part 5.** Full implementation details: new config defaults with exact key names and values, new state fields, code-block snippets ready to paste in, file paths and line-number anchors (`TokenTraderStrategy.js:80, :126, :847, :1130, :1389, :1740`), test plan, risks/gotchas, recommended sequencing (4a first as v2.25.51, then 4b Phase 1 as v2.25.52, 4b Phase 2 as v2.25.53 after observing detection accuracy on a test token). Estimated effort: 4a ~45 min, 4b ~3-5 hours split across phases.

## [2.25.49] - 2026-05-22

Opt-out gate for the v2.25.48 gateway auto-onboarding client + ALICE deployed. Required so legacy agents (ALICE, BETA) that already have a gateway agent record without a P2P fingerprint AND/OR an existing `wg0` interface don't re-register and accidentally overwrite their tunnel config in a TUNNEL-mode fallback. ALICE is now running v2.25.49 with `GATEWAY_AUTO_ONBOARD=false`; verified post-restart that the boot log emits `Gateway auto-onboarding skipped (GATEWAY_AUTO_ONBOARD=false)`, ALICE's existing `wg0` at `10.8.0.x/24` is untouched, no `data/wireguard/` keypair was generated by the client, and the gateway agent record (`agentId 2930, fingerprint:null`) was not duplicated.

### Added

- **`GATEWAY_AUTO_ONBOARD` env var** (`src/core/agent.js`) — when set to the literal string `'false'`, the `GatewayClient` initialization is skipped and a one-line `[INFO] Gateway auto-onboarding skipped` is emitted. Any other value (including unset) keeps the v2.25.48 default behavior of running the client. Documented in `.env.example` with the two opt-out conditions (legacy gateway record, pre-existing wg0).

### Operational

- **ALICE prod (`192.168.x.x`)** — `GATEWAY_AUTO_ONBOARD=false` appended to `$DEPLOY_PATH/.env` (with `.env.bak-*` backup), then `agent.js` + `gatewayClient.js` + `install.sh` + `.env.example` deployed via `deploy-files.sh`. PM2 restarted with `--update-env`. New PID `3349209` boot log confirms the gate is active. No new gateway registration POST observed.

### Future work flagged

- **BETA** (`agentId:2931`, separate Docker Compose deployment) is also a legacy record with `fingerprint:null`. When BETA gets updated to v2.25.49+, set `GATEWAY_AUTO_ONBOARD=false` in its `.env` before the deploy. Otherwise BETA will create a duplicate gateway record (and could overwrite its wg0 config if its serviceUrl probe transiently fails).

## [2.25.48] - 2026-05-22

Auto-onboarding for new agents to the api.lanagent.net gateway: a signed self-registration flow that uses each agent's existing P2P Ed25519 identity (no admin key needed) and falls back to a gateway-provisioned WireGuard tunnel when the agent isn't directly reachable. Gateway side is live in production and already serving an external agent (logs show 5-min heartbeat cadence from a TUNNEL-mode peer at 10.8.0.x). Genesis side is committed but not yet deployed to ALICE prod — see the 2026-05-22 session report for the deployment-risk analysis on ALICE's existing tunnel.

### Added

- **`src/services/gateway/gatewayClient.js` (new)** — self-onboarding service. Generates/persists a WireGuard keypair at `data/wireguard/{private,public}.key` (0600/0644), signs the canonical-JSON registration payload with the existing P2P identity (`cryptoManager.getPublicKeys()`), POSTs to `${GATEWAY_URL}/agents/register` (default `https://api.lanagent.net`), handles the gateway's DIRECT-vs-TUNNEL response, writes `/etc/wireguard/wg0.conf` idempotently when in TUNNEL mode (replaces an existing peer block for the same fingerprint rather than appending), brings up `wg-quick@wg0`, and heartbeats every 5 min to refresh `lastSeen`. Best-effort: failures are logged and retried in the background with exponential backoff (30s → 30min), never block agent startup.
- **`src/core/agent.js`** — wires the gateway client into background-services init, inside the existing `p2pEnabled` gate. Try/catch wrapped so a setup failure logs a warning and continues. Exposed via `agent.services.get('gatewayClient')`.
- **`scripts/setup/install.sh`** — installs `wireguard` + `wireguard-tools` when absent on apt-based hosts; when `RUN_AS_USER` is non-root, drops `/etc/sudoers.d/lanagent-wg` with the four exact commands `wg-quick` needs (`up wg0`, `down wg0`, `install wg0.conf`, enable/disable systemd unit). `visudo`-validated before install; falls back to a warning if validation fails so the install script doesn't break the host's sudo config.

### Added (gateway side — separate repo `PortableDiag/api-lanagent-net`, commit `d0d4342`)

- **`POST /agents/register`** — accepts an Ed25519-signed payload `{ fingerprint, name, walletAddress, publicKey, serviceUrl, agentPort, wgPublicKey, version, agentId, timestamp, nonce, sig }`. Verifies signature against `publicKey` (which must hash to `fingerprint`), enforces a ±5min timestamp window and per-nonce replay protection. Probes `serviceUrl` for DIRECT-mode reachability; on failure, allocates a free `10.8.0.X/32` slot on `wg0` (range .3–.254; .1 = gateway, .2 = ALICE legacy), appends a peer block to `/etc/wireguard/wg0.conf` for the agent's `wgPublicKey`, hot-reloads via `wg syncconf` (no tunnel restart, no peer churn), and returns the tunnel config. Idempotent on `fingerprint` — re-registration updates the existing record and replaces the peer block instead of allocating a new IP.
- **`GatewayAgent` schema gains** `fingerprint` (unique sparse index), `mode` (`'direct' | 'tunnel'`), `wgPublicKey`, `allocatedIp`. All optional / backwards-compatible with the legacy ALICE record which has these fields null.
- **Gateway-assigned agentId range starts at 10001** to stay clear of the 2000–9999 range ERC-8004 mints land in. A claimed `agentId` in the registration payload is honored if there's no clash, otherwise a new ID is auto-assigned.

## [2.25.47] - 2026-05-22

Scheduler stall recovery + health-endpoint freshness check. On 2026-05-22 a transient `MongoNetworkTimeoutError` around 04:15 UTC wedged Agenda's job processor: 35 jobs sat locked-but-never-completing for 8–10 hours, with no scheduled task firing while HTTP traffic still flowed normally. The diagnostic that did run before the wedge had one failed sub-test (`API: Health Check` self-call exceeded 10s), which pinned `overallHealth=critical` permanently because no subsequent diagnostic ever ran to replace it — and `/api/health` faithfully reported "unhealthy" for ~10 hours, propagating to the api-gateway's agent-refresh loop.

### Fixed

- **Scheduler stuck-job watchdog (`src/services/scheduler.js`)** — added `agenda.on('error', ...)` handler so MongoDB connection errors during job-pool operations surface as logged errors instead of `unhandledRejection`, and a 5-min interval that scans `scheduled_jobs` for entries where `lockedAt < now - 20min` (2× default lockLifetime) and force-unlocks them. Recovers from the wedged state automatically without requiring a PM2 restart. Skips when `mongoose.connection.readyState !== 1` to avoid fighting an ongoing reconnect.
- **`/api/health` stale-report degradation (`src/interfaces/web/webInterface.js`)** — the handler now compares `latest.timestamp` against `selfDiagnosticsService.config.autoRunInterval * 2` (default 12h). When the report is older than that threshold, `health` returns `'stale'` and `status` returns `'ok'` instead of letting an outdated critical reading pin the endpoint to "unhealthy" indefinitely. Adds a `reportAgeSeconds` field to the response so external callers (the api-gateway, the postman collection) can see report freshness directly.

## [2.25.46] - 2026-05-15

DollarMaximizer stablecoin position tracker now treats the on-chain wallet as the single source of truth — eliminating an inter-strategy drift bug where state could lag wallet by hundreds of dollars between reconciliations.

### Fixed

- **`CryptoStrategyAgent.executeDollarMaximizer` stablecoin reconciliation**. The "stablecoin mismatch" branch in the reconciliation loop only synced `position.stablecoinAmount` when drift exceeded 10% of wallet value (`Math.abs(actualStable - position.stablecoinAmount) / Math.max(actualStable, 1) > 0.1`). Below that threshold, state stayed stale while TokenTrader spent from the shared on-chain stablecoin pool. Over 14 hours of trading on 2026-05-15, state drifted from the on-chain balance — the gap surfaced only when the next reconciliation finally tripped the threshold, generating a misleading "discovering wallet balance" Telegram notification.

  New behavior: when `position.inStablecoin && actualStable > 0.01`, always assign `stablecoinAmount = actualStable` on every DM tick (no threshold). In-memory sync is unconditional; DB persist still gates on drift > $0.50 to avoid hammering Mongo every 10 minutes when the value hasn't meaningfully changed. The "stablecoin mismatch" INFO log is demoted to a DEBUG "position sync" line because it's no longer an anomaly — it's the normal flow. No new wallet RPC calls: uses the `marketData.balances[network].stablecoin` value the agent already fetches once per tick.

  The reconciliation branch for direction changes (Branches 1, 2, 4: flip-to-stable, reset-to-native, flip-to-native) is untouched. Only the "amount drift" branch (3) was the leaky abstraction. `_refreshDMPositionAfterTrade` (the post-TokenTrader-trade refresh hook at `CryptoStrategyAgent.js:3623`) is retained for keeping the DB current between DM ticks for UI display, but is no longer load-bearing — the next DM tick re-syncs from wallet regardless.

  Verified in production after deploy at 12:35 PDT: TokenTrader BUY at 12:55:20 consumed stablecoin → next DM tick at 12:57:03 evaluated cleanly with no "stablecoin mismatch" warning. Old code would have logged the warning whenever drift crossed 10%.

## [2.25.45] - 2026-05-13

ALICE PR triage round (#2158-#2164, 7 PRs from the auto-improve loop). All 7 closed; 5 manually re-implemented with corrections, 2 partially-applied with the unsafe/dead pieces dropped.
...
- **`BaseStrategy.calculateAverageTradeDuration()` / `calculateMaximumDrawdown()` / `calculateSharpeRatio()`** (closed-and-reimplemented from #2158 — PR's `StrategyRegistry.generatePerformanceReport()` called these methods but they didn't exist on any strategy, guaranteed `TypeError` on first invocation). Implemented properly on BaseStrategy so all subclasses inherit: trade-duration pairs each closing trade (sell/exit/close) with the most recent prior opening trade (buy/enter/open) on the same network and averages durations; max-drawdown walks cumulative pnl through `state.performanceHistory` tracking peak-to-trough; Sharpe annualizes daily-aggregated pnl with crypto's 365-day calendar and rf=0. Each returns sensible nulls/zeros for insufficient-data cases instead of throwing.
- **`StrategyRegistry.generatePerformanceReport({ log })`** — calls each strategy's calculation methods through a `safeCall` wrapper so a custom strategy that doesn't extend BaseStrategy still appears in the report (with nulls) rather than throwing. Returns enabled flag, tradesExecuted, totalPnL, plus the three calc'd metrics. Genesis-only file; will not sync to public.
...
- **Blockchain RPC retry tuning in `payment.js`** (closed-and-partial-applied from #2159 — bumped `getTransaction` and `getTransactionReceipt` retries from 2 → 5 with `minTimeout: 1000, maxTimeout: 5000`. `retryOperation` already does exponential backoff so no util refactor was needed despite what the PR description claimed). Worst-case extra latency for a permanently-failing tx becomes ~17s with default factor=2 — fine for blockchain confirmations that already span minutes. Skipped the rest of #2159 — module-scope `apiLimiter = rateLimit(...)` declaration that was never wired into any route handler. Pure dead code; if we want to rate-limit credit-debiting endpoints that belongs on the gateway side (already done via `portal.mjs` `authLimiter`) not buried in agent middleware where it would never execute.

### Notes

- `BasePlugin`, `_ai_template.js`, `payment.js`, `imageGen.js`, `oauthmanager.js`, `xueqiu.js`, `googlesecops.js` are all public-safe and will sync.
- `BaseStrategy.js` and `StrategyRegistry.js` are genesis-only per the standing policy on crypto strategy code (memory: `crypto_strategy_registry_genesis_only.md`). The trade-metrics methods stay genesis-only too.
- All 7 PRs have per-PR closure comments explaining what was kept, what was changed, and why.

## [2.25.44] - 2026-05-13

Production log hygiene pass — five fixes in response to a log audit that surfaced one real outage path (notify TypeError) plus four sources of error-log noise.

### Fixed

- **`message.substring is not a function` TypeError in `agent.notify`.** `cryptoMonitor.js:362` and `:380` were calling `agent.notify({title, message, priority})` while every other caller in the codebase passes a string. `agent.notify(message, userId)` then did `message.substring(0,100)` and crashed. Fixed both call sites to pass formatted strings (`💰 Balance changed on …`, `🚨 Crypto Alert: …`) AND hardened `agent.notify` to coerce object/non-string inputs (extracts `title`/`message`/`text` fields) so a future caller passing an object can't break notifications again. Recurring since at least 2026-05-02.
...
- Pre-fix prod (v2.25.41) was generating ≈100 spurious `[ERROR]` lines per restart from the plugin-init path alone; this patch should eliminate that and leave only real failures in errors.log.

## [2.25.43] - 2026-05-12

API-mode account registration + reapption.net strategy. End-to-end test of the auto-register/storage/usage system.

### Added

- **API-mode registration strategy** in `accountRegistrationService.js`. Strategies now support `mode: 'api'` alongside the existing browser path. API-mode skips Puppeteer entirely — talks JSON directly to the site's signup endpoint, which is far more reliable for SPAs (selector matching against React-rendered DOM is brittle, especially when forms live inside `<details>` panels or share input id namespaces with multiple forms on the same page). Strategy fields: `challengeUrl`, `registerUrl`, optional `loginUrl`, and a `buildBody(creds)` mapper to translate generic credentials into the API's expected body shape.
- **`reapption` strategy** wired up. Calls `/api/auth/challenge` (handles both the active LANAgent challenge-question gate and the `{disabled:true}` off-state), then `POST /api/auth/register`. When the gate is on, semantic challenge questions are answered by the agent's own AI via `providerManager.generateResponse` (low temperature, 1500-token budget for reasoning headroom). Up to 3 retries on `challenge_failed` / `challenge_expired`, matching the reapption frontend's behavior. Server response (user id, auth token) is merged into the encrypted credential blob under `_server` so downstream usage doesn't need a re-login.
- **`getGlobalAgent()` use in non-plugin services.** accountRegistrationService is a singleton, not a plugin, so it can't get `this.agent` via injection. Used the existing `src/core/agentAccessor.js` accessor to reach `providerManager` for challenge-answering.

### Verified

- ALICE registered `alice@lanagent.net` at reapption.net via `POST /api/accounts/register {serviceName:"reapption",serviceUrl:"https://reapption.net",isPrimary:true}`. Account ID `6a03a8127cb7ded6a0003a4d`, status `active`, marked primary.
- Decrypted stored credentials and tested `POST https://reapption.net/api/auth/login` — HTTP 200, JWT issued, server-side user_id `f8d30e22-71c8-4e44-85b8-91259c05cca7`.

## [2.25.42] - 2026-05-12

Social-media auto-post unwedge + hardening. Twitter and MindSwarm both stopped posting around 2026-05-07 (mindswarm) / 2026-05-09 (twitter) — same failure mode, double cause.
...
- **Empty completions from GPT-5 with `maxTokens: 120`.** The OpenAI provider correctly routes to `max_completion_tokens` for GPT-5/o-series, but that budget includes reasoning tokens — a 120-token cap leaves zero for visible output. Worked silently on gpt-4o; broke when v2.25.38's ModelCache started routing chat traffic to `gpt-5-2025-08-07`. Bumped MindSwarm to 1500, Twitter to 3000 (Twitter's stricter 270-char hard limit pushes longer reasoning chains).
...
- **Session reports never leak into auto-posts.** `EXCLUDED_GIT_PATHS` already filtered `docs/sessions/` for pure-session commits, but a commit touching both `src/` and `docs/sessions/` slipped through the pathspec. Extended `SENSITIVE_COMMIT_PATTERNS` in `src/utils/autoPostFilter.js` with `session.*\d{4}-\d{2}-\d{2}|docs\(session` (defense in depth) and added an explicit `SENSITIVE_OUTPUT_RULES` entry telling the AI never to mention dev sessions, session reports, or operator workflow.

### Operational

- Deploy-files.sh silently no-op'd on some pushes during this session (md5 mismatch between local and prod despite "Deployed N files" success message). Worked around with direct `scp`. Worth investigating separately — possible rsync-exclude or stale-build behavior in the script.

## [2.25.41] - 2026-05-11

Self-modification routing for GitHub-discovered features. Every self-mod tick today (76/76) failed on the same chain: plugin/core analysis correctly found nothing actionable (yesterday's PR triage drained that queue), self-mod fell through to `getStoredGitHubFeatures()`, and all 286 implementable records had `implementation.targetFile = null` (hard-coded at `scheduler.js:1735` with a "Will be determined later" comment that never happened). The AI fallback inside `applyAICapabilityUpgrade` silently missed because its `/\{[^}]+\}/` regex didn't match the AI's responses — no log markers, just throws.

### Added

- **`implementation.kind` on `DiscoveredFeature`** — enum `'modify' | 'new-plugin' | 'skip'`, set by the classifier at discovery time. `findImplementable` now filters `kind ∈ {modify, new-plugin}` AND `targetFile` non-empty, so unclassified or skipped records never reach self-mod.
- **`src/services/featureClassifier.js`** — `classifyDiscoveredFeature(agent, feature)` asks the AI to pick `modify` (with existing file path), `new-plugin` (with fresh `src/api/plugins/<name>.js` path), or `skip`. Validates every path: `fs.access` for modify, parent-dir exists + filename collision check + loader-exclude-pattern check for new-plugin. Robust JSON extraction (handles ```json fences). Never throws — returns `kind: 'skip'` with rationale on any failure.
- **`applyNewPluginFromFeature`** in `selfModification.js` — parallel apply-time path for new-plugin. Generates a complete BasePlugin subclass with `anime.js` + `nasa.js` as few-shot structural references, validates with `node --check` (rollback on syntax failure), writes, and stages a commit. Plugins are auto-discovered from `src/api/plugins/` on next restart — no registry edit needed.
- **`backfill-discovered-feature-kinds` Agenda job** — runs every 30 min, classifies 10 unclassified records per tick, logs queue depth. No-op once empty. Drains the legacy 286-record backlog without burning AI quota in one shot.

### Fixed

- **Dirty discovery titles** in `githubFeatureDiscovery.js`. `isGenericFeature` now rejects malformed bullet-list extractions: JSON-looking fragments (starts with `{`/`[`, has adjacent brace pairs, has `"key":"value"` shape), single-letter + JSON prefixes (e.g. `D {"file":...}`), and over-long lines (>200 chars). One of today's 18 failed titles was literally `D {"file":"logs/diagnostics.log","severity":"high"}` — leaked structured data from an AI-edited README in an upstream project.
- **github-discovery classification at write time** in `scheduler.js`. After `DiscoveredFeature.create`, the job now calls the classifier inline and updates the record with kind/targetFile/rationale. `kind: 'skip'` flips `status: 'rejected'` so it never pollutes `findImplementable`.

### Decided against

- **Delegating new-plugin to `PluginDevelopmentService`** — that service is built around web-discovering APIs and would need a meaningful adapter to consume pre-discovered features. Parallel dedicated path is more code but more reliable, per user direction.
- **`new-service` / `new-utility` kinds.** Services require editing `src/core/agent.js` (multi-file risk); utilities are rare. Anything that genuinely needs a new service gets `kind: 'skip'` with a rationale and surfaces for manual review.

## [2.25.40] - 2026-05-11

ALICE PR triage round (#2138–#2157, 20 PRs from the auto-improve loop). 4 merged, 1 salvaged manually, 15 closed with rationale. Notes from the round worth keeping:

### Added

- **`GET /api/external/social/supported-sites`** (salvaged from #2147). Returns the list of social extractors the agent currently has loaded (`twitter`, `ytdlp`) with their capabilities and the Phase-1-verified host list. Lets the SKYNET API bot probe coverage without trial-and-error against `/probe`. Implemented directly rather than merging the PR because the PR also stripped the route file's header docblock — that comment documents the Phase 1 / 2 / 3 tier matrix and is load-bearing context.

### Fixed

- **Header docblocks restored** in `src/utils/flareSolverr.js` and `src/utils/ytdlpCookieJar.js`. The merges of #2151 and #2152 dropped the file headers as a side-effect — those headers carry the prod FlareSolverr `docker run` invocation and the cookie-jar storage-path rationale (kept outside the deploy dir so cookies survive deploys), neither of which is reconstructable from the surrounding code.
- **Relative import paths** in `ytdlpCookieJar.js`: `../utils/retryUtils.js` → `./retryUtils.js`, `../utils/logger.js` → `./logger.js`. The file lives in `src/utils/`; the `../utils/` prefix accidentally works but reads as cross-module instead of same-directory.

### Merged from PR loop

- **#2151 `ytdlpCookieJar.js`** — retry-wrap on `ensureCookiesDir()` mkdir + log on failure. mkdir can fail on transient EAGAIN / read-only-during-snapshot conditions.
- **#2152 `flareSolverr.js`** — pass caller-provided headers through to the FS request, falling back to the existing Content-Type default. Lets callers forward User-Agent / Cookie for sites that need them.
- **#2154 `MqttDevice.js`** — observability hook (`notifyStatusChange`) on `addStatusHistory` and `transitionLifecycleState`. Currently just logs structured info; placeholder for future notification integration. Fire-and-forget via `withErrorHandler`.
- **#2155 `SSHConnection.js`** — defensive guard in `startSession()` against double-starting a session while one is still active (`endTime: null`).

### Closed with rationale

- **#2138, #2141** — both passed options that aren't part of `retryOperation`'s API (`retryCondition`, `randomize`). Unknown options are silently ignored; `isRetryableError` is already the default classifier. Net no-op.
- **#2139** — `getActivePluginsStatus()` + handler added to `MCPServerService`, but no route mounts the handler and no MCP tool registers the inner method. Dead code.
- **#2140** — sorts MCP servers by `priority`, but the `MCPServer` schema has no `priority` field; `b.priority - a.priority` is `NaN` for every pair. Would need schema + admin-UI changes.
- **#2142** — change-stream watcher set up at module-import time, before mongoose is connected; also dereferences `change.fullDocument` without `{ fullDocument: 'updateLookup' }`. Existing mongoose `post('findOneAndUpdate')` invalidation already covers this for the cache that needs it.
- **#2143** — vendored third-party WebXR profiles library; patching local copies of upstream libs creates divergence pain.
- **#2144** — opens the auto-post sensitive-content filter to runtime override via a JSON file. The filter is deliberately hardcoded constants because an override path is exactly the attack surface that defeats the purpose.
- **#2145** — `DailyPnL.findWithRetry` caches by `JSON.stringify(query)` for 10 min, but `updateWithRetry` doesn't invalidate. Realized P&L moves continuously; stale reads aren't acceptable here.
- **#2146** — replaces the exported `GeneratedSong` mongoose model with a wrapper class instance. Callers (`src/api/plugins/music.js`) call `GeneratedSong.create() / .find() / .findOne()` directly and would all break.
- **#2148** — wraps `await import('./scammerRegistryService.js')` in NodeCache; ES modules are already cached by Node's loader after first import. Pure overhead.
- **#2149** — calls `TaskScheduler.scheduleJob(name, date, callback)`, which doesn't exist. Our scheduler uses Agenda (`agenda.every` / `agenda.schedule`) with pre-defined named handlers; inline callbacks aren't supported. Implementing scheduled SMS/MMS properly is a larger change.
- **#2150** — wrapping the Ollama `/api/chat` and `/api/embeddings` calls in `retryOperation` composes badly with `responseType: 'stream'` (mid-stream failures would restart and duplicate output) and with the 10-min `this.timeout` (3 retries could turn a single timeout into ~35 min hang). The `availableModels` cache wrap is also pointless since `initialize()` only runs once per process.
- **#2153** — defines a `getMetricsTrends` handler and then instantiates a brand-new `express()` app at module load to mount it on, exporting that app via default-export. The new app is never wired into the main server.
- **#2156** — `new Function('value', \`return value ${rule.condition};\`)` with a DB-sourced `rule.condition` is an arbitrary-code-execution path. Anyone with write access to `UpsConfig.customAlertRules` can run JS in the agent process.
- **#2157** — converts `encrypt` / `decrypt` from sync to async; both are imported synchronously across 30+ files (every crypto service, half the plugins, several mongoose pre-save hooks). Without a coordinated repo-wide migration, sync callers would `JSON.stringify(Promise)` into the DB.

### Files changed

- `src/api/external/routes/social.js` (new `/supported-sites` endpoint)
- `src/utils/flareSolverr.js` (header docblock restored)
- `src/utils/ytdlpCookieJar.js` (header docblock restored, relative-import cleanup)
- `package.json` (version bump)

## [2.25.39] - 2026-05-11

Order-of-init bug: plugins register Agenda handlers in their `initialize()`, but `apiManager.initialize()` (which loads plugins) runs at agent bootstrap line 127, while `scheduler.initialize()` runs at line 249. So every `agent.scheduler?.agenda.define/every` call in a plugin's init phase silently bailed via the `if (!this.agent.scheduler) return` guard — handlers never registered, recurring rows orphaned in mongo. Surfaced by today's Agenda audit: `update-pr-review-models` had `nextRunAt` 125 days in the past (Jan 6 row sitting with no handler), `review-prs` was missing entirely, and the three `system-admin-*` jobs were also missing.

### Added

- **Framework hook in `agent.js`**: after `scheduler.initialize(this)` completes, iterates every plugin in `apiManager.apis` and calls `plugin.defineSchedulerJobs()` if the method exists. Plugins opt in by implementing that method; existing init guards stay intact for plugins that don't.

### Fixed

- **`prReviewer.js`**: moved Agenda handler registration out of `initialize()` into a new `defineSchedulerJobs()` method (framework hook now calls it AFTER agenda is ready). Handlers are registered unconditionally and re-read settings each tick — operator can toggle the `enabled` flag via UI without a restart, and the schedule rows stay current. Pre-existing rows are cancelled at register time so stale repeatInterval / orphaned nextRunAt from earlier codebases don't survive. After this: `review-prs` and `update-pr-review-models` are both scheduled with valid `nextRunAt` values on every restart.
- **`systemAdmin.js`**: same pattern — `defineSchedulerJobs()` delegates to the existing `scheduleMaintenanceTasks()` which still respects `config.autoUpdatesEnabled`. With autoUpdates on, the three `system-admin-*` jobs (updates / zombie-cleanup / log-cleanup) now actually appear in the recurring schedule instead of silently no-op'ing.
- **`sinch.js`** (incidental): the plugin already named its method `defineSchedulerJobs()`, so the new framework hook picks it up automatically — its `sinch-scheduled-sms` / `sinch-scheduled-call` handlers are now actually registered post-restart instead of vanishing into the order-of-init crack.
- **`system-status-report`** wasn't actually broken — its handler was defined unconditionally in `scheduler.js:1559`, the row had a valid future `nextRunAt`, the `lastRunAt: null` was just because the settings UI had rescheduled the cron and the new schedule hadn't fired yet. Verified end-to-end by force-triggering: report generated, saved to DB, notification sent in 4s, row's `lastRunAt`/`lastFinishedAt` populated.

### Operations

- Deleted the orphaned `update-pr-review-models` row (`_id: 695c81466700b071761c5ede`, `nextRunAt: 2026-01-06`) on ALICE so the fresh `every()` call could create a clean row. Verified after redeploy: new row at `2026-05-11T09:00:00.000Z`, no `failedAt`, post-trigger `lastRunAt`/`lastFinishedAt` confirm handler runs.

### Files changed

- `src/core/agent.js` (post-scheduler-init plugin hook)
- `src/api/plugins/prReviewer.js` (move scheduling into `defineSchedulerJobs`, always-on handler + per-tick settings re-read)
- `src/api/plugins/systemAdmin.js` (add `defineSchedulerJobs` delegating to existing scheduling code)
- `package.json` (version bump)

## [2.25.38] - 2026-05-10

Dashboard AI Providers page was stuck showing the gpt-4o / claude-3.5 era despite the daily `model-update` Agenda job running cleanly. Tracked it down to two combined problems: provider `availableModels` is populated only when the scheduled job fires and lives only in memory (so every restart drops the dashboard back to `[]` until the next 3 AM run), and Anthropic's `getAvailableModels()` was hardcoded to a fixed 5-model list that ignored `this.availableModels` entirely. End-to-end after this release: dashboard shows `gpt-5.5` for OpenAI and `claude-opus-4-7` / `claude-sonnet-4-6` for Anthropic immediately after restart.

### Fixed

- **Provider model cache lost on every restart.** `availableModels` lived only in the in-memory provider instance — `ModelUpdaterService` writes the fresh list to `ModelCache` (Mongo) on every run, but nothing read that cache back into the live providers at startup. So after any deploy/restart, `getAvailableModels()` returned `[]` until the next scheduled 3 AM job. Fix: `providerManager.syncModelsWithDatabase()` now pulls every `ModelCache` row and calls `provider.updateAvailableModels(cache.models)` per provider. Runs at startup + every post-deploy reload, so the dashboard has fresh data immediately. Verified on ALICE: `Loaded 129 cached models for openai`, `Loaded 9 cached models for anthropic` in the boot log.
- **Anthropic `getAvailableModels()` was hardcoded.** Comment claimed "this list is refreshed by the model-update scheduler job (daily 3 AM)" but the code returned a static 5-model array regardless of what the job pushed via `updateAvailableModels()`. Refreshed data went into `this.availableModels` and stayed there, unread. Fix: prefer `this.availableModels` when populated, fall back to the hardcoded list (mirrors the existing HuggingFace pattern). After this, the dashboard shows the dynamically refreshed Anthropic list including `claude-opus-4-7` and `claude-sonnet-4-6` instead of stopping at `claude-opus-4-5`.
- **Scheduler's retired-model fallback default for OpenAI was itself a year stale.** `defaultModels.openai = 'gpt-4o'` — this is the value used to reset agent configs that point to a retired model. Bumped to `'gpt-5'` so the fallback isn't itself the next thing to retire. Anthropic default (`claude-opus-4-5-20251101`) left as-is; it's current.

### Operations

- Audited Agenda state on ALICE. 39 jobs registered, all running on cadence, zero failures. Two outliers worth flagging for a future pass (not part of this release): `update-pr-review-models` has `nextRunAt` 125 days in the past — orphaned, not picking up. `system-status-report` is defined but has never fired (`lastRunAt: null`).

### Files changed

- `src/core/providerManager.js` (load ModelCache at startup, trim verbose sync log)
- `src/providers/anthropic.js` (honor `this.availableModels` first, hardcoded list as fallback)
- `src/services/scheduler.js` (fallback default openai → gpt-5)
- `package.json` (version bump)

## [2.25.37] - 2026-05-10

End-to-end advertisement creation on MindSwarm. The plugin already had `submitAd` / `getAdSettings` / `payForAd` etc. wired to the MindSwarm REST API, but: (a) `submitAd` didn't enforce `imageUrl` (server requires it, returned a generic 400); (b) `payForAd` insisted on all six crypto fields even when MindSwarm is in `ad_free_mode` (server accepts an empty body there); (c) no orchestration method tied submit → settings-check → pay together, so creating an ad meant six manual steps that the agent couldn't reliably chain.

### Added

- **`createAd` command** on the `mindswarm` plugin. Single call: resolves an image (`imageUrl` | `imageFilePath` | `imagePrompt` for AI generation via `imageGenerationService`), uploads to MindSwarm if needed, submits the ad, reads `/ads/settings`, and either (free mode) calls `/pay` with an empty body or (paid mode) sends native crypto via `transactionService.sendNative` and submits the txHash for on-chain verification. Returns a result with `stage` (`pending_review` / `active` / `payment_submitted` / etc.), `imageUrl`, `txHash` + `amount` when paid.
...
- `src/api/plugins/mindswarm.js` (`_submitAd` imageUrl required, `_payForAd` free-mode body, `_createAd` orchestration, `createAd` dispatch + commands[] entry)
- `package.json` (version bump)
...
- Restarted BETA + BETA-mongodb containers; agent re-indexed 1162 intents and the `/api/health` endpoint became reachable externally.
...
Operational visibility + a long-standing dual-record agent bug. Three bugs found while triaging a "wallet purchased credits" Telegram notification that didn't show up in the gateway admin panel — turned out the wallet wasn't *on* the gateway (it lived on ALICE), the gateway admin had no cross-agent view, and the auto-replenish payment record was missing the fields needed to audit it. While in there, found and fixed the responseSanitizer flattening every Date in every external response to `{}` and a missing-blue-bars CSS bug on the gateway admin Scrapes page. Closed by tracking down a multi-month dual-record bug where `loadAgentModel` was hardcoded to query `name:"LANAgent"` while every other code path used `process.env.AGENT_NAME` — production had two `agents` documents (ALICE + a LANAgent ghost) being written by different code paths, with ~14 fields divergent including `aiProviders.configurations`, `mediaGeneration.image.provider`, and `erc8004.status`.

### Added

- **`GET /api/external/admin/wallets`** — cross-agent operator endpoint returning all `ExternalCreditBalance` rows + summary totals on this agent. Auth via `X-Admin-Key` header matched against the new `AGENT_ADMIN_KEY` env var. Lets the gateway admin panel fan out and surface wallets that don't live in the gateway's own DB (per-agent customer wallets and the gateway-as-a-customer wallet that receives auto-replenishment SKYNET).
- **`GET /api/external/admin/payments/recent?limit=20`** — recent `ExternalPayment` rows with `currency` / `creditsIssued` / `usdValue` surfaced. Same admin-key auth.
- **`AGENT_ADMIN_KEY` env var** (instance config). Required for the new admin endpoints; absent → 503. Generated 32 bytes hex; keep matched on every cooperating agent + the gateway that calls them.
- **`ExternalPayment` schema fields**: `currency`, `creditsIssued`, `bonusCredits` (default 0), `promotion` (default null), `usdValue`. All optional / backwards-compatible. Populated for `serviceId === 'credit-purchase'` records so the row alone tells the full story without joining to the balance.

### Fixed

- **`loadAgentModel` was hardcoded to `name:"LANAgent"` regardless of `AGENT_NAME`** (`src/core/agent.js:295-329`). Comment was self-incriminating: `// Always try to load the LANAgent record first, regardless of AGENT_NAME`. Result on multi-instance installs: two `agents` documents per agent — one by the instance name (ALICE), one by the framework default ("LANAgent") — both written continually by different code paths. Production drift between the two records: `aiProviders.configurations` (ALICE had gab + huggingface configs the ghost didn't), `mediaGeneration.image.provider` (`huggingface` vs `openai`), `mediaGeneration.video.provider` (`huggingface` vs `modelslab`), `erc8004.status` (`active` with agentId 2930 vs `none`), `serviceConfigs.*.lastCheckTime` (real timestamps vs `null`), `serviceConfigs.selfHealing.enabled` (true vs false), `serviceConfigs.selfModification.maxDailyImprovements` (4 vs 2), and several others. Anything reading `this.agentModel` was getting the stale ghost while the web UI / providerManager wrote to the correct record. Fix: `loadAgentModel` now uses `process.env.AGENT_NAME || "LANAgent"` like the rest of the codebase. Verified post-deploy that the freshly-restarted ALICE writes only to the correct document; LANAgent ghost record was deleted out-of-band after verification (its only "newer" value was `state.lastHeartbeat`, which is now written to ALICE on every heartbeat). Backups of both pre-merge documents preserved at `<local-scratch-path>…` on the dev box.
- **`responseSanitizer` was flattening every Date in every external response to `{}`** (`src/api/external/middleware/responseSanitizer.js`). The middleware walks the response body and rebuilds objects via `Object.entries(val)`. Date instances pass `typeof val === 'object'` but `Object.entries(date) === []`, so every Date got rebuilt as an empty object — `lastPurchase`, `lastUsed`, `createdAt`, etc. all came through as `{}` to consumers. Fix: pass `Date` and `Buffer` instances through unchanged before the generic object branch. This was a wide-scope bug affecting every external response that contained a date, not just the new admin endpoints.
- **`ExternalPayment` records for credit purchases were missing audit fields.** `credits.js` was creating the doc with `txHash`, `chain`, `serviceId='credit-purchase'`, `callerAgentId`, `amount` (raw crypto amount as string), `recipientAddress`, `blockNumber`, `confirmations`, `verifiedAt`, `consumed`, `consumedAt` — but not the currency, the credits issued, or the USD value. Replenishment records in production showed `wallet=undefined, currency=undefined, credits=undefined` to anyone querying by those names (the wallet was actually in `callerAgentId`, the currency / credits were never written at all). Fix: `credits.js` now passes `currency`, `creditsIssued`, `usdValue` on every credit-purchase record. The wallet stays in `callerAgentId` (the existing field, unchanged) — backwards-compatible.

### Operations

- Bumped `AGENT_ADMIN_KEY` into ALICE's `.env` (32-byte hex). Env var is read directly from `.env` by the running PM2 process; verified via `pm2 env 0`.
- Deleted the LANAgent ghost record from `agents` collection (`{name:"LANAgent", _id: ObjectId("69dae8e5181a4c3d24a4fdac")}`, `deletedCount: 1`). ALICE record retained its original `_id: 6956c9817155553ad16546e6`. Verified post-restart that no new ghost recreated. Crypto wallet / ERC-8004 registration data untouched (lives on ALICE record + separate `cryptowallets` collection).

### Files changed

- `src/core/agent.js` (loadAgentModel honors AGENT_NAME)
- `src/api/external/routes/admin.js` (NEW — admin-key-auth endpoints)
- `src/api/external/externalGateway.js` (mounts `/admin` routes)
- `src/api/external/routes/credits.js` (populates new ExternalPayment fields)
- `src/models/ExternalPayment.js` (currency / creditsIssued / bonusCredits / promotion / usdValue)
- `src/api/external/middleware/responseSanitizer.js` (Date / Buffer pass-through)
- `package.json` — 2.25.34 → 2.25.35
- `CHANGELOG.md` — this entry
- `docs/feature-progress.json` — new `externalAdminEndpoints_2026_05_09` entry
- `docs/api/API_README.md` — admin endpoint section
- `docs/api/LANAgent_API_Collection.postman_collection.json` — Admin folder + version bump
- `docs/sessions/SESSION-SUMMARY-2026-05-09.md` — session report

### Companion gateway repo (api-lanagent-net)

Independent repo, separate commits (`9ae04d5`, `778667e`):

- `admin.mjs` — new `GET /admin/api/agent-wallets` fan-out endpoint hitting each enabled agent's `/api/external/admin/wallets` in parallel with the shared admin key (5s timeout each, graceful per-agent failure). New "Per-agent wallets" section on the `/admin/wallets` page rendering each agent's table. Added `Refunded` column to the existing wallets table — the data was already in `CreditBalance.totalRefunded` but never displayed, so wallets where lifetime gross-spent exceeded gross-purchased looked like accounting drift when in fact the offset was tracked correctly all along (e.g. `0xce8efb55…`: balance 238 = totalPurchased 1794 − totalSpent 1966 + totalRefunded 410). Fixed missing blue bars on `/admin/scrapes` Top services / Top agents — `.bar-fill` was a `<span>` with no `display:block`, so the inline `width:X%` was ignored and the bar never showed.
- `ecosystem.config.cjs` — `AGENT_ADMIN_KEY` env var matched to ALICE's value.
...
- **#2129 `MoonIndicators.getMoonPhaseForDate(date)`** — trivial wrapper around the existing `calculateMoonPhase(date)`. Exposes a public-named method for callers that want the phase for an arbitrary date (not just "now"). No behavior change.
- **#2131 `PeerManager.getPeerAnalytics()`** — read-only analytics aggregation across `P2PPeer` documents. Returns `{averageSessionDurationTrend (by date), peakConnectionTimes (by hour), trustLevelDistribution}`. No new deps, no schema changes.
- **#2133 `donationService.generateDonationQR()` color customization** — adds `colorDark`, `colorLight`, `errorCorrectionLevel` options (defaults match the previous hardcoded `#000000` / `#FFFFFF` / `'M'`). Cache key correctly extended so calls with different colors don't collide.

### Closed and re-implemented (manual salvage)

- **#2127 `validation.js` async/try-catch** — closed because making `validateInput` async cascades through every plugin's `execute()` method via `basePlugin.validateParams` (line 317 calls `validateInput` synchronously). No existing schema uses async custom validators (or they'd already be silently broken), so the async change has no real motivation. **Salvaged**: the try/catch around `rules.validate(value)` so a throwing custom validator now produces a normal validation error message ("Validation error for field 'X': msg") instead of bubbling out as a 500. Function stays sync.
- **#2137 `GitHostingProvider` batch ops** — closed because `Promise.all` aborts the entire batch on a single failure, but the function signature `Promise<Array<{success: boolean, error?: string}>>` implies per-item results. **Salvaged**: re-implemented `batchCreateMergeRequests`, `batchMergeMergeRequests`, `batchCloseMergeRequests`, `batchCreateIssues`, `batchCloseIssues` with `Promise.allSettled` so each item gets its own `{success, ...}` (or `{success: false, error}`) entry. Caller can correlate failures by the echoed-back `mrNumber` / `issueNumber`.

### Closed without salvage

- **#2128 `ytdlpCookieJar.js`** — adds `readCookieFile`/`writeCookieFile`/`cacheCookie`/`getCachedCookie` helpers and a NodeCache cookie cache, but the module's role is just to provide path constants and a directory-ensure helper. The actual reading/writing of cookie content is done by the yt-dlp subprocess (`--cookies <path>`), not by Node code. No caller for the new helpers, no Node-side cookie content for `cookieCache` to hold. Retry-wrap on `ensureCookiesDir` adds overhead without benefit (`fs.mkdir({recursive: true})` is idempotent and doesn't throw on EEXIST).
- **#2130 `payment.js` rate-limiter + array return** — would break every payment-protected route. Four of six call sites invoke the middleware factory directly: `paymentMiddleware(serviceId)(req, res, next)` (in `hybridAuth.js`, `youtube.js`, `scraping.js`, `social.js`). The PR changes the return value from a function to an array, so the direct invocation pattern would TypeError. Separately: the gateway already has a `globalLimiter` at `externalGateway.js:34/41` covering all external traffic, and payments have inherent per-tx idempotency via `usedTxHashes` cache + DB duplicate-tx checks. Adding a per-IP rate-limiter could cut off legitimate users behind shared NAT. The `backoff: true` retryOperation option is also a no-op (exponential backoff is already on by default, factor=2).
- **#2132 `pluginUIManager.js`** — wrong logger (imports Node winston into a browser file that uses `fetch`/`DOMParser`/`document`/`window`); wrong retry semantics (wraps DOM mutations, would re-inject styles and re-dispatch events on retry); and the file doesn't appear to be loaded anywhere — no HTML `<script>` references it and no other module imports it. Even with the bugs fixed, the salvage would add retry to dead code.
- **#2134 `outputSchemas.js`** — same defect as previously-closed #2113 and #2124. Adds inert `version: '1.0'` field to every schema but doesn't add version-aware logic to `adjustSchema` or `validateData`. `version` isn't a JSON Schema keyword.
- **#2136 `auditLog.js`** — `applyRateLimiting` targets `/api/audit-logs`, a route that doesn't exist anywhere in the codebase. `applyRateLimiting`/`applyCompression` are exported but never wired up. Compression at the audit-log layer is wrong — `compression` middleware compresses HTTP responses, but `auditLogMiddleware` writes to MongoDB. The gateway already has `globalLimiter` covering external traffic.

### Files changed

- `src/utils/validation.js` (#2127 salvage — sync try/catch around `rules.validate`)
- `src/services/gitHosting/GitHostingProvider.js` (#2137 salvage — five `batch*` methods using `Promise.allSettled`)
- `src/services/crypto/indicators/MoonIndicators.js` (#2129 merged)
- `src/services/p2p/peerManager.js` (#2131 merged)
- `src/services/crypto/donationService.js` (#2133 merged)
- `package.json` — 2.25.33 → 2.25.34
- `CHANGELOG.md` — this entry
- `docs/feature-progress.json` — new `alicePRTriage_2026_05_08_round2` entry

## [2.25.33] - 2026-05-08

Crypto strategy hardening pass — three fixes prompted by a routine status audit. [redacted-token] trader was profitable but logging four `INSUFFICIENT_FUNDS` swap rejections on grid-buy attempts; the API was reporting a misleading trailing-stop value; and the LP MM had been silently collecting fees with no accounting update for 70 days.

### Fixed

- **TokenTrader: `INSUFFICIENT_FUNDS` swap failures on native-paired buys.** Two bugs in `executeTokenTrader` (`src/services/subagents/CryptoStrategyAgent.js`):
  - The "use native if stablecoin insufficient" branch read `marketData.prices?.[network]?.nativeBalance`. That field doesn't exist; the native balance lives at `marketData.balances?.[network]?.native`. Always read 0 → `maxNativeAvailable` always 0 → branch never approved a switch.
  - The price-impact branch right after switched to native path **without checking the wallet's actual BNB**. Native almost always wins on impact for BSC tokens (1-hop via WBNB pair), so the trader dispatched e.g. a 0.6 BNB swap when the wallet had 0.09 BNB. Chain rejected with `INSUFFICIENT_FUNDS`.
  - Fix: the price-impact branch now gates on `requiredNative + 0.005 (gas reserve) <= actualNative`, falling back to the stablecoin path with a `falling back to stablecoin path` warn line if not. The first branch also now reads from the right field for sane logging.
- **TokenTrader: misleading trailing-stop display.** `state.trailingStopPrice` was always set with `pumpTrailingStopPercent` (4.5%) for both PUMP and MOON regimes, but `_analyzePump` ignores that value and triggers on its own inline `peakPrice * (1 - pumpWideTrailingStopPercent / 100)` (12% by default). Status payloads showed a 4.5% stop that never actually fired in PUMP. Fix: the stored value now reflects the regime-applicable stop — 12% in PUMP, 4.5% in MOON, 5% in high-gain MOON, 8% standard. `_analyzePump`'s inline calc is unchanged so it still triggers identically; `_analyzeMoon`'s reliance on `state.trailingStopPrice` continues to use the 4.5% MOON value.
- **LP MM: 70 days of zero fee accounting.** `lpManager.collectFeesV3` returned only `{txHash, success}`, never the actual amounts collected. `lpMarketMaker.collectFees` updated the `lastFeeCollectAt` timestamp but never incremented `totalFeesCollectedSKYNET` / `totalFeesCollectedWBNB`. Result: the API status displayed `0/0` lifetime fees no matter how many on-chain Collects had happened. Fix: `collectFeesV3` now returns `{txHash, success, amount0, amount1}` with `tokensOwed0/1` formatted to ether-units (read just before the collect tx — same value the chain withdraws, modulo trailing-block accruals). `lpMarketMaker.collectFees` reads those, increments the lifetime totals, persists in one `saveState` call, and logs `+X SKYNET, +Y WBNB (lifetime: ...)` so the deltas are visible in `crypto.log`.

### Files changed

- `src/services/subagents/CryptoStrategyAgent.js` — TokenTrader buy-path balance gating
- `src/services/crypto/strategies/TokenTraderStrategy.js` — regime-aware `state.trailingStopPrice`
- `src/services/crypto/lpManager.js` — `collectFeesV3` returns formatted amounts
- `src/services/crypto/lpMarketMaker.js` — `collectFees` increments lifetime totals; `rebalancePosition` resets `openedAt` on new tokenId (see follow-up)
- `package.json` — 2.25.32 → 2.25.33
- `CHANGELOG.md` — this entry

### Follow-up (same session, post-deploy)

- **LP MM `openedAt` stale across rebalances.** `rebalancePosition` correctly updates `state.tokenId` when it mints a new position, but never refreshed `openedAt` — so the displayed value stuck on the very first position's mint time, even after rebalances created several new tokenIds. Confirmed live: state reported `openedAt: 2026-02-27T02:50:01` while the actual current `tokenId: 6694001` was minted at block 89,508,147 (= `2026-03-29T23:43:43Z`). Fix in `lpMarketMaker.rebalancePosition` adds `openedAt: now` to the `saveState` call alongside the new tokenId. Comments document why `rebalanceCount` (strategy-lifetime telemetry) and `rebalancesLast24h` (circuit-breaker rolling window) stay rolling. Prod state was corrected in-place via `mongosh`; PM2 restarted afterward so the in-memory state matches the DB row.

## [2.25.32] - 2026-05-08

ALICE-authored capability-upgrade PR triage (#2116–#2126) — 11 PRs reviewed one by one. 1 merged as-is, 4 closed-and-replaced with manually-implemented working versions, 6 closed without salvage. Per-PR rationale on each closed PR.

### Added (manual salvage of closed PRs)

- **`POST /api/crypto/lp/mm/health`** — public unauthenticated liveness probe for the LP market maker. Mounted above `router.use(authenticateToken)` so an external monitor can hit it without a JWT. Calls `lpMarketMaker.getConfig()` to verify the underlying service is responsive; returns `{ success, enabled, initialized }` on 200, 503 with the error message if the config fetch throws. Replacement for #2122 which placed an auth-gated payload-less endpoint *after* the auth middleware. (genesis #2122 manually re-implemented.)
- **Self-healing service: scheduled actions.** Three new `selfHealing` plugin actions backed by the existing Agenda `taskScheduler` singleton:
  - `schedule({ actionType, time, reason?, force?, targetPaths? })` — schedules a one-shot future healing action via `agenda.schedule()`. The named Agenda job `'self-healing-action'` is registered at plugin init so the handler is present on every restart.
  - `listScheduled` — lists pending scheduled jobs with `{ jobId, actionType, scheduledFor, reason, lastRunAt, failCount, failReason }`.
  - `cancelScheduled({ jobId })` — `ObjectId`-validates the id and calls `agenda.cancel({ name, _id })`.
  - The job handler `throw`s on healing-action failure so Agenda records `failReason` on the document, visible via `listScheduled`.
  - All three return `{ success: false, error: 'Scheduler not available' }` if the taskScheduler service didn't initialize, rather than throwing.
  - Replacement for #2119 which invented `TaskScheduler.scheduleJob()` (doesn't exist; the class wraps Agenda) and used in-process anonymous-callback closures that wouldn't survive a restart. (genesis #2119 manually re-implemented.)
- **Checkly plugin: scheduled polls.** Same Agenda pattern, recurring instead of one-shot:
  - `schedule_check({ checkId, interval })` — uses `agenda.every(interval, 'checkly-scheduled-poll', { checkId })`. Re-scheduling the same `checkId` cancels the prior schedule first (idempotent). The handler bypasses the local NodeCache and calls `getCheckDetails(checkId)`; throws on failure so Agenda records the reason.
  - `list_scheduled_checks` — `{ jobId, checkId, interval, nextRunAt, lastRunAt, failCount, failReason }`.
  - `cancel_scheduled_check({ checkId })` — drops the schedule for that check.
  - Replacement for #2125 which had the same `TaskScheduler.scheduleJob` invention as #2119, and no list/cancel actions. (genesis #2125 manually re-implemented.)
- **`cookiesAdmin`: actor identity in audit log lines.** Small `actorOf(req)` helper that returns `jwt:<user>` (web admin login), `apikey:<name>` (API-key requests), or `unknown`. Both `POST /cookies/:host` and `DELETE /cookies/:host` log lines now include `by=<actor>` so the central log captures who modified the cookie jar. Replacement for #2118 which referenced `req.user.username` (the JWT payload field is `user`, not `username`) and would crash on API-key auth where `req.user` is undefined. The PR also wanted a flat-file `audit.log` — closed as redundant since the central logger already records the same info per `docs/LOGGING.md`. (genesis #2118 manually re-implemented.)

### Merged (PR #2121)

- **`ModelCache.getUsageAnalyticsData()`** — static aggregation across all ModelCache documents producing `{ totalRequests, averageResponseTime, errorRate }` global rollup. Mirrors the existing per-document `getUsageAnalytics()` instance method. Underlying `usageAnalytics` field is currently dead infrastructure (no caller invokes `trackUsage()`), so the method returns zeros until something starts populating the data. Per-provider breakdown not added; would need a separate small PR alongside the trackUsage wire-up. (genesis #2121.)

### Closed without salvage

- **#2116 agentAccessor.js** — `validateAgent()` checks for placeholder property names (`someMethod`, `someProperty`) the AI didn't fill in; would always return false and prevent `setGlobalAgent(this)` at `agent.js:114` from registering the agent. Also makes `setGlobalAgent` async without awaiting at the call site, race condition by design. Wraps a local-variable assignment in `retryOperation` (can't fail). Adds NodeCache duplicating a singleton.
- **#2117 ContractABI.js** — `getByAddressAndNetwork` adds a projection that strips the `abi` field, defeating the only reason to call the method. The `findByMetadata` projection is defensible but the method has zero callers.
- **#2120 documentLoader.js** — conditional-GET branch (`If-None-Match` / `If-Modified-Since`) is unreachable because the cache-first early-return at `WebLoader.load:130-132` exits before any HTTP request runs. Empty conditional headers when cache is empty. Axios default `validateStatus` rejects 304 as error. The 304 handler reads from a `cached` object that's `undefined` on the only path that reaches it.
- **#2123 UserPreference.js** — three hardcoded "templates" (`{theme, notifications}`) that don't match any real plugin's preference schema. Calling `applyTemplate('youtube', userId, 'subscriptions', 'darkMode')` would overwrite YouTube subscription preferences with `{ theme: 'dark', notifications: true }` — gibberish for that plugin. No template registry, no caller.
- **#2124 outputSchemas.js** — adds `version: 1` to every schema but doesn't add version-aware logic to `adjustSchema` or `validateData`. `version` isn't a JSON Schema keyword. Same defect as previously-closed #2113.
- **#2126 metricsUpdater.js** — wraps `ImprovementMetrics.updateMetrics(date)` (a *write* operation) in a 5-minute result cache, so the cron stops actually re-running the metrics rebuild for the cache TTL. Backfill cache key strips time, causing date-boundary bugs. Description claims exponential backoff but doesn't touch `retryOperation` (which already implements it).

### Files changed

- `src/models/ModelCache.js` (PR #2121 merged)
- `src/api/lpMarketMaker.js` — public `/health`
- `src/api/plugins/selfHealing.js` — `schedule` / `listScheduled` / `cancelScheduled`
- `src/api/plugins/checkly.js` — `schedule_check` / `list_scheduled_checks` / `cancel_scheduled_check`
- `src/interfaces/web/cookiesAdmin.js` — actor identity in audit logs
- `src/interfaces/web/webInterface.js` — mount-order fix (lpMarketMakerRoutes before cryptoRoutes — see follow-up below)
...
- `docs/sessions/SESSION-SUMMARY-2026-05-07.md` — appended PR triage section

### Follow-up: mount-order fix for `/api/crypto/lp/mm/health`

Post-deploy verification of the new public `/health` endpoint surfaced a routing bug — it was returning `401 Authentication required` instead of the liveness payload, because `cryptoRoutes` (mounted at `/api/crypto`) has `router.use(authMiddleware)` at line 53 that 401s any request matching the `/api/crypto/*` prefix before Express can fall through to the deeper `/api/crypto/lp/mm` mount. The new `/health` endpoint inside `lpMarketMaker.js` (correctly mounted above its own router-level auth middleware) was unreachable for unauthenticated callers.

Fixed by swapping the mount order in `src/interfaces/web/webInterface.js` so `lpMarketMakerRoutes` mounts *before* `cryptoRoutes`. The `/api/crypto/lp/mm/*` paths now hit `lpMarketMaker.js` first, where `/health` is defined above the auth middleware and the rest of the routes still gate on the lpMarketMaker router-level `authMiddleware`. Verified live on production:

```
$ curl http://192.168.x.x/api/crypto/lp/mm/health
...
Operator noticed that the `tokenTraderStatus` block on the crypto strategy agent reported `lastAnalysis: 2026-05-06T03:57:46.364Z` — ~36 hours stale despite the agent itself being healthy and ticking dollar_maximizer every 10 minutes. Investigation surfaced that the registry-rebuild incident from the previous day (commit `2853417f` restored the missing strategy-class registrations from the 2026-04-10 public→genesis sync, but the runtime token_trader instance for [redacted-token] at `0x997a58129890bbda032231a52ed1ddc845fc18e1` was lost at the same time — `secondaryStrategy` reset to `null`, `tokenTraders` map empty after restart). The wallet still held the [redacted-token] position entirely unmanaged: no trailing stop, no scale-out logic, no monitoring. The cached `tokenTraderStatus` display block in `domainState` was a stale leftover from when [redacted-token] was last alive.

Fixing it by re-running `POST /api/crypto/strategy/token-trader/configure` + `POST /api/crypto/strategy/token-trader/restore-state` exposed two latent bugs in `src/api/crypto.js`:

### Fixed

- **Token-trader heartbeat manager doesn't auto-start when first instance is added at runtime.** `src/api/crypto.js:1441` guarded the `startToken()` call on `if (handler.tokenHeartbeatManager?.started)` — but `started` only flips to `true` inside `TokenTraderHeartbeatManager.startAll()`, which `CryptoStrategyAgent.initialize()` only calls when `allTokenTraders.size > 0` at agent init. Configure on a fresh registry (e.g., after a restart with empty `tokenTraders` map) registered the new instance, set `secondaryStrategy='token_trader'`, and persisted to MongoDB — but never started the heartbeat. The token then only ticked via the 10-min main-heartbeat fallback path on `CryptoStrategyAgent.js:567-589` (which runs token_trader sequentially without the regime-based interval). For [redacted-token] sitting in `PUMP` regime that's a 7× degradation — should be 3-min ticks (PUMP), got 10-min ticks. Configure now does: `if (!handler.tokenHeartbeatManager.started) handler.tokenHeartbeatManager.startAll(); else handler.tokenHeartbeatManager.startToken(tokenAddress);` — `startAll()` picks up the newly-registered instance from the registry on its first call.
- **`/strategy/token-trader/restore-state` mutated state in-memory only, never persisted.** The endpoint at `src/api/crypto.js:1568-1642` writes restored `position`, `pnl`, `tracking`, `circuitBreaker`, `regime` directly to `tokenStrategy.state.*` and returned 200 — but no `persistRegistryState()` call. The restored cost basis / realized PnL / trailing-stop / peak-price values only survived in memory until the next full `execute()` cycle ran, where the `finally` block at `CryptoStrategyAgent.js:747` finally persists. If the agent restarted before that, the restore was lost. Compare `/configure` at line 1436 which does call `persistRegistryState()` before returning. `/restore-state` now matches.

### Operational notes

- Recovered [redacted-token] live during the investigation. Two API calls (configure with `tokenAddress=0x997a…18e1`, `tokenNetwork=bsc`, `capitalAllocationPercent=50`, `tokenTaxPercent=0`; then restore-state with the cached cost basis from the stale `tokenTraderStatus` block) re-armed the trader. Unbeknownst to the operator, [redacted-token] had pumped **[redacted]** while the trader was offline. The first three ticks after restore-state caught the pump: scale-out at +20% level (sold [redacted] [redacted-token] @ [redacted], [redacted] PnL), then +25% level (sold [redacted] [redacted-token] @ [redacted], [redacted] PnL), then +30% level (sold [redacted] [redacted-token] @ [redacted], [redacted] PnL). **Realized PnL [redacted] [redacted] in 7 minutes** during the rescue. Trailing stop updated from [redacted] to [redacted] (8% below new peak [redacted]). Verified post-deploy: `heartbeats.started: true`, `tokenCount: 1`, [redacted-token] listed with `regime: PUMP`, `interval: 180s (3m)`, `consecutiveErrors: 0`, `backedOff: false`, `lastRun: 2026-05-07T16:51:54.599Z`.
- Genesis-only fix — public repo's scrubbed registry stays as-is per the v2.25.26 strategy-name scrub. Do NOT sync this commit to public.
...
- `CHANGELOG.md` — this entry
- `docs/api/API_README.md` — v2.25.31 section
- `docs/api/LANAgent_API_Collection.postman_collection.json` — version bump + description update
- `docs/feature-progress.json` — `cryptoTokenTraderHeartbeatFix_2026_05_07` entry
- `docs/sessions/SESSION-SUMMARY-2026-05-07.md` — this session's report
...
FlareSolverr cost-to-serve is well under [redacted]/call (long-running Docker container, no per-call cost on top of fixed VPN/server overhead). The 5cr sticker was almost entirely margin and read expensive next to:

- **ScraperAPI** ultra_premium: $0.03+/call at the cheapest plan, plus a $49/mo floor
- **ScrapeGraphAI** stealth-extract: [redacted]/call on their [redacted]/mo Pro plan
...
### Verified end-to-end on prod (<vps-ip>)
- Format validation: `gsk_short` rejected, arbitrary text rejected, real key accepted
- `validateKey` against gateway: real key returns `{ ok: true, credits: [redacted] }`, fake-but-well-formed key returns `unauthorized`
...
- **PancakeSwap liquidity deepened** — added BNB + SKYNET to the V2 LP and staked the resulting LP tokens at Tier 3 (180 days, 3x multiplier).
...
- ALICE restaked SKYNET rewards on diamond tier
...
- SKYNET rewards deposited on-chain (proportional to staked holdings)
...
- **Gateway agent URL** — was pointing to `api.lanagent.net` (itself); fixed to `http://10.8.0.x:80` (ALICE via VPN)
...
- Swap tx: `0x8b9ede...` (0.008 BNB → [redacted] SKYNET)
- Transfer tx: `0x7c16e3...` ([redacted] SKYNET → ALICE → +491 credits)
...
### Added (Staking)
- **Auto-claim rewards** — CryptoStrategyAgent heartbeat claims staking rewards every 6 hours when above [redacted] SKYNET threshold
...
- Test exercised ETH + BSC gas paths and a cross-agent SKYNET transfer (BETA → ALICE)
...
- **Token Trader P&L** — Now computed from actual instance data (lifetimeRealizedPnL - gasCost) instead of broken accumulator that showed [redacted] instead of real ~-$70
...
### Decommissioned
- SkynetStaking V1 (`0x9205b5E1...`) — phantom totalStaked bug, SKYNET locked
...
### Fixed
- **Crypto trading system offline** — `CryptoStrategyAgent.js` was deployed as 0-byte empty file on Mar 17, taking all 4 token traders (BANANA, RIVER, BTW, [redacted-token]) offline for ~16 hours. Positions were safe in MongoDB; restored by redeploying intact file
...
- **CoW Protocol (CoW Swap) integration** — Intent-based DEX aggregator for Ethereum, Base, and Arbitrum swaps
  ...
  - Successfully sold residual tokens that had no Uniswap V2/V3 liquidity
...
### Added
- **On-Chain Staking Contract** — Synthetix-style `SkynetStaking.sol` deployed to BSC (`0x9205b5E16E3Ef7e6Dd51EE6334EA7f8D7Fec31d6`), funded with a SKYNET reward pool (7-day epochs, auto-renewed by scheduler). Verified via Sourcify perfect match.
...
- **SKYNET/BNB liquidity pool** — Initial PancakeSwap V2 pool seeded with SKYNET + BNB. Pair: `0xF3dEF3534EEC3195e0C217938F710E6F2838694A`
...
- **Token trader balance in wallet UI** — Balances section now shows the active token trader token (e.g., [redacted-token]) with its current balance
...
- **Token symbol resolution** — fixed KNOWN_TOKEN_SYMBOLS map with correct contract addresses for FHE, [redacted-token], LINK
...
- **Crypto Agent Not Trading**
  ...
  - Example: ETH baseline reset from [redacted] → [redacted] (was 9.34% below)
  - Example: BNB baseline reset from [redacted] [redacted] (was 5.24% below)
...
- **Network Scanner**: Fixed multiple issues preventing device detection
  ...
  - Fixed subnet detection (192.168.x.x/24 instead of hardcoded 192.168.x.x/24)
