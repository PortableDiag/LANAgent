...
- **SIREN TokenTrader `capitalAllocationPercent` 50 → 65** on ALICE via `POST /api/crypto/strategy/token-trader/configure`. Position state and all other settings preserved.
...
- **SIREN smart-sizing fix from v2.25.50 had its first real production test today.** Yesterday's 12/12 grid-buy cap-jam cleared in a single 1.6h window (21:33–23:13 UTC): grid sells fired at +1.1–1.8% adaptive triggers; `consecutiveGridBuys` reset 10 → 0. Strategy is back in its natural buy-low/sell-high grind.
...
- **Item 4a/4b** (SIDEWAYS-regime peak-anchored scale-out, plus a new `RangeTraderStrategy` class) — discussed and authorized this session but deferred. Reasoning: (a) SIREN's current price ([redacted] vs peak [redacted]) is at 89.6% of peak, so a range-anchored sell rule wouldn't fire today even with the override in place — no observable behavior change; (b) #4b is a substantial new strategy class needing careful design; (c) deploying these alongside the just-shipped smart-sizing and DM tightening would confound any performance-impact observations from #1 + #2. Better to let #1 + #2 run for a day, observe SIREN's first post-sell cycle under the new sizing, then tackle #4a/#4b informed by data.
...
Operator noticed that the `tokenTraderStatus` block on the crypto strategy agent reported `lastAnalysis: 2026-05-06T03:57:46.364Z` — ~36 hours stale despite the agent itself being healthy and ticking dollar_maximizer every 10 minutes. Investigation surfaced that the registry-rebuild incident from the previous day (commit `2853417f` restored the missing strategy-class registrations from the 2026-04-10 public→genesis sync, but the runtime token_trader instance for SIREN at `0x997a58129890bbda032231a52ed1ddc845fc18e1` was lost at the same time — `secondaryStrategy` reset to `null`, `tokenTraders` map empty after restart). The wallet still held the SIREN position entirely unmanaged: no trailing stop, no scale-out logic, no monitoring. The cached `tokenTraderStatus` display block in `domainState` was a stale leftover from when SIREN was last alive.
...
- Recovered SIREN live during the investigation. Two API calls (configure with `tokenAddress=0x997a…18e1`, `tokenNetwork=bsc`, `capitalAllocationPercent=50`, `tokenTaxPercent=0`; then restore-state with the cached cost basis from the stale `tokenTraderStatus` block) re-armed the trader. Unbeknownst to the operator, SIREN had pumped **[redacted]** while the trader was offline. The first three ticks after restore-state caught the pump: scale-out at +20% level (sold [redacted] SIREN @ [redacted], [redacted] PnL), then +25% level (sold [redacted] SIREN @ [redacted], [redacted] PnL), then +30% level (sold [redacted] SIREN @ [redacted], [redacted] PnL). **Realized PnL [redacted] [redacted] in 7 minutes** during the rescue. Trailing stop updated from [redacted] to [redacted] (8% below new peak [redacted]). Verified post-deploy: `heartbeats.started: true`, `tokenCount: 1`, SIREN listed with `regime: PUMP`, `interval: 180s (3m)`, `consecutiveErrors: 0`, `backedOff: false`, `lastRun: 2026-05-07T16:51:54.599Z`.
