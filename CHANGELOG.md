...
Gateway-side release. Agent code unchanged from v2.25.12. All work lives in the `api-lanagent-net` gateway repo deployed to the public portal at `api.lanagent.net`; this entry tracks it from the platform-level changelog.

### Added
- **Gateway: usage-threshold notification emails** — transactional emails fire automatically as a portal user crosses 80% / 90% / 100% of their subscription bucket (basic + render, separately) or their credit-pool balance. Credit thresholds are computed against a `creditAlertBaseline` (snapshot of balance after most recent top-up) so each top-up resets the alert window. Sub thresholds reset on `invoice.paid` (`subscription_cycle`). Fire-once-per-cycle enforced via atomic `findOneAndUpdate` so concurrent debits can't double-send. Send is fire-and-forget — `/scrape` request handlers never block on SMTP. New `email.mjs` module on the gateway with `nodemailer` SMTP transport over `mail.lanagent.net:587` via a dedicated `noreply@lanagent.net` mailbox. Sender displays as `LANAgent API <noreply@lanagent.net>` to distinguish service-level alerts from agent-level mail.
- **Gateway: account-management email suite** — password reset (`POST /portal/forgot-password` + `POST /portal/reset-password` + HTML reset form at `GET /portal/reset?token=…`), email verification (`GET /portal/verify?token=…` + `POST /portal/resend-verification`), welcome email on signup (with API key + first-curl example), API-key created/revoked notifications (with name, prefix, source IP, timestamp), subscription cancellation confirmation (with period-end date). New `PortalToken` collection with TTL index — Mongo auto-removes expired tokens. Tokens are SHA-256 hashed in DB; raw value only ever leaves in the email URL. Minting a fresh token invalidates prior unused tokens of the same purpose so stale links can't be replayed. Polished HTML wrapper template across all transactional emails (brand-color top stripe, monospace code blocks, two-column footer, preheader for inbox preview).
- **Gateway: support inbox v1 + v2** — `support@lanagent.net` mailbox monitored by a new `support-poller` PM2 service on the gateway VPS. Polls IMAPS every 60s for `UNSEEN` messages. v1 (notification-only): for each new message, sends a Telegram alert to the operator with from / subject / body snippet; skips bouncebacks (`Auto-Submitted` header), `Precedence: bulk/junk/list`, and noreply-pattern senders. v2 (context-enriched): same flow plus user-context lookup — looks up the `From:` address in `PortalUser`, builds 7-day usage context (sub state with bucket %, credits, recent request totals, top failing routes, most recent failure with error message + age, last payment), and includes that in the Telegram message. Marks messages `\Seen` after notify so each ticket pings exactly once.

### Documentation
- **New proposal**: `docs/proposals/ai-support-system.md` documents the v3 → v5 roadmap for AI-driven support — LLM-drafted replies with operator approval (v3), auto-send for trusted categories (v4), proactive outreach on internal events (v5). Architecture split: gateway owns customer state, ALICE owns LLM. Recommended sequence: sit on v2 for at least 2 weeks first, real ticket volume informs whether the LLM-draft investment is worth it.
...
- **Gateway: Scraper Subscriptions** (separate repo) — three monthly Stripe subscription tiers on `api.lanagent.net`, additive to existing one-time credit packages and crypto purchases: Scraper Starter ($10/mo, 100K basic + 2.5K render), Scraper Pro ($25/mo, 500K basic + 15K render), Scraper Business ($50/mo, 2M basic + 50K render). Subscriptions cover only `/scrape` and `/scrape/batch`; all 25 services still bill from the credit pool. Atomic Mongo ops via `$expr` enforce limits at the DB level. Tier mapping: `basic`/`stealth`/`full` → basic counter, `render` → render counter (separate independent buckets). When a bucket is exhausted, `/scrape` falls back to the credit pool automatically — never hard-fails as long as the user has either subscription quota or credits. Quota resets on `invoice.paid` (`subscription_cycle`).
- **Gateway: first-month promo discount on subscriptions** — when an active `Promotion` document exists, `/portal/subscribe` attaches a Stripe coupon (idempotent, `duration: 'once'`) to the Checkout session. Discount applies to first invoice only; renewals revert to full base price.
- **Gateway: defensive Stripe customer recovery** — `getOrCreateStripeCustomer(user)` validates a stored `stripeCustomerId` against Stripe before use. If the customer is missing (left over from test→live key switches, or deleted in the dashboard), a new customer is created and the user record is updated.

### Fixed
- **Portal landing-page price labels** — service cards on `api.lanagent.net` were showing the cheaper plugin-route prices instead of the dedicated-route prices. Web Scraping now shows tier breakdown (1/2/3/5 cr); YouTube split into Video (10 cr) + Audio (8 cr) cards; Media Processing renamed to Media Transcoding (20 cr). Removed redundant "Advanced Scraper" card.
