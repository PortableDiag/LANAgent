...
export class OpenAIProvider extends BaseProvider {
  ...
  async generateResponse(prompt, options = {}) {
    ...
    try {
      ...
      //
      // Reasoning models (gpt-5, o-series) consume the budget on internal
      // thinking before any visible token. Without enough headroom, the
      // model returns content="" with reasoning_tokens equal to the full
      // budget (observed across the entire codebase: auto-post, scanners,
      // intent detector, scam classifier, etc.). The fix needs to apply at
      // every caller — so we apply it here, in the provider, rather than
      // in 124+ call sites.
      //
      // - Default reasoning_effort to 'minimal' for reasoning models if the
      //   caller didn't specify. Callers that want deeper thinking can pass
      //   additionalParams: { reasoning_effort: 'low'|'medium'|'high' }.
      // - Floor the budget at 4000 for reasoning models so even
      //   "maxTokens: 20" intent-detection calls have room for one short
      //   visible answer after minimal reasoning.
      const callerMax = options.maxTokens || 1000;
      if (isGPT5Model) {
        completionParams.max_completion_tokens = Math.max(callerMax, 4000);
        if (completionParams.reasoning_effort === undefined) {
