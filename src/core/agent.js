...
import { ragVectorStore } from "../services/ragVectorStore.js";
...
export class Agent extends EventEmitter {
  ...
  async initialize() {
    ...
    // don't block the agent — the plugin falls back to degraded mode.
    try {
      await ragVectorStore.initialize();
      this.vectorStore = ragVectorStore;
    } catch (err) {
      logger.warn(`RAG vector store init failed (knowledge plugin will run in degraded mode): ${err?.message || err}`);
    ...
    try {
      ...
      // Initialize P2P Federation service (opt-in via SystemSettings or env var)
      {
        ...
        if (p2pEnabled) {
          ...
          // Auto-onboard with api.lanagent.net gateway (signed self-registration
          // using the P2P Ed25519 identity; provisions a WireGuard tunnel if
          // the gateway can't reach the agent directly). Best-effort: failures
          // are retried in the background and never block startup.
          //
          // Opt-out via `GATEWAY_AUTO_ONBOARD=false` in .env — required for
          // legacy agents (ALICE, BETA pre-migration) that already have a
          // gateway record without a fingerprint AND/OR an existing wg0
          // interface that the auto-onboarding client could overwrite.
          if (process.env.GATEWAY_AUTO_ONBOARD === 'false') {
            logger.info('Gateway auto-onboarding skipped (GATEWAY_AUTO_ONBOARD=false)');
          } else {
            try {
              const { GatewayClient } = await import('../services/gateway/gatewayClient.js');
              this.gatewayClient = new GatewayClient(this);
              await this.gatewayClient.initialize();
              this.services.set('gatewayClient', this.gatewayClient);
            } catch (error) {
              logger.warn(`Gateway client setup failed: ${error.message}`);
            }
          }
        }
      }
              ...
              try {
                ...
                if (!provider) {
                  const msg = 'No email provider peer is online right now. Make sure the P2P network is connected and the genesis instance is running.\n\n' +
                    ...
                    `3. SKYNET token address: \`0x8b77CC5c6cB3d846608d9d5Dd03fA406BA03b8F1\`\n` +
                ...
                const msg = `Email lease requested: **${desiredUsername}@lanagent.net** from peer \`${provider.fingerprint.slice(0, 8)}...\`\n\n` +
                  ...
                  (price > 0 ? `**Cost:** ${price} SKYNET tokens (BSC)\n` +
                    `**SKYNET token:** \`0x8b77CC5c6cB3d846608d9d5Dd03fA406BA03b8F1\`\n` +
...
Return ONLY a valid JSON object with the extracted parameters, nothing else.`;
              ...
              const aiResponse = await this.providerManager.generateResponse(extractionPrompt, {
                temperature: 0.3,
                // Reasoning models (gpt-5, o-series) consume the maxTokens budget
                // internally before any visible output token. 200 was fine for
                // gpt-4o but with gpt-5 the response came back empty, JSON.parse
                // failed, and downstream plugins received no params (manifested
                // as ytdlp "URL or search query required" on a clear URL input).
                maxTokens: 1500
              });

              // Parse the AI response
              const rawContent = aiResponse?.content ?? '';
              const cleanedResponse = rawContent.trim()
                .replace(/^```json\s*/, '')
                .replace(/\s*```$/, '')
                .replace(/^```\s*/, '');

              if (!cleanedResponse) {
                throw new Error(`empty response from ${aiResponse?.provider || 'provider'} (${aiResponse?.model || 'unknown model'})`);
              }
