...
/**
 * Batch process emails matching a filter, paging through results without
 * prefetching every id up front. Delegates per-page work to batchProcessEmails.
 */
emailSchema.statics.batchProcessByFilter = async function(filter, {
  markAsRead = false,
  ...
  logger.info(`batchProcessByFilter starting: filter=[${filterKeys.join(',')}] pageSize=${pageSize} maxDocs=${maxDocs} markAsRead=${markAsRead} markAsProcessed=${markAsProcessed} processedBy=${processedBy}`);

  let page = 0;
  while (scannedCount < maxDocs) {
    const limit = Math.min(pageSize, maxDocs - scannedCount);
    const docs = await retryOperation(
      () => this.find(filter).select('messageId').skip(page * pageSize).limit(limit).lean(),
      { retries: 3 }
    );
    const pageIds = (docs || []).map(d => d.messageId).filter(Boolean);
    if (pageIds.length === 0) break;
    scannedCount += pageIds.length;

    const result = await this.batchProcessEmails(pageIds, { markAsRead, markAsProcessed, processedBy });
    totals.matchedCount += result.matchedCount || 0;
    totals.modifiedCount += result.modifiedCount || 0;
    totals.upsertedCount += result.upsertedCount || 0;
    processedCount += pageIds.length;

    logger.info(`batchProcessByFilter page ${page + 1}: scanned=${scannedCount} processed=${processedCount}`);

    if (pageIds.length < pageSize) break;
    page += 1;
  }

  return { ...totals, scannedCount, processedCount };
};
