...
const monitoringConfig = {
  ...
  // Alert throttling + dedup + maintenance-window suppression
  alertCooldownSeconds: parseInt(process.env.DIAGNOSTIC_ALERT_COOLDOWN_SECONDS || '900'),
  maintenanceWindow: process.env.DIAGNOSTIC_MAINTENANCE_WINDOW || '', // "HH:MM-HH:MM", crosses midnight OK
  dedupeBy: (process.env.DIAGNOSTIC_ALERT_DEDUPE_BY || 'overall') // 'overall' | 'summary' | 'hash'
};

// Alert throttling state (in-memory, bounded LRU)
const lastAlertAtByKey = new Map();
const recentSummariesCache = new NodeCache({ stdTTL: 3600, checkperiod: 300, useClones: false });
let recentSummariesKeys = [];
function addRecentKey(key) {
  if (!recentSummariesCache.has(key)) {
    recentSummariesCache.set(key, true);
    recentSummariesKeys.push(key);
    if (recentSummariesKeys.length > 100) {
      const toRemove = recentSummariesKeys.shift();
      if (toRemove) recentSummariesCache.del(toRemove);
    }
  } else {
    recentSummariesKeys = recentSummariesKeys.filter(k => k !== key);
    recentSummariesKeys.push(key);
  }
}
function hasSeenRecently(key) { return !!recentSummariesCache.get(key); }
function parseMaintenanceWindow(s) {
  if (!s || typeof s !== 'string') return null;
  const parts = s.split('-');
  if (parts.length !== 2) return null;
  const toMin = (x) => {
    const [hh, mm] = x.split(':').map(n => parseInt(n, 10));
...
function isWithinMaintenanceWindow(now, windowStr) {
  const p = parseMaintenanceWindow(windowStr);
  if (!p) return false;
  const m = now.getHours() * 60 + now.getMinutes();
  if (p.start === p.end) return false;
  return p.start < p.end ? (m >= p.start && m < p.end) : (m >= p.start || m < p.end);
}
function getAlertKey(report, mode) {
  try {
    if (mode === 'summary') return `${report.overallHealth}:${(report.summary || '').slice(0, 256)}`;
    if (mode === 'hash') {
      const content = JSON.stringify({
        h: report.overallHealth,
        s: report.summary,
        t: (report.tests || []).filter(t => t.status !== 'passed').map(t => ({ n: t.name, s: t.status, m: t.message }))
      });
      let hash = 0;
      for (let i = 0; i < content.length; i++) { hash = ((hash << 5) - hash) + content.charCodeAt(i); hash |= 0; }
      return `${report.overallHealth}:${hash}`;
    }
    return `overall:${report.overallHealth}`;
  } catch (e) {
    logger.warn('Failed to build alert key, falling back to overall', e);
    return `overall:${report.overallHealth}`;
  }
}
...
if (monitoringConfig.enabled) {
  reportEventEmitter.on('statusChange', async (report) => {
    try {
      ...
      // Logs always fire regardless of maintenance/cooldown — visibility is independent of email send
      ...
      if (!shouldAlert || !monitoringConfig.emailConfig.enabled) return;

      const now = new Date();

      // Maintenance window suppression
      if (isWithinMaintenanceWindow(now, monitoringConfig.maintenanceWindow)) {
        logger.info('Alert suppressed (maintenance window)', {
          window: monitoringConfig.maintenanceWindow,
          overallHealth: report.overallHealth
        });
        return;
      }

      // Dedup + cooldown throttling
      const key = getAlertKey(report, monitoringConfig.dedupeBy);
      const cooldownMs = (Number.isFinite(monitoringConfig.alertCooldownSeconds) ? monitoringConfig.alertCooldownSeconds : 900) * 1000;
      const lastAt = lastAlertAtByKey.get(key);
      if (hasSeenRecently(key) && lastAt && (now.getTime() - lastAt) < cooldownMs) {
        logger.info('Alert suppressed (cooldown/dedup)', {
          key,
          sinceLastSeconds: Math.floor((now.getTime() - lastAt) / 1000),
          remainingSeconds: Math.ceil((cooldownMs - (now.getTime() - lastAt)) / 1000),
          dedupeBy: monitoringConfig.dedupeBy
        });
        return;
      }

      await retryOperation(() => sendAlertEmail(report), { retries: 3, context: 'DiagnosticReport.sendAlertEmail' });
      addRecentKey(key);
      lastAlertAtByKey.set(key, now.getTime());
    } catch (error) {
