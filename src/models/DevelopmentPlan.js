...
/**
 * Set the progress percentage (0-100) on a development item.
 */
developmentItemSchema.statics.updateProgress = async function(itemId, percentage) {
  ...
  if (!updated) throw new Error(`Development item ${itemId} not found`);
  logger.info(`DevelopmentPlan: progress for ${itemId} = ${percentage}%`);
  return updated;
};

/**
 * Push a milestone onto a development item.
 * @param {string} itemId
 * @param {{title:string, description?:string, completed?:boolean, dueDate?:Date}} milestone
 */
developmentItemSchema.statics.addMilestone = async function(itemId, milestone) {
  ...
  if (!updated) throw new Error(`Development item ${itemId} not found`);
  logger.info(`DevelopmentPlan: added milestone "${milestone.title}" to ${itemId}`);
  return updated;
};
