...
/**
 * Atomically add a single task using $addToSet. Idempotent (duplicates ignored).
 * Invalidates the tasks_<projectId> cache on success.
 */
projectSchema.statics.addTaskAtomic = async function(projectId, taskId) {
  try {
    const updated = await retryOperation(() => this.findOneAndUpdate(
      { _id: projectId },
      { $addToSet: { tasks: taskId } },
      { new: true }
    ), { context: 'Project.addTaskAtomic', retries: 3 });
    if (updated) cache.del(`tasks_${projectId}`);
    return updated;
  } catch (error) {
    logger.error('Error in addTaskAtomic:', error, { projectId, taskId });
    throw new Error('Failed to atomically add task');
  }
};

/**
 * Atomically add multiple tasks via $addToSet+$each. Duplicates filtered locally
 * and by MongoDB. Returns the project unchanged when taskIds is empty.
 */
projectSchema.statics.addTasksAtomic = async function(projectId, taskIds) {
  try {
    const unique = [...new Set(taskIds || [])];
    if (unique.length === 0) return await this.findById(projectId);
    const updated = await retryOperation(() => this.findOneAndUpdate(
      { _id: projectId },
      { $addToSet: { tasks: { $each: unique } } },
      { new: true }
    ), { context: 'Project.addTasksAtomic', retries: 3 });
    if (updated) cache.del(`tasks_${projectId}`);
    return updated;
  } catch (error) {
    logger.error('Error in addTasksAtomic:', error, { projectId, taskCount: Array.isArray(taskIds) ? taskIds.length : 0 });
    throw new Error('Failed to atomically add tasks');
  }
};

/**
 * Atomically remove a single task via $pull.
 */
projectSchema.statics.removeTaskAtomic = async function(projectId, taskId) {
  try {
    const updated = await retryOperation(() => this.findOneAndUpdate(
      { _id: projectId },
      { $pull: { tasks: taskId } },
      { new: true }
    ), { context: 'Project.removeTaskAtomic', retries: 3 });
    if (updated) cache.del(`tasks_${projectId}`);
    return updated;
  } catch (error) {
    logger.error('Error in removeTaskAtomic:', error, { projectId, taskId });
    throw new Error('Failed to atomically remove task');
  }
};

/**
 * Atomically remove multiple tasks via $pull+$in.
 */
projectSchema.statics.removeTasksAtomic = async function(projectId, taskIds) {
  try {
    if (!Array.isArray(taskIds) || taskIds.length === 0) return await this.findById(projectId);
    const updated = await retryOperation(() => this.findOneAndUpdate(
      { _id: projectId },
      { $pull: { tasks: { $in: taskIds } } },
      { new: true }
    ), { context: 'Project.removeTasksAtomic', retries: 3 });
    if (updated) cache.del(`tasks_${projectId}`);
    return updated;
  } catch (error) {
    logger.error('Error in removeTasksAtomic:', error, { projectId, taskCount: Array.isArray(taskIds) ? taskIds.length : 0 });
    throw new Error('Failed to atomically remove tasks');
  }
};
