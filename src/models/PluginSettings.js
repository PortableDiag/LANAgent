...
// In-flight dedup for getCached — prevents cache stampedes on cold load
const inFlight = new Map();
function computeSoftExpiresAt(ttl) {
  // ±10% jitter avoids synchronized expiry across many keys
  const jitter = 1 + ((Date.now() % 1000) / 1000 - 0.5) * 0.2;
 ...
 */
pluginSettingsSchema.statics.getCached = async function(pluginName, settingsKey, optionsOrTtl = 300) {
  // Backwards-compat: accept legacy `(name, key, ttl)` signature or new options object.
  const opts = typeof optionsOrTtl === 'number'
    ? { ttl: optionsOrTtl, staleWhileRevalidateSeconds: 120 }
    : { ttl: 300, staleWhileRevalidateSeconds: 120, ...optionsOrTtl };
  const { ttl, staleWhileRevalidateSeconds } = opts;
  const cacheKey = `${pluginName}:${settingsKey}`;
  const now = Date.now();
  
  // Cache hit path — entries are `{ value, softExpiresAt }`
  const cached = settingsCache.get(cacheKey);
  if (cached !== undefined) {
    const { value, softExpiresAt } = cached;
    if (typeof softExpiresAt === 'number' && now < softExpiresAt) {
      logger.debug(`Cache hit (fresh) for ${cacheKey}`);
      return value;
    }
    // Stale-while-revalidate: serve stale immediately, refresh in background
    logger.debug(`Cache hit (stale) for ${cacheKey}, kicking background refresh`);
    if (!inFlight.has(cacheKey)) {
      inFlight.set(cacheKey, (async () => {
        try {
          const doc = await this.findOne({ pluginName, settingsKey });
          const fresh = doc ? doc.settingsValue : null;
          settingsCache.set(cacheKey, { value: fresh, softExpiresAt: computeSoftExpiresAt(ttl) }, ttl + staleWhileRevalidateSeconds);
          return fresh;
        } catch (err) {
          logger.warn(`Background refresh failed for ${cacheKey}: ${err.message}`);
          return value;
        } finally {
          inFlight.delete(cacheKey);
        }
  ...
  // Cold miss — dedup concurrent loads
  if (inFlight.has(cacheKey)) {
    logger.debug(`Awaiting in-flight load for ${cacheKey}`);
  ...
  const loadPromise = (async () => {
    try {
      const doc = await this.findOne({ pluginName, settingsKey });
      const value = doc ? doc.settingsValue : null;
      settingsCache.set(cacheKey, { value, softExpiresAt: computeSoftExpiresAt(ttl) }, ttl + staleWhileRevalidateSeconds);
      return value;
    } finally {
      inFlight.delete(cacheKey);
    }
...
pluginSettingsSchema.statics.clearCache = function(pluginName) {
  if (pluginName) {
    ...
    pluginKeys.forEach(key => { settingsCache.del(key); inFlight.delete(key); });
