...
// In-flight de-dup map prevents cache stampedes on hot getCached/getByName/getEnabled paths
  ...
  const p = (async () => {
    try {
      ...
      if (doc) {
        ...
        serverCache.set(cacheKey, null, 30); // negative cache 30s
      }
      return doc;
    } finally {
  ...
  const p = (async () => {
    try {
      ...
      if (doc) {
        ...
        serverCache.set(cacheKey, null, 30);
      }
      return doc;
    } finally {
      inFlight.delete(cacheKey);
...
mcpServerSchema.statics.getEnabled = async function(ttl = 300) {
  ...
  const p = (async () => {
    try {
      ...
      // Cache even the empty result for 30s to avoid stampedes when no servers exist
      serverCache.set(cacheKey, docs || [], (docs && docs.length) ? ttl : 30);
      return docs || [];
    } finally {
      inFlight.delete(cacheKey);
    }
