...
router.post('/bulk-register', async (req, res) => {
    try {
        ...
        const results = settled.map((item, index) => {
            ...
            logger.warn('Bulk register item failed', {
                index,
            ...
            return { index, success: false, error: safeError };
        });
        ...
        if (summary.succeeded > 0) invalidateAccountCache();

        const status = (summary.failed > 0 && summary.succeeded > 0) ? 207 : 200;
...
router.patch('/bulk-status', async (req, res) => {
    try {
        ...
        const settled = await Promise.allSettled(updates.map(update =>
        ...
        const results = settled.map((item, index) => {
            ...
            const original = updates[index] || {};
            const err = item.reason || {};
            const safeError = {
                message: err.message || 'Operation failed',
                code: err.code || err.status || err.statusCode
            };
            logger.warn('Bulk status update item failed', {
                index,
                accountId: original.accountId,
                desiredStatus: original.status,
                errorMessage: safeError.message,
                errorCode: safeError.code
            });
            return { index, success: false, error: safeError };
