...
export class BasePlugin extends EventEmitter {
   ...
   * @returns {Promise<Object>} Send result. When no notification channel is
   *   configured, resolves to `{ delivered: false, reason: 'no_notification_interface' }`
   *   instead of throwing — a missing channel is a normal deployment state, not
   *   an error. This keeps the ~80 notify() call sites across plugins from each
   *   having to guard against / log a non-error on agents without Telegram.
   */
  async notify(message, options = {}) {
    const telegram = this.getInterface('telegram');
    if (!telegram) {
      this._logger.debug(`notify skipped — no telegram interface: ${String(message).slice(0, 80)}`);
      return { delivered: false, reason: 'no_notification_interface' };
    }
