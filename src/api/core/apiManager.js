    ...
    try {
      ...
      if (plugin.initialize) {
        try {
          ...
          // Two "soft fail" shapes register the plugin as disabled rather
          // than crashing the load — both leave the plugin visible in the
          // UI with a clear reason so the operator can act on it.
          //   - Missing credentials → fixable from Settings.
          //   - UPSTREAM_UNAVAILABLE → external dependency is down (or has
          //     been retired); plugin will stay disabled until the
          //     upstream comes back AND the plugin is reloaded.
          ...
          const isUpstreamUnavailable = initError.code === 'UPSTREAM_UNAVAILABLE';
          if (isCredentialError || isUpstreamUnavailable) {
            this.apis.set(plugin.name, {
              ...
              disabledReason: isUpstreamUnavailable ? 'upstream_unavailable' : 'missing_credentials'
            });
            const tail = isUpstreamUnavailable
              ? `Plugin will stay disabled until the upstream is reachable and the plugin is reloaded.`
              : `Add credentials via Settings.`;
            pluginLogger.warn(`Plugin ${plugin.name} v${plugin.version} registered (disabled — ${initError.message}). ${tail}`);
            return;
          }
          throw initError;
        }
