 ...
 * @param {{ ttl?: number, swrTtl?: number, revalidate?: boolean }} [options]
 *   ttl: hard freshness window (seconds, default 300)
 *   swrTtl: extra seconds to keep serving stale during background refresh (default 120)
 *   revalidate: bypass cache and force-fetch (safe endpoints only)
 */
async function getCachedData(key, fetchFunc, { ttl = 300, swrTtl = 120, revalidate = false } = {}) {
  try {
    const now = Date.now();
    const cached = cache.get(key);

    // SWR-envelope entries
    if (cached && typeof cached === 'object' && cached.expiresAt !== undefined) {
      if (!revalidate && now < cached.expiresAt) return cached.data;
      // Within SWR window: return stale, kick background refresh
      if (now < cached.expiresAt + swrTtl * 1000) {
        if (!inFlight.has(key)) {
          inFlight.set(key, (async () => {
            try {
              const value = await retryOperation(() => fetchFunc(), { retries: 3 });
              cache.set(key, { data: value, expiresAt: Date.now() + ttl * 1000, swrTtl });
              return value;
            } finally {
              inFlight.delete(key);
            }
        ...
        if (!revalidate) return cached.data;
      }
    } else if (!revalidate && cached !== undefined) {
      // Backwards-compat: legacy primitive cache entry
      return cached;
    }

    // Cache miss (or post-SWR or revalidate) — dedupe concurrent fetches
    ...
    logger.error(`getCachedData error for key=${key}:`, err);
    // Fall back to stale on error if we have any
    const fallback = cache.get(key);
    if (fallback && typeof fallback === 'object' && fallback.data !== undefined) return fallback.data;
