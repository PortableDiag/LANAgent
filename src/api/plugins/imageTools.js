...
export default class ImageToolsPlugin extends BasePlugin {
  constructor(agent) {
    ...
    this.commands = [
      ...
      { command: 'transcode', description: 'Stateless format transcode (avif/png/jpeg/webp) with oversize-passthrough — offloads heavy AVIF encoding', usage: 'transcode({ image: "<base64>", target: "avif", quality: 82, effort: 2, maxPixels: 8000000 })', offerAsService: true }
    ];
  }
  ...
  async execute(params) {
    ...
    switch (action) {
      ...
      case 'transcode': return this._transcode(data);
      default:
        return { success: false, error: `Unknown action: ${action}. Available: optimize, resize, crop, convert, watermark, metadata, transform, transcode` };
    }
  }
  ...
  // Larger-cap buffer source for transcode (full-page screenshots run tens of MB;
  // route enforces the real upload limit, url-fetch covers >100MB sources).
  async _getTranscodeBuffer(params) {
    const CAP = 200 * 1024 * 1024; // 200MB ceiling (route caps multipart lower)
    if (params._buffer && Buffer.isBuffer(params._buffer)) return params._buffer;
    if (params.url) {
      const resp = await axios.get(params.url, {
    ...
    const b64 = params.image || params.base64 || params.imageBase64;
    if (b64) {
      const raw = String(b64).replace(/^data:image\/\w+;base64,/, '');
      const buf = Buffer.from(raw, 'base64');
      if (buf.length > CAP) throw new Error(`Image exceeds ${CAP / 1024 / 1024}MB limit`);
      return buf;
    }
    throw new Error('Provide image (base64), url, or a multipart file');
  }

  /**
   * Stateless format transcode with server-side oversize-passthrough.
   * Returns RAW base64 (no data-URI prefix) + format/dimension metadata so the
   * caller can drop its own metadata() probe. Error objects carry a `code` so the
   * route can map to the right HTTP status (400 vs 500) and refund accordingly.
   */
  async _transcode(params) {
    const VALID = ['avif', 'png', 'jpeg', 'webp'];
    const target = String(params.target || '').toLowerCase();
    if (!VALID.includes(target)) {
      return { success: false, code: 'INVALID_TARGET', error: `Invalid target '${params.target}'. Use: ${VALID.join('|')}` };
    }

    let buf;
    try {
      buf = await this._getTranscodeBuffer(params);
    } catch (e) {
      return { success: false, code: 'BAD_INPUT', error: e.message };
    }

    // Header-only probe (no full decode) for the passthrough decision + metadata.
    let meta;
    try {
      meta = await sharp(buf, { limitInputPixels: false }).metadata();
    } catch (e) {
      return { success: false, code: 'UNDECODABLE', error: `Could not decode image: ${e.message}` };
    }
    const sourceFormat = meta.format || params.sourceFormat || null;
    const width = meta.width || 0;
    const height = meta.height || 0;
    const pixels = width * height;

    // Oversize passthrough: huge already-compressed rasters are returned verbatim
    // (skips the >1GB libvips working set — the whole reason this service exists).
    const maxPixels = Number(params.maxPixels) || 0;
    const COMPRESSED = ['png', 'jpeg', 'webp', 'avif', 'gif', 'tiff'];
    if (maxPixels > 0 && pixels > maxPixels && COMPRESSED.includes(sourceFormat)) {
      // Caller can opt out of getting the bytes back (it already has the source it
      // sent) — keeps a large passthrough from round-tripping through the gateway.
      const includeBytes = !(params.passthroughBytes === false || params.passthroughBytes === 'false');
      return {
        success: true,
        data: {
          ...(includeBytes ? { image: buf.toString('base64') } : {}),
          outputFormat: sourceFormat, passthrough: true,
          sourceFormat, width, height, pixels, bytes: buf.length
        }
      };
    }

    try {
      const quality = params.quality != null ? Math.min(100, Math.max(1, Math.round(params.quality))) : 82;
      ...
      switch (target) {
        ...
        case 'png':  pipeline = pipeline.png({ compressionLevel: 9 }); break; // png is always lossless
      }
      ...
      logger.error(`imageTools transcode encode failed (${target}): ${e.message}`);
      return { success: false, code: 'ENCODE_FAILED', error: e.message };
    }
  }
