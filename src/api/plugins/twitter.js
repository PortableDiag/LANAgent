...
Return ONLY the tweet text, nothing else.`;
      ...
      //
      // Validate-and-retry loop. See mindswarm.js for the full rationale;
      // tl;dr: 10 days of audit showed the AI was reusing 4-word phrases
      // from prior tweets ("scammer addresses on-chain", "SKYNET
      // decentralized marketplace"), opening with banned templates
      // ("Just shipped", "Operating with"), and hallucinating items not in
      // the grounded context (e.g. naming "Gab" as one of 6 AI providers).
      // Three-attempt validate-and-retry with rejected drafts fed back to
      // the AI as DO-NOT-REPEAT examples.
      let tweetText = '';
      ...
      for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
        ...
        const rep = repetitionConflict(draft, context.recentTweetTexts || []);
        if (!rep.ok) {
        ...
        if (attempt > 1) this.logger.info(`Twitter auto-post: accepted on attempt ${attempt}/${MAX_ATTEMPTS}`);
        break;
      }
      ...
      if (!tweetText) {
        ...
        this.logger.info(
          `Twitter auto-post: skipping — all ${MAX_ATTEMPTS} attempts failed validation ` +
  ...
  async _gatherPostContext() {
    ...
    try {
      ...
      const tweetsResForDedup = await this.xApiRequest('GET', `/users/${userIdForDedup}/tweets?max_results=10`);
      recentTweetTexts = (tweetsResForDedup.data || []).slice(0, 8).map(t => t.text || '').filter(Boolean);
    } catch { /* fall through — validator just won't catch overlap with history */ }
