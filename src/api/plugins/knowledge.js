...
export default class KnowledgePlugin extends BasePlugin {
  ...
  async initialize() {
    try {
      ...
      // RAGChain can't function without a vector store; skip its construction
      // instead of letting it throw inside initialize() and surfacing as an
      // opaque error log on every boot.
      if (!vectorStore) {
        this.logger.warn('Knowledge plugin: Vector store not available, RAG features disabled (plugin loads in degraded mode)');
        return;
      }
      ...
      // Inline the message so the log line is actually readable — previously
      // this passed error.message as a second arg and the logger dropped it.
      this.logger.error(`Knowledge plugin initialization error: ${error?.message || error}`);
