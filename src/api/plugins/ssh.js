...
export default class SSHPlugin extends BasePlugin {
  constructor(agent) {
    ...
    // Remote commands like `apt-get update && apt-get install --only-upgrade <pkg>`
    // routinely exceed the default 30s plugin timeout (Telegram "Update prowlarr
    // on Goliath" died silently at 30s on 2026-05-27). Give SSH a 3-minute
    // budget so package upgrades and large rsyncs can complete.
    this.timeout = 180000;
    this.commands = [
      {
        command: 'connect',
  ...
  async execute(params) {
    ...
    this.validateParams(params, {
      action: {
        required: true,
        type: 'string',
        enum: ['connect', 'disconnect', 'execute', 'list-connections', 'save-connection', 'delete-connection', 'test-connection', 'status', 'analytics', 'session-report']
      }
    });
    ...
    switch (action) {
      ...
      case 'analytics':
        return await this.getSessionAnalytics(data);

      case 'session-report':
        return await this.getSessionReport(data);
   ...
   *
   * @param {{ connectionId?: string, id?: string }} data
   * @returns {Promise<{ success: boolean, report?: object, error?: string }>}
   */
  async getSessionReport(data) {
    const id = data?.connectionId || data?.id;
    if (!id) return { success: false, error: 'connectionId is required' };

    const conn = await SSHConnection.findOne({ connectionId: id });
  ...
  /**
   * Filtered session analytics for a saved SSH connection — backs the
   * "GET /api/ssh/connections/:connectionId/analytics" intent from PR #2232.
   * Calls the new `generateFilteredSessionReport` instance method on the
  ...
  async getSessionAnalytics(data) {
    ...
    const conn = await SSHConnection.findOne({ connectionId: id });
    if (!conn) return { success: false, error: 'Connection not found' };
    if (typeof conn.generateFilteredSessionReport !== 'function') {
      return { success: false, error: 'generateFilteredSessionReport not available on SSHConnection model' };
    }

    // Parse window — default to the last 30 days when the caller omits
    // dates entirely, so the action is useful without configuration.
    const now = new Date();
    const defaultStart = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);
    const startDate = data?.startDate ? new Date(data.startDate) : defaultStart;
    const endDate = data?.endDate ? new Date(data.endDate) : now;
    if (isNaN(startDate.getTime()) || isNaN(endDate.getTime())) {
      return { success: false, error: 'Invalid startDate or endDate' };
    }
