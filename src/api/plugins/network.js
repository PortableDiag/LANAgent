...
export default class NetworkPlugin extends BasePlugin {
  ...
  startAutoScanning() {
    ...
    // Run initial scan (match pollDevices pattern: catch transient nmap
    // failures here so they don't escalate to unhandledRejection).
    this.scanNetwork(this.settings.subnets[0] || this.getLocalSubnet())
      .catch(err => logger.warn('Initial network scan failed:', err.message));
    ...
    this.scanInterval = setInterval(() => {
      if (this.settings.enabled) {
        ...
        this.scanNetwork(this.settings.subnets[0] || this.getLocalSubnet())
          .catch(err => logger.warn('Scheduled network scan failed:', err.message));
      }
