...
export default class SystemAdminPlugin extends BasePlugin {
  ...
  async initialize() {
    try {
      ...
      // Agenda jobs are wired in defineSchedulerJobs() instead of here —
      // plugins initialize before the scheduler exists, so registering
      // handlers at this point used to silently bail.
  ...
  /**
   * Framework hook — called by agent.js after this.agent.scheduler is ready.
   * Delegates to the existing scheduleMaintenanceTasks() which checks the
