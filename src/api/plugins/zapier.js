...
export default class ZapierPlugin extends BasePlugin {
  constructor(agent) {
    ...
    this.commands = [
      ...
      {
        command: 'trigger_webhook',
        description: 'POST a payload to a Zap Catch Hook URL (or any webhook URL)',
        usage: 'trigger_webhook({ url: "https://hooks.zapier.com/hooks/catch/...", payload: { ... }, headers: { ... }, method: "POST" })',
        examples: [
  ...
  /**
   * Generic webhook trigger. Posts payload to a Zap Catch Hook URL (or any webhook URL).
   * Doesn't require the Zapier API — direct HTTP POST/PUT/PATCH.
   */
  async triggerWebhook({ url, payload, headers, method }) {
    this.validateParams({ url, headers, method }, {
      url:     { required: true,  type: 'string' },
      headers: { required: false, type: 'object' },
      method:  { required: false, type: 'string', enum: ['POST', 'PUT', 'PATCH'] }
    });

    let requestUrl;
    try { requestUrl = new URL(url); }
    catch { return { success: false, error: 'Invalid URL provided for webhook' }; }
    ...
    try {
      ...
      this.logger.error('triggerWebhook failed:', error);
      return {
        success: false,
        error: error.message,
