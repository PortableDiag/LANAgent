...
export default class MindSwarmPlugin extends BasePlugin {
  ...
  async _engagementCycle() {
    ...
    try {
      ...
      // Also scan recent posts for unreplied replies (MindSwarm may not send
      // reply-type notifications). This sub-scan calls
      //   GET /users/<me>/posts
      //   GET /posts/<id>/replies × up-to-10 posts
      //   GET /posts/<replyId>/replies × N (subreply check)
      // — so at the previous 5-min cadence it was burning ~30 reads per cycle
      // = ~8.6k/day on the bot's own profile. Gate it to once per hour
      // regardless of engagement cycle frequency.
      const REPLY_SCAN_INTERVAL_MS = 60 * 60 * 1000; // 1h
      const nowMs = Date.now();
      const lastScan = this._lastReplyScanMs || 0;
      const dueForScan = (nowMs - lastScan) >= REPLY_SCAN_INTERVAL_MS;
      if (this._engagementConfig.autoReplyToReplies
          && repliesSent < this._engagementConfig.maxRepliesPerCycle
          && dueForScan) {
        try {
          this._lastReplyScanMs = nowMs;
...
Return ONLY the post text, nothing else.`;
      ...
      // reasoning headroom.
      //
      // Validate-and-retry loop. Past behavior: AI would return content
      // that (a) reused a 4-word phrase from a recent post, (b) opened
      // with banned templates ("Just shipped...", "I'm now offering..."),
      // or (c) hallucinated facts not in the items list (e.g. naming
      // "Gab" as one of 6 AI providers). The single-shot generate +
      // length-check let all three through for ~10 days of posts. Now
      // we re-generate up to 3 times with the rejected output included
      // as a "DO NOT REPEAT" example.
      let postContent = '';
      ...
      for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
        const augmentedPrompt = rejectedDrafts.length === 0 ? prompt :
          prompt + `\n\nDRAFTS YOU ALREADY GENERATED THAT WERE REJECTED — pick a DIFFERENT angle, do NOT echo any 4-word phrase from these, do NOT use these openings:\n` +
          rejectedDrafts.map((d, i) => `[REJECTED-${i + 1}, reason=${d.reason}]: ${d.text}`).join('\n');

        lastResult = await this.agent.providerManager.generateResponse(augmentedPrompt, {
          temperature: 0.85, maxTokens: 1500
        });
        const draft = (lastResult?.content || lastResult?.text || '').toString().trim()
          .replace(/^["']|["']$/g, '');
        ...
        const badOpener = isBadOpener(draft);
        if (badOpener) {
          validationFailure = `bad_opener:${badOpener}`;
          rejectedDrafts.push({ text: draft.slice(0, 200), reason: validationFailure });
          continue;
        }

        const rep = repetitionConflict(draft, context.recentPostTexts || []);
        if (!rep.ok) {
          validationFailure = `repetition:"${rep.sharedNgrams.slice(0, 2).join('","')}"_vs_"${rep.conflict.slice(0, 50)}"`;
          rejectedDrafts.push({ text: draft.slice(0, 200), reason: `repetition:[${rep.sharedNgrams.slice(0, 2).join('|')}]` });
          continue;
        }

        const ground = groundingAnchor(draft, context.itemTexts || []);
        if (!ground.ok) {
          validationFailure = 'no_grounding_anchor';
          rejectedDrafts.push({ text: draft.slice(0, 200), reason: 'no_anchor_from_items' });
          continue;
        }

        postContent = draft;
        validationFailure = null;
        if (attempt > 1) this.pluginLogger.info(`Auto-post: accepted on attempt ${attempt}/${MAX_ATTEMPTS}`);
        break;
      }

      if (!postContent) {
        // All retries failed. Log what was rejected and why so the next
        // pass at this code knows what shape of content keeps slipping.
        let resultDump = '(no result)';
        try {
          resultDump = JSON.stringify(lastResult, (k, v) => {
            if (typeof v === 'string' && v.length > 200) return v.slice(0, 200) + '...';
            return v;
          }).slice(0, 1000);
        } catch (e) { resultDump = `(dump err: ${e.message})`; }
        this.pluginLogger.info(
          `Auto-post: skipping — all ${MAX_ATTEMPTS} attempts failed validation ` +
          `(last=${validationFailure}, ` +
          `drafts=${rejectedDrafts.map(d => `[${d.reason}]"${d.text.slice(0, 60)}…"`).join(' || ')}, ` +
          `raw=${resultDump})`
        );
        return;
      }
  ...
  async _gatherPostContext() {
    ...
    try {
      const postsResult = await this._apiRequest('get', `/users/${this.username}/posts`, { page: 1 });
      recentPostTexts = (postsResult.data?.posts || []).slice(0, 8).map(p => p.content || '').filter(Boolean);
    } catch { /* dedup will fall through gracefully */ }
...
Return ONLY valid JSON.`;
  ...
  async _apiRequest(method, path, data = null, authenticated = true) {
    ...
    return await retryOperation(async () => {
      try {
        ...
        // 403 banned/suspended is permanent — dropping tokens and disabling
        // engagement so we stop hammering the API forever. (Email-verification
        // 403s are write-only, so they won't hit the polling read path; if
        // they ever do, the same disable-and-stop applies.)
        if (status === 403 && authenticated && /banned|suspended|email[ _]verif/i.test(apiError)) {
          this.pluginLogger.error(`MindSwarm 403 (${apiError}) — dropping tokens, disabling engagement, will not retry`);
          this.accessToken = null;
          this.refreshToken = null;
          if (this._engagementConfig) this._engagementConfig.enabled = false;
          try { await this._saveTokens(); } catch { /* persist failure non-fatal */ }
          const permanent = new Error(`MindSwarm account ${apiError}`);
        ...
        // The shared refresh promise (_getRefreshPromise) coalesces concurrent
        // 401s into a single /auth/refresh call so MindSwarm's single-use
        // refresh-token replay detection doesn't wipe our session.
        if (status === 401 && authenticated && !refreshed) {
          refreshed = true;
          if (this.refreshToken) {
            try {
              this.pluginLogger.info('Access token expired, refreshing (serialized)...');
              await this._getRefreshPromise();
      ...
      // Permanent errors (banned/suspended 403, normalized non-axios errors)
      // should never be retried — defer to default classifier otherwise.
      customErrorClassifier: (err) => {
  ...
  async _register(data = {}) {
    ...
    // First post — say hi so the social-network operator can see new agents arrive.
    // Deliberately PRIVACY-SAFE: only the agent name + host OS family + a tag. No
    // IP, no email, no hostname, no plugin list, no credentials.
    await this._postFirstHello();

    return result;
  }

  /**
   * One-time "hello, I just joined" post fired only on fresh registration.
   * Skipped on subsequent logins (auto-register only runs when we have no
   * saved credentials).
   */
  async _postFirstHello() {
    try {
      const agentName = this.agent.config?.name || process.env.AGENT_NAME || 'an autonomous agent';
      const osFamily = process.platform === 'linux' ? 'a Linux system'
        : process.platform === 'darwin' ? 'a macOS system'
        : process.platform === 'win32' ? 'a Windows system'
        : 'a server';

      // Try to generate something a little more interesting via AI, fall back
      // to a fixed template. The AI prompt explicitly forbids personal info.
      let content = `Hi MindSwarm 👋 I'm ${agentName}, a new LANAgent running on ${osFamily}. Looking forward to seeing what this network is about. #lanagent #firstpost`;
      if (this.agent.providerManager) {
        try {
          const result = await this.processWithAI(
            `You are an autonomous AI agent named "${agentName}" running on ${osFamily}. ` +
            `Write a short, friendly first post (max 220 chars) introducing yourself to the MindSwarm social network. ` +
            `MUST include the agent name and OS family. MUST include hashtags #lanagent and #firstpost. ` +
            `DO NOT mention: your operator, IP address, hostname, email, credentials, plugin list, system specs, or any personal info. ` +
            `Return only the post text — no quotes, no preamble.`
          );
          const generated = (result?.content || result?.text || '').toString().trim();
          if (generated.length >= 20 && generated.length <= 280 && /lanagent/i.test(generated)) {
            content = generated;
          }
        } catch { /* keep template */ }
      }

      await this._apiRequest('post', '/posts', { content, isAiGenerated: true });
      this.pluginLogger.info(`Posted first-hello to MindSwarm`);
    } catch (err) {
      // Never fail registration over a missing hello post — log and move on.
      this.pluginLogger.warn(`First-hello post failed (non-fatal): ${err?.message || err}`);
    }
  }
  ...
  /**
   * Serialize concurrent refresh attempts behind a single in-flight promise.
   * MindSwarm uses single-use refresh tokens with replay detection (since
   * 2026-05-09 — see backend auth notes). When two parallel pollers both
   * 401 at the same minute mark, each calling _refreshTokens() races the
   * other's token rotation; whoever loses gets a 'token reused' error and
   * the *winning* refresh's new tokens get clobbered. Result: full re-
   * login on every cycle, ~49/161 /notifications calls 4xx'd.
   *
   * Mirrors the pattern in the MindSwarm web frontend
   * (frontend/src/lib/api.ts:46-59): all callers await the same promise,
   * which is cleared only after success/failure resolves.
   */
  async _getRefreshPromise() {
