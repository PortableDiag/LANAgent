...
export default class JournalPlugin extends BasePlugin {
  ...
  async stopSession(data) {
    ...
    if (fullText.length > 20 && this.agent.providerManager) {
      try {
        ...
        summary = this.buildDeterministicSummary(journal, fullText);
      }
    } else {
      summary = this.buildDeterministicSummary(journal, fullText);
    }
  ...
  /**
   * Build an honest, deterministic session summary when AI is unavailable.
   * This is NOT an AI summary — it is a stats line plus the first and last
   * sentences of the session so the reader gets real anchors without any
   * fabricated synthesis.
   *
   * @param {Object} journal - The journal document being closed
   * @param {string} [fullText] - Pre-computed full text (defaults to journal.getFullText())
   * @returns {string} A deterministic summary string
   */
  buildDeterministicSummary(journal, fullText = journal.getFullText()) {
    const text = (fullText || '').trim();
    const entryCount = journal.entries?.length || 0;
    const wordCount = journal.metadata?.totalWordCount
      || (text ? text.split(/\s+/).filter(Boolean).length : 0);

    if (!text) {
      return `Auto-summary: ${entryCount} ${entryCount === 1 ? 'entry' : 'entries'} recorded.`;
    }
    ...
    const stats = `Auto-summary: ${entryCount} ${entryCount === 1 ? 'entry' : 'entries'}, ${wordCount} words`
      + (journal.tags?.length ? `, tags: ${journal.tags.join(', ')}` : '')
      + '.';
    ...
    const excerpt = last && last !== first
      ? `Opened with: "${first}" Closed with: "${last}"`
      : `Opened with: "${first}"`;
