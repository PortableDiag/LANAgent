...
// Per-network RPC fallback lists. Public endpoints rate-limit aggressively;
// we rotate through a small pool and prefer env-configured private RPCs first.
...
export default class WalletProfilerPlugin extends BasePlugin {
    ...
    _getProvider(networkCfg, rpcIndex = 0) {
        const { ethers } = ethersLib || {};
        if (!ethers) throw new Error('ethers not loaded');
        const url = networkCfg.rpcUrls[rpcIndex];
        if (!url) throw new Error(`No RPC URL available at index ${rpcIndex} for ${networkCfg.nativeSymbol}`);
        // Pin staticNetwork + disable batch to avoid the "id: null" rate-limit response bug
        const staticNetwork = ethers.Network.from(networkCfg.chainId);
        return new ethers.JsonRpcProvider(url, staticNetwork, {
            staticNetwork: true,
            batchMaxCount: 1
        });
    }
    ...
    /**
     * Run an RPC operation against a provider, rotating to the next RPC URL on
     * transient errors (429, timeouts, 5xx). Returns the operation result.
     */
    async _withProviderFallback(networkCfg, operation) {
        const urls = networkCfg.rpcUrls;
        let lastErr;
        for (let i = 0; i < urls.length; i++) {
            const provider = this._getProvider(networkCfg, i);
            try {
                return await operation(provider);
            } catch (err) {
                lastErr = err;
                if (!isTransientRpcError(err)) throw err;
                this.logger.warn(`RPC ${urls[i]} failed (${err.message?.slice(0, 80)}); trying next`);
            }
        }
        throw lastErr || new Error('All RPC endpoints exhausted');
    }

    async _getExplorerKey(networkCfg) {
        const cacheKey = `apikey:${networkCfg.chainId}`;
        const hit = apiKeyCache.get(cacheKey);
        if (hit !== undefined) return hit;

        let key = '';
        try {
            const stored = await PluginSettings.getCached('crypto', 'explorer_api_keys');
            // Single Etherscan V2 key works for all chains; check the chain's slot
            // first, then fall back to any stored key.
            const candidate = stored?.[networkCfg.explorerKeySlot] || stored?.eth || stored?.bsc;
            if (candidate) key = decrypt(candidate) || '';
        } catch (err) {
            this.logger.debug(`PluginSettings explorer_api_keys lookup failed: ${err.message}`);
        }
        if (!key) {
            key = process.env.ETHERSCAN_API_KEY ||
                  (networkCfg.legacyEnvKey && process.env[networkCfg.legacyEnvKey]) ||
                  '';
        }
    ...
    async _explorerGet(networkCfg, queryParams) {
        const apiKey = await this._getExplorerKey(networkCfg);
        const params = { chainid: networkCfg.chainId, ...queryParams };
        if (apiKey) params.apikey = apiKey;

        try {
            const { data } = await axios.get(ETHERSCAN_V2_URL, { params, timeout: 15000 });
            // V2 returns {status:"0", message:"NOTOK", result:"..."} for missing key,
            // {status:"0", message:"No transactions found", result:[]} for empty results.
            // Surface the missing-key case as null so callers can fall through gracefully.
            if (data?.status === '0' && !apiKey && /api key|apikey|missing|invalid/i.test(String(data.result))) {
                this.logger.warn(`Etherscan V2 rejected request — no API key configured (set ETHERSCAN_API_KEY)`);
                return null;
            }
            return data;
        } catch (error) {
    ...
    async profile(data) {
        ...
        // Fetch in parallel — RPC calls go through the fallback wrapper so a
        // rate-limited node rotates instead of failing the whole profile.
        const [balanceWei, isContract, txHistory, goPlusData] = await Promise.all([
            this._withProviderFallback(networkCfg, p => p.getBalance(address)),
            this._isContract(networkCfg, address),
    ...
    async tokens(data) {
        ...
        const balanceResults = await Promise.allSettled(
            tokenEntries.map(async (token) => {
                try {
                    const balance = await this._withProviderFallback(networkCfg, (p) => {
                        const contract = new ethers.Contract(token.contractAddress, erc20Abi, p);
