...
export default class ProwlarrPlugin extends ArrBasePlugin {
  constructor(agent) {
    super(agent, {
      ...
      commands: [
        ...
        {
          command: 'search_advanced',
          description: 'Advanced, type-aware search with media identifiers, pagination, and sorting',
          usage: 'search_advanced({ type: "movie|tv|music|book|search", query, imdbId, tmdbId, tvdbId, season, episode, categories, indexerIds, limit, offset, sortBy })',
          examples: [
            'prowlarr advanced search Inception by imdb id tt1375666',
            'prowlarr tv search S01E02 of The Office',
            'paged prowlarr search offset 50 limit 25',
            'music search Daft Punk categories 3000,3010'
          ]
        },
        {
  ...
  async execute(params) {
    return this._executeAction(params, {
      ...
      search_advanced: this.searchAdvanced,
  ...
  /**
   * Type-aware Prowlarr search with media identifiers, pagination, and sorting.
   * Maps Prowlarr/Jackett t= values: search | movie | tvsearch | music | book.
   */
  async searchAdvanced(data = {}) {
    const allowedTypes = ['search', 'movie', 'tvsearch', 'music', 'book'];
    let type = (data.type || 'search').toString().toLowerCase();
    if (!allowedTypes.includes(type)) {
      if (type === 'tv' || type === 'tvshows') type = 'tvsearch';
      else if (type === 'movies') type = 'movie';
      else return { success: false, error: `Invalid type "${data.type}". Allowed: ${allowedTypes.join(', ')}` };
    }

    const params = { type };
    if (data.query) params.query = data.query;
    if (data.imdbId) params.imdbId = data.imdbId;
    if (data.tmdbId) params.tmdbId = data.tmdbId;
    if (data.tvdbId) params.tvdbId = data.tvdbId;
    if (data.season != null) params.season = data.season;
    if (data.episode != null) params.episode = data.episode;
    if (data.categories) params.categories = Array.isArray(data.categories) ? data.categories : [data.categories];
    if (data.indexerIds) params.indexerIds = Array.isArray(data.indexerIds) ? data.indexerIds : [data.indexerIds];

    const limit = Number.isFinite(data.limit) ? Math.max(0, Number(data.limit)) : undefined;
    const offset = Number.isFinite(data.offset) ? Math.max(0, Number(data.offset)) : 0;
    if (limit != null) params.limit = limit;
    if (offset) params.offset = offset;

    const sortBy = data.sortBy && ['seeders', 'size', 'publishDate'].includes(data.sortBy) ? data.sortBy : null;

    const allResults = await this._apiRequest('get', '/search', null, params);
    const resultsArray = Array.isArray(allResults) ? allResults : [];
    if (resultsArray.length === 0) return { success: false, error: 'No results found' };

    let sorted = resultsArray.slice();
    if (sortBy) {
      sorted.sort((a, b) => {
        const av = a?.[sortBy]; const bv = b?.[sortBy];
        if (av == null && bv == null) return 0;
        if (av == null) return 1; if (bv == null) return -1;
        if (sortBy === 'publishDate') return (new Date(bv).getTime() || 0) - (new Date(av).getTime() || 0);
        return (Number(bv) || 0) - (Number(av) || 0);
      });
    }

    // Client-side pagination — honours limit/offset even when API ignores them
    const total = sorted.length;
    const pageOffset = offset || 0;
    const pageLimit = limit != null ? limit : 25;
    const paged = sorted.slice(pageOffset, pageOffset + pageLimit);

    const formatted = paged.map(r => ({
      title: r.title, indexer: r.indexer, size: r.size,
      sizeFormatted: r.size ? `${(r.size / 1073741824).toFixed(2)} GB` : 'unknown',
      seeders: r.seeders, leechers: r.leechers, publishDate: r.publishDate,
      categories: r.categories?.map(c => c.name), downloadUrl: r.downloadUrl, infoUrl: r.infoUrl
    }));

    const preview = paged.slice(0, Math.min(15, paged.length)).map((r, i) => {
      const size = r.size ? `${(r.size / 1073741824).toFixed(1)}GB` : '?';
      const seeds = r.seeders != null ? `S:${r.seeders}` : '';
      return `${i + 1 + pageOffset}. ${r.title} [${r.indexer}] ${size} ${seeds}`;
    }).join('\n');

    return {
      success: true, total, offset: pageOffset, limit: pageLimit, sortBy: sortBy || undefined,
      results: formatted,
      result: `Found ${total} result(s)${sortBy ? ` sorted by ${sortBy}` : ''}${pageOffset || pageLimit ? ` — showing ${paged.length} (offset ${pageOffset}, limit ${pageLimit})` : ''}${paged.length ? `:\n${preview}${(pageOffset + pageLimit) < total ? `\n... and ${Math.max(0, total - (pageOffset + pageLimit))} more` : ''}` : ''}`
    };
  }
