...
export default class CryptoMonitorPlugin extends BasePlugin {
    ...
    async checkBalances() {
        try {
            ...
            for (const addr of wallet.addresses) {
                ...
                // RPC-flap guard: when every fallback RPC fails, getAddressBalance
                // returns null (was previously '0', which was indistinguishable
                // from a real zero balance). Skip cache update + notifications +
                // alerts on null — pretending unknown == 0 produced spurious
                // "balance went from 28 to 0" Telegram alerts whenever the
                // public RPC providers were rate-limited or having a moment,
                // followed by "0 to 28" once they recovered. Pure noise.
                if (balance === null) {
                    continue;
                }
                ...
                // Zero-debounce: a synced-but-stale RPC sometimes successfully
                // returns 0 for a non-empty wallet (no error thrown — bypasses
                // the null guard above). Require 2 consecutive 0 reads before
                // believing a non-zero → 0 transition. Real drains get alerted
                // one cycle late; transient RPC lies are filtered out.
                const prevNonZero = previousBalance && Number(previousBalance.balance) > 0;
                const nowZero = Number(balance) === 0;
                if (prevNonZero && nowZero) {
                    const zeroStreak = (previousBalance.zeroStreak || 0) + 1;
                    if (zeroStreak < 2) {
                        // Don't overwrite cached balance; bump the streak counter
                        // so the next cycle either confirms (streak=2 → real)
                        // or resets (non-zero read).
                        this.balanceCache.set(cacheKey, {
                            balance: previousBalance.balance,
                            timestamp: previousBalance.timestamp,
                            zeroStreak
                        });
                        this.logger.warn(`Suspicious 0 balance for ${addr.address} on ${network} (cached ${previousBalance.balance}); awaiting confirmation next cycle.`);
                        continue;
                    }
                }
    ...
    async getAddressBalance(address, network) {
        try {
            ...
            // Return null (not '0') when every fallback RPC has failed. Callers
            // need to distinguish "balance unknown" from "balance is actually
            // zero". Conflating them caused spurious balance-change Telegram
            // alerts during normal RPC flap.
