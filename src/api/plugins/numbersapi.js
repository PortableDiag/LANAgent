...
export default class NumbersapiPlugin extends BasePlugin {
  constructor(agent) {
    ...
    // Upstream-health flags — used to keep the plugin loaded even when
    // numbersapi.com is degraded (it returns 404 for every endpoint as of
    // 2026-06). Read by makeRequest() to short-circuit doomed network
    // calls instead of taking the full 3-retry path on every request.
    // _upstreamAvailable=true is the optimistic default; flipped to false
    // by testConnection() failures, flipped back to true the next time a
    // probe succeeds (opportunistic recovery on cache miss).
    this._upstreamAvailable = true;
    this._upstreamError = null;
    this._upstreamLastCheckedAt = 0;
    this._upstreamRecheckCooldownMs = 10 * 60 * 1000; // 10m between retries when down
  }
  ...
  async initialize() {
    ...
    try {
      ...
      // Probe the upstream. On failure, throw an UPSTREAM_UNAVAILABLE
      // error — apiManager.loadPlugin recognizes that shape and registers
      // the plugin as disabled (same path as missing-credentials), rather
      // than treating it as a load crash. Avoids the boot-time error spam
      // we hit when numbersapi.com went 404-for-everything in 2026-06.
      try {
        await this.testConnection();
        this._upstreamAvailable = true;
        this._upstreamError = null;
      } catch (probeErr) {
        this._upstreamAvailable = false;
        this._upstreamError = probeErr.message;
        this._upstreamLastCheckedAt = Date.now();
        const e = new Error(`Numbers API upstream unavailable (${probeErr.message})`);
        e.code = 'UPSTREAM_UNAVAILABLE';
        e.upstream = 'numbersapi.com';
        throw e;
      }
      ...
      // Two "expected failure" shapes that should NOT log as ERROR — both
      // should hit apiManager's disable-and-register-anyway path:
      //   1) Missing credentials (legacy pattern, kept for compatibility)
      //   2) UPSTREAM_UNAVAILABLE — external service returned a clear
      //      "I'm not working" signal (e.g. 404, 5xx, connection refused)
      const isCredentialError = error && error.message && (
      ...
      const isUpstreamError = error && error.code === 'UPSTREAM_UNAVAILABLE';
      if (isCredentialError || isUpstreamError) {
  ...
  async makeRequest(endpoint, retries = 0) {
    // Short-circuit known-bad upstream — avoids the 3x retry × 5s timeout
    // path (15s of dead-air per call) and keeps the error message
    // consistent so callers can match on it.
    //
    // Opportunistic recovery: after the cooldown window elapses, allow ONE
    // attempt through to probe whether the upstream has come back. A
    // successful response flips the flag back to available; another
    // failure refreshes the cooldown.
    if (!this._upstreamAvailable) {
      const now = Date.now();
      const sinceLastCheck = now - this._upstreamLastCheckedAt;
      if (sinceLastCheck < this._upstreamRecheckCooldownMs) {
        const err = new Error(
          `numbersapi upstream unavailable (${this._upstreamError || 'last probe failed'}); ` +
          `next recovery probe in ${Math.ceil((this._upstreamRecheckCooldownMs - sinceLastCheck) / 1000)}s`
        );
        err.code = 'UPSTREAM_UNAVAILABLE';
        err.upstream = 'numbersapi.com';
        throw err;
      }
      // Cooldown elapsed — fall through and attempt one real probe.
      this._upstreamLastCheckedAt = now;
    }
    ...
    try {
      ...
      // Recovery: a successful response when we previously thought we
      // were down means the upstream is back. Flip the flag and clear
      // the error message so the next call goes through the fast path.
      if (!this._upstreamAvailable) {
        this._upstreamAvailable = true;
        this._upstreamError = null;
        this.logger.info(`${this.name} upstream recovered`);
      }
      ...
      // All retries exhausted. Mark upstream as unavailable so subsequent
      // calls short-circuit instead of repeating the same retry storm.
      this._upstreamAvailable = false;
      this._upstreamLastCheckedAt = Date.now();
      let msg;
      if (error.response) {
        msg = `API returned ${error.response.status}: ${error.response.statusText}`;
      } else if (error.request) {
        msg = 'No response from Numbers API';
      } else {
        msg = error.message;
      }
      this._upstreamError = msg;
      throw new Error(msg);
    }
  }
