...
export default class PRReviewerPlugin extends BasePlugin {
  ...
  async initialize() {
    ...
    // Agenda handler registration is deferred to defineSchedulerJobs(),
    // which the agent calls AFTER the scheduler is up. Plugins are
    // initialized before the scheduler exists, so trying to register
    // handlers here would silently bail.
  }

  /**
   * Framework hook — called by agent.js after this.agent.scheduler is up.
   * Registers handlers and recurring schedules for both review-prs and
   * update-pr-review-models. Handlers run unconditionally; the enabled
   * check happens inside each handler so the operator can toggle via UI
   * without a restart. Pre-existing rows are cancelled first to avoid
   * stale repeatInterval / orphaned nextRunAt from older codebases.
   */
  async defineSchedulerJobs() {
    if (!this.agent.scheduler?.agenda) {
      logger.warn('[PRReviewer] defineSchedulerJobs called but agenda is unavailable');
      return;
    }
    const agenda = this.agent.scheduler.agenda;
    const settings = await this.getSettings();

    await agenda.cancel({ name: 'review-prs' });
    await agenda.cancel({ name: 'update-pr-review-models' });

    agenda.define('review-prs', async () => {
      const live = await this.getSettings();
      if (!live.enabled) {
        logger.debug('[PRReviewer] review-prs tick: disabled in settings, skipping');
        return;
      }
    ...
    agenda.define('update-pr-review-models', async () => {
      const live = await this.getSettings();
      if (!live.enabled) {
        logger.debug('[PRReviewer] update-pr-review-models tick: disabled in settings, skipping');
        return;
      }
    ...
    await agenda.every(settings.schedule || '0 */6 * * *', 'review-prs');
    await agenda.every('0 2 * * *', 'update-pr-review-models');
    logger.info(`[PRReviewer] Schedules registered (review-prs: ${settings.schedule || '0 */6 * * *'}, model-update: 0 2 * * *; enabled=${settings.enabled})`);
  }
