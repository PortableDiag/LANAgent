...
const apiKeyCache = new NodeCache({ stdTTL: 300 }); // 5 min — refreshed if user updates the stored key
...
// ── Explorer config ─────────────────────────────────────────────────────────
//
// Etherscan migrated to a unified V2 API in 2025. The old per-chain hosts
// (api.bscscan.com, api.polygonscan.com, etc.) are deprecated and return
// {status:"0", message:"NOTOK"} pointing at the migration doc — even with a
// valid key. V2 takes a single ETHERSCAN_API_KEY plus a `chainid` parameter.
const ETHERSCAN_V2_URL = 'https://api.etherscan.io/v2/api';

const EXPLORER_CONFIG = {
  bsc:      { chainId: 56, envKey: 'ETHERSCAN_API_KEY', legacyEnvKey: 'BSCSCAN_API_KEY',  signupUrl: 'etherscan.io' },
  eth:      { chainId: 1,  envKey: 'ETHERSCAN_API_KEY', signupUrl: 'etherscan.io' },
  ethereum: { chainId: 1,  envKey: 'ETHERSCAN_API_KEY', signupUrl: 'etherscan.io' }
};
...
export default class ContractAuditPlugin extends BasePlugin {
  ...
  /**
   * Look up the explorer API key. Checks (in order): in-process cache,
   * PluginSettings DB entry written by the crypto plugin's UI, env var,
   * legacy per-chain env var (BSCSCAN_API_KEY etc.) for backwards compat.
   * Returns '' if none configured.
   */
  async _getExplorerKey(network) {
    const cacheKey = `apikey:${network}`;
    ...
    try {
      ...
      // PluginSettings stores per-chain slots; etherscan key works for all
      // chains under V2, so fall back across slots if the requested one is missing.
      const slot = network === 'bsc' ? 'bsc' : 'eth';
      const candidate = stored?.[slot] || stored?.eth || stored?.bsc;
      if (candidate) {
        key = decrypt(candidate) || '';
      }
    } catch (err) {
    ...
    if (!key) {
      ...
      key = (cfg?.envKey && process.env[cfg.envKey]) ||
            (cfg?.legacyEnvKey && process.env[cfg.legacyEnvKey]) ||
            '';
    }
  ...
  async _fetchFromExplorer(address, network) {
    ...
    const apiKey = await this._getExplorerKey(network.toLowerCase());
    ...
    const url = `${ETHERSCAN_V2_URL}?${params.toString()}`;

    const noKeyHint = ` — no explorer API key configured. Set ${config.envKey} in .env or save one via the crypto plugin settings (free key at ${config.signupUrl}).`;

    const response = await fetch(url);
    if (!response.ok) {
      throw new Error(`Explorer API returned HTTP ${response.status}${!apiKey ? noKeyHint : ''}`);
    }

    const json = await response.json();

    if (json.status !== '1' || !json.result || !json.result[0]) {
      const upstream = (typeof json.result === 'string' ? json.result : null) || json.message || 'Unknown error';
      throw new Error(`Could not fetch source from explorer: ${upstream}${!apiKey ? noKeyHint : ''}`);
    }
