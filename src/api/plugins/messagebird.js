...
export default class MessageBirdPlugin extends BasePlugin {
  constructor(agent) {
    ...
    this.commands = [
      ...
      {
        command: 'getMessageStatus',
        description: 'Get delivery status of a sent message',
        usage: 'getMessageStatus({ messageId: "<id>" })',
        examples: [
          'check status for message 1234567890',
          'what is the delivery status of my last SMS',
          'get delivery details for message id abcdef'
        ]
      }
    ];
    ...
    try {
      switch (action) {
        ...
        case 'getMessageStatus':
          return await this.getMessageStatus(data);
  ...
  /**
   * Retrieve delivery status and metadata for a specific message via
   * GET /messages/{id}.
   */
  async getMessageStatus({ messageId }) {
    this.validateParams({ messageId }, {
      messageId: { required: true, type: 'string' }
    });

    const url = `${this.config.baseUrl}/messages/${encodeURIComponent(messageId)}`;
    const headers = { Authorization: `AccessKey ${this.config.apiKey}` };

    try {
      const response = await retryOperation(() => axios.get(url, { headers }), { retries: 3, context: 'getMessageStatus' });
      return { success: true, data: response.data };
    } catch (error) {
      this.logger.error('getMessageStatus failed:', error);
      return { success: false, error: error.message };
    }
  }
