...
export default class FreshpingPlugin extends BasePlugin {
  constructor(agent) {
    ...
    this.commands = [
      ...
      {
        command: 'searchchecks',
        description: 'Search checks by name/url/status',
        usage: 'searchchecks({ q: "my-site", status: "up|down|unknown" })'
      }
    ];
  ...
  async execute(params) {
    ...
    try {
      switch(action) {
        ...
        case 'searchchecks':
          return await this.searchChecks(params);
  ...
  /**
   * Search checks client-side by name, URL substring, and/or status
   * @param {Object} params - Parameters for searching checks
   * @param {string} [params.q] - Query substring to match against name or url
   * @param {string} [params.status] - Filter by check_state (e.g., 'up', 'down', 'unknown')
   * @returns {Promise<Object>} Filtered results with counts
   */
  async searchChecks(params = {}) {
    const { q, status } = params || {};
    const all = await this.getChecks();
    if (!all.success) {
      return all;
    }

    // Normalize query for case-insensitive search
    const query = typeof q === 'string' && q.length > 0 ? q.toLowerCase() : null;
    const statusFilter = typeof status === 'string' && status.length > 0 ? status : null;

    const filtered = (all.data || []).filter((check) => {
      const name = (check && typeof check.name === 'string') ? check.name : '';
      const url = (check && typeof check.url === 'string') ? check.url : '';
      const state = check && check.check_state;

      const matchesQuery = query
        ? (name.toLowerCase().includes(query) || url.toLowerCase().includes(query))
        : true;
