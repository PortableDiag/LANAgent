...
export default class MonitoringPlugin extends BasePlugin {
    ...
    try {
      ...
      return {
        ...
        performance: {
          ...
          memory: (() => {
            const actuallyUsed = typeof mem.available === 'number'
              ? mem.total - mem.available
              : (mem.active ?? mem.used);
            return {
              used: actuallyUsed,
              total: mem.total,
              percent: (actuallyUsed / mem.total * 100).toFixed(2),
  ...
  async sendTemperatureAlert(temperature, level) {
    ...
    // Send notification. notify() degrades gracefully (no throw) when no
    // channel is configured — only log delivery when it actually went out.
    try {
      const result = await this.notify(message);
      if (result?.delivered === false) {
        logger.debug(`Temperature alert not delivered (${result.reason}): ${message}`);
      } else {
