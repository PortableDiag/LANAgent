    ...
    try {
        ...
        if (pnl) {
            ...
            if (pnl.lifetimeRealized !== undefined) {
                // getTokenTraderStatus() reports `lifetimeRealized = lifetimeRealizedPnL + realizedPnL`,
                // so writing back the displayed value naively doubles it. Subtract the current realized
                // so that round-tripping the status response is a no-op.
                tokenStrategy.state.lifetimeRealizedPnL = pnl.lifetimeRealized - (pnl.realized || 0);
            }
        }
        ...
        const { circuitBreaker, cooldowns, priceHistory } = req.body;
        if (circuitBreaker) {
        ...
        // Cooldown timestamps gate re-entry after exits — exposing them lets a caller
        // wipe the slate clean for stress-test runs that need an immediate buy.
        // Pass any field as `null` to clear it. Caller-of-truth for the wire format:
        // { cooldowns: { lastEmergencySell: null, lastDowntrendExit: null, lastDipBuy: null,
        //                lastGridTrade: null, lastPumpStallSell: null, consecutiveGridBuys: 0 } }
        if (cooldowns) {
            const COOLDOWN_FIELDS = [
                'lastEmergencySell', 'lastDowntrendExit', 'lastDipBuy',
                'lastGridTrade', 'lastPumpStallSell', 'consecutiveGridBuys'
            ];
            for (const k of COOLDOWN_FIELDS) {
                if (cooldowns[k] !== undefined) tokenStrategy.state[k] = cooldowns[k];
            }
        }

        // `priceHistory: 'clear'` wipes the rolling price buffer so the trend gates
        // (_calculateTrend / _calculateLongTermTrend) start fresh from the next tick.
        // Useful after a major dump — old highs at $1.15 will keep the long-trend
        // negative for hours and block any grid/DCA buys.
        if (priceHistory === 'clear') {
            tokenStrategy.state.tokenPriceHistory = [];
        }
