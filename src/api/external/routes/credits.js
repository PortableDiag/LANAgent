...
// Internal wallets — gateway-owned funding wallets that auto-replenish their
// own ALICE credit balance via on-chain SKYNET transfers. Their purchases are
// infrastructure churn, not customer revenue, and shouldn't generate a
// Telegram alert (otherwise the user gets a noise notification per scrape
// burst). Configured via the INTERNAL_WALLETS env var as comma-separated
// lowercase addresses. Match is case-insensitive.
const INTERNAL_WALLETS = new Set(
  (process.env.INTERNAL_WALLETS || '')
    .split(',')
...
// SKYNET token on BSC
const SKYNET_TOKEN = '0x8b77CC5c6cB3d846608d9d5Dd03fA406BA03b8F1';
...
router.post('/purchase', creditAuth(true), async (req, res) => {
  try {
    ...
    // Notify owner — skip when the buyer is an internal/gateway wallet (the
    // gateway auto-replenishes its own credit balance and would otherwise
    // spam alerts for every infrastructure top-up).
    try {
      const buyerIsInternal = INTERNAL_WALLETS.has((req.wallet || '').toLowerCase());
      const agent = req.app?.locals?.agent;
      if (agent?.notify && !buyerIsInternal) {
      ...
      } else if (buyerIsInternal) {
        logger.info(`Internal credit purchase by ${req.wallet} (${credits} credits) — Telegram notification suppressed`);
      }
    } catch {
