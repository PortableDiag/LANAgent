...
// Inline IPv6-safe bucket key: trim IPv6 addresses to their /64 prefix so
// a single user can't rotate suffix bits to dodge the limit. This express-
// rate-limit build doesn't ship a named `ipKeyGenerator` export.
const ipBucket = (ip) => {
...
import imageTranscodeRoutes from './routes/imageTranscode.js';
...
// Per-API-key bucket so paid customers don't share the rate-limit budget
// with each other (and aren't all bottlenecked through the gateway VPN's
// single IP). Unauthenticated traffic still gets the lower IP-based cap as
// an anti-abuse floor.
//
// Sizing rationale: a single Puppeteer/FlareSolverr browser-tier scrape
// takes 2-15s. With pipelined work an honest client peaks at ~5 req/s
// sustained. 5000/15min = 5.56 req/s allows that comfortably while still
// catching truly runaway clients before they exhaust the agent.
const externalLimiter = rateLimit({
  windowMs: 15 * 60 * 1000,
  max: (req) => req.headers['x-api-key'] ? 5000 : 100,
  keyGenerator: (req) => {
    const apiKey = req.headers['x-api-key'];
    if (apiKey) return `key:${apiKey}`;
    return `ip:${ipBucket(req.ip)}`;
  },
  standardHeaders: true,    // X-RateLimit-Remaining / -Reset so clients self-throttle
  legacyHeaders: false,
  message: { success: false, error: 'Too many requests — slow down or contact support to raise your per-key limit' }
});
...
router.use('/service/imageTools/transcode', imageTranscodeRoutes); // dedicated multipart route — must precede the generic proxy
