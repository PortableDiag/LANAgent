...
export class IntentIndexer {
  ...
  /**
   * Generate a pooled embedding for long text by chunking and mean-pooling.
   * Most embedding models silently truncate inputs > ~512 tokens; for intent
   * descriptions with many examples this loses signal. Splitting on logical
   * boundaries and averaging the per-chunk vectors preserves more of it.
   *
   * @param {string} text
   * @param {{maxCharsPerChunk?: number}} [options]
   * @returns {Promise<number[]>}
   */
  async generatePooledEmbedding(text, { maxCharsPerChunk = 1500 } = {}) {
    if (!text || typeof text !== 'string') {
      return retryOperation(() => embeddingService.generateEmbedding(''));
    }

    // Split on logical sections ('|') then on sentence boundaries, then pack
    // sentences into chunks up to maxCharsPerChunk. Sentences longer than the
    // cap are hard-split.
    const sections = text.split('|').map(s => s.trim()).filter(Boolean);
    const sentences = [];
    for (const sec of sections) {
      const parts = sec.split(/(?<=[.!?])\s+/).map(p => p.trim()).filter(Boolean);
      sentences.push(...(parts.length ? parts : [sec]));
    }
      ...
      if (current.length + s.length + 1 <= maxCharsPerChunk) {
        ...
        if (s.length > maxCharsPerChunk) {
          for (let i = 0; i < s.length; i += maxCharsPerChunk) {
            chunks.push(s.slice(i, i + maxCharsPerChunk));
          }
          current = '';
        } else {
          current = s;
    ...
    if (current) chunks.push(current);
    if (chunks.length === 0) chunks.push(text.slice(0, maxCharsPerChunk));

    const embeddings = [];
    for (let i = 0; i < chunks.length; i++) {
      try {
        const emb = await retryOperation(() => embeddingService.generateEmbedding(chunks[i]));
        if (Array.isArray(emb) && emb.length > 0 && emb.every(n => Number.isFinite(n))) {
          embeddings.push(emb);
        } else {
          logger.warn(`generatePooledEmbedding: invalid embedding for chunk ${i}, skipping`);
        }
      } catch (err) {
        logger.warn(`generatePooledEmbedding: chunk ${i} failed: ${err.message || err}`);
      }
    }

    if (embeddings.length === 0) {
      logger.warn('generatePooledEmbedding: no valid chunk embeddings, falling back to truncated full-text');
      return retryOperation(() => embeddingService.generateEmbedding(text.slice(0, maxCharsPerChunk)));
    }

    const dim = embeddings[0].length;
    if (embeddings.some(v => v.length !== dim)) {
      logger.error(`generatePooledEmbedding: dim mismatch (${embeddings.map(e => e.length).join(',')}), using first valid`);
      return embeddings[0];
    }

    const pooled = new Array(dim).fill(0);
    for (const v of embeddings) for (let i = 0; i < dim; i++) pooled[i] += v[i];
  ...
  async generateIntentEmbedding(intent) {
    try {
      ...
      const embedding = await this.generatePooledEmbedding(text, { maxCharsPerChunk: 1500 });
