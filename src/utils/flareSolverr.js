 ...
 *
 * FlareSolverr sessions do NOT auto-expire — they spawn a Chromium browser
 * that persists until sessions.destroy is called or the container restarts.
 * Restarting LANAgent without cleanup leaks these browsers, accumulating
 * Chromium processes that each consume ~600MB RAM and significant CPU.
 * fsCleanupOrphanSessions() is called at startup to wipe stale sessions
...
/**
 * Destroy every session currently held by the FlareSolverr container.
 * Intended to run at agent startup so that leaked sessions from previous
 * runs don't accumulate as long-lived Chromium browsers.
 */
export async function fsCleanupOrphanSessions() {
  try {
    const res = await axios.post(
      FLARESOLVERR_URL,
      { cmd: 'sessions.list' },
    ...
    logger.info(`[FlareSolverr] cleaned up ${sids.length} orphan session(s) from previous runs`);
    return sids.length;
  } catch {
    return 0;
  }
...
/**
 * List all active FlareSolverr sessions. The upstream sessions.list command
 * returns just an array of session IDs (no creation timestamps), so the
 * returned objects always have createdAt: null.
 */
export async function fsListSessions() {
  const res = await axios.post(
    FLARESOLVERR_URL,
    { cmd: 'sessions.list' },
    { timeout: 5000, headers: { 'Content-Type': 'application/json' } }
  ...
  if (res.data?.status !== 'ok') {
    throw new Error(`FlareSolverr sessions.list error: ${res.data?.message || 'unknown'}`);
  }
  return (res.data.sessions || []).map(id => ({ id, createdAt: null }));
}

/**
 * Force-destroy every session FlareSolverr currently knows about and clear
 * our local sessionCache.
 */
export async function fsForceCleanup() {
  const sessions = await fsListSessions();
  let destroyed = 0;
  for (const { id } of sessions) {
    try {
      ...
      logger.warn(`[FlareSolverr] failed to destroy ${id}: ${err.message}`);
    }
  }
  sessionCache.clear();
  logger.info(`[FlareSolverr] force-cleanup destroyed ${destroyed}/${sessions.length} sessions`);
  return { success: true, destroyed, total: sessions.length };
}

/**
 * Lightweight stats snapshot from data we can actually observe — session
 * count, availability flag, last availability check time.
 */
export async function fsGetStats() {
  const sessions = await fsListSessions().catch(() => []);
  return {
    sessions: { count: sessions.length },
    availability: await isFlareSolverrAvailable(),
    availabilityCheckedAt: availabilityCache.checkedAt || null,
    localSessionCacheSize: sessionCache.size
  };
}
