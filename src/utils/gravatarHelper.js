...
// In-flight de-dup + simple circuit breaker (prevents API stampedes during outages).
const inFlight = new Map();
const breaker = { openUntil: 0, failCount: 0 };
const GRAVATAR_FAIL_THRESHOLD = Number(process.env.GRAVATAR_FAIL_THRESHOLD || 5);
const GRAVATAR_BREAKER_COOLDOWN_MS = Number(process.env.GRAVATAR_BREAKER_COOLDOWN_MS || 60_000);
const GRAVATAR_NEGATIVE_TTL_SECONDS = Number(process.env.GRAVATAR_NEGATIVE_TTL_SECONDS || 300);
...
export async function fetchGravatarProfile(email) {
  ...
  const cacheKey = `profile_${emailHash}`;
  const cachedProfile = gravatarCache.get(cacheKey);
  if (cachedProfile) {
    if (cachedProfile.__notFound === true) {
      logger.debug(`Negative cache hit for Gravatar profile of ${email}`);
      return null;
    }
  ...
  // Circuit breaker open → short-circuit, don't pile on a failing API
  if (Date.now() < breaker.openUntil) {
    logger.debug(`Gravatar circuit breaker OPEN, skipping fetch for ${email}`);
    return null;
  }

  // In-flight dedup: if another caller is already fetching this hash, await it
  if (inFlight.has(emailHash)) {
    logger.debug(`Deduplicating in-flight Gravatar request for ${email}`);
  ...
  const doFetch = async () => {
    try {
      ...
      if (GRAVATAR_API_KEY) headers['Authorization'] = `Bearer ${GRAVATAR_API_KEY}`;
      ...
      if (!response.ok) {
        if (response.status === 404) {
          ...
          // Negative cache (short TTL) so we don't re-hit for known-missing profiles
          gravatarCache.set(cacheKey, { __notFound: true }, GRAVATAR_NEGATIVE_TTL_SECONDS);
          breaker.failCount = 0;
          breaker.openUntil = 0;
          return null;
        }
      ...
      // Success → reset breaker, cache profile
      breaker.failCount = 0;
      breaker.openUntil = 0;
      gravatarCache.set(cacheKey, profile);
      return profile;
    } catch (error) {
      // Non-404 errors increment breaker; open it when threshold hit
      breaker.failCount += 1;
      if (breaker.failCount >= GRAVATAR_FAIL_THRESHOLD) {
        breaker.openUntil = Date.now() + GRAVATAR_BREAKER_COOLDOWN_MS;
        logger.warn(`Gravatar circuit breaker OPENED for ${GRAVATAR_BREAKER_COOLDOWN_MS}ms after ${breaker.failCount} failures`);
      } else {
        logger.debug(`Gravatar profile fetch failed for ${email} (${breaker.failCount}/${GRAVATAR_FAIL_THRESHOLD}): ${error.message}`);
      }
      return null;
    }
  };

  const promise = doFetch().finally(() => { inFlight.delete(emailHash); });
