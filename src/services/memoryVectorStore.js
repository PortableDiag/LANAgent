...
export class MemoryVectorStore {
  constructor() {
    ...
    this.cache = new NodeCache({
      stdTTL: parseInt(process.env.SEARCH_CACHE_TTL || '300', 10),
      checkperiod: 60
    });
    this.enableSearchCache = (process.env.ENABLE_SEARCH_CACHE || 'true').toLowerCase() === 'true';
    this.enableDupCache = (process.env.ENABLE_DUP_CACHE || 'true').toLowerCase() === 'true';
    this.memoryBatch = [];
  ...
  /**
   * Build a compact cache key by hashing the embedding (typically 1536 floats,
   * so a raw join would balloon NodeCache keys to ~30KB each). SHA-256 → 16
   * hex chars is short enough for fast lookup and collision-resistant for our
   * scale.
   */
  _hashEmbedding(embedding) {
    return crypto.createHash('sha256')
      .update(Float32Array.from(embedding))
      .digest('hex')
      .slice(0, 16);
  }

  _searchCacheKey(embedding, options) {
    const opts = {
    ...
    return `s:${this._hashEmbedding(embedding)}:${JSON.stringify(opts)}`;
  }

  _dupCacheKey(embedding, threshold) {
    return `d:${threshold}:${this._hashEmbedding(embedding)}`;
  }
  ...
  _invalidateCache() {
    try {
      ...
      logger.warn(`MemoryVectorStore: cache invalidation failed: ${err?.message || err}`);
    }
  }
  ...
  async findDuplicate(embedding, threshold = null) {
    ...
    try {
      ...
      if (cacheKey) this.cache.set(cacheKey, duplicate);
      return duplicate;
    } catch (error) {
  ...
  async search(queryEmbedding, options = {}) {
    ...
    const cacheKey = this.enableSearchCache
      ? this._searchCacheKey(queryEmbedding, { limit, minSimilarity, type, userId, category, source, context })
      : null;
    if (cacheKey) {
      const cached = this.cache.get(cacheKey);
      if (cached !== undefined) return cached;
    ...
    try {
      ...
      if (cacheKey) this.cache.set(cacheKey, formattedResults);
