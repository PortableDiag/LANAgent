...
export class SubAgentOrchestrator extends EventEmitter {
  ...
  async runAgent(agentId, options = {}) {
    ...
    this.executeAgent(handler, agentDoc, options).catch(error => {
      logger.error(`Agent ${agentDoc.name} [id=${agentDoc._id}] execution error (outer):`, error);
    });
  ...
  async executeAgent(handler, agentDoc, options = {}) {
    ...
    try {
      ...
      clearTimeout(timeoutHandle);
      // Disambiguate failure mode + which agent doc + what the handler was doing.
      // Past confusion: "Crypto Strategy Agent execution failed: Session timeout"
      // gave no clue WHICH step inside execute() was stuck, and prompted a
      // misdiagnosis (the actual stuck operation was attributed to an
      // unrelated component running on its own scheduler). The agentId +
      // stage + duration triple makes the next timeout self-explanatory.
      const elapsedMs = Date.now() - startedAt;
      const isTimeout = /Session timeout/.test(error?.message || '');
      const tag = isTimeout ? 'TIMEOUT' : 'FAILED';
      const stage = handler?._currentStage || 'unknown';
      logger.error(
        `Agent ${agentDoc.name} [${tag}] [id=${agentDoc._id}] [stage=${stage}] [elapsed=${(elapsedMs / 1000).toFixed(1)}s]:`,
        error
      );
      ...
      try {
        await agentDoc.endSession({
          success: false,
          error: error.message,
          context: { stack: error.stack, stage, elapsedMs, timeout: isTimeout }
        });
      } catch (endErr) {
        logger.error(`Failed to end session for ${agentDoc.name} [id=${agentDoc._id}]:`, endErr);
        // Force status reset as last resort
        try {
          agentDoc.status = 'idle';
          await agentDoc.save();
        } catch (saveErr) {
          logger.error(`Failed to force-reset agent ${agentDoc.name} [id=${agentDoc._id}] status:`, saveErr);
        }
      }
