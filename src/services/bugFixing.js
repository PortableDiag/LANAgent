  ...
  async loadConfig() {
    try {
      ...
      if (agent && agent.serviceConfigs && agent.serviceConfigs.bugFixing) {
        ...
        // Sync top-level flag with persisted config so a saved disable actually disables the service
        this.enabled = this.config.enabled;

        logger.info(`Bug fixing configuration loaded from database (enabled: ${this.enabled})`);
      } else {
        // No persisted config — keep this.enabled in sync with the default config.enabled
        this.enabled = this.config.enabled;
