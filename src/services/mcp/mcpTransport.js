...
class MCPTransport extends EventEmitter {
   ...
   * @param {number|object} [timeoutOrOptions] - Either a numeric timeout (legacy positional API)
   *   or an options object: { timeout, signal, cancelMethod }
   * @returns {Promise} Response promise
   */
  async request(method, params = {}, timeoutOrOptions) {
    const opts = (typeof timeoutOrOptions === 'object' && timeoutOrOptions !== null)
      ? timeoutOrOptions
      : (typeof timeoutOrOptions === 'number' ? { timeout: timeoutOrOptions } : {});
    const { timeout: optTimeout, signal, cancelMethod } = opts;

    const id = this.nextId();
    const message = { jsonrpc: '2.0', id, method, params };

    if (signal?.aborted) {
      this.emit('requestAborted', { id, method });
      throw new AbortError(`Request aborted before send: ${method}`);
    }

    return new Promise((resolve, reject) => {
      const timeoutDuration = typeof optTimeout === 'number' ? optTimeout : 30000;
      ...
      const onAbort = () => {
        ...
        this.pendingRequests.delete(id);
        reject(new AbortError(`Request aborted: ${method}`));
        if (cancelMethod && this.connected) {
          try { this.notify(cancelMethod, { id }); }
          catch (e) { logger.debug(`Failed to send cancel notification ${cancelMethod} for ${method}#${id}: ${e?.message}`); }
        }
        this.emit('requestAborted', { id, method });
      };
