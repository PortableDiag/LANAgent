...
export class SelfModificationService extends EventEmitter {
  constructor(agent) {
    ...
    this.config = {
      ...
      allowedUpgrades: [
        ...
        // github_discovered_feature is re-enabled now that enrichDiscoveredFeature
        // hydrates each record with the actual commit diff before it reaches the
        // AI prompt + PR builder. Quality gate inside the enricher drops anything
        // we can't fetch a real, code-only, <8K-char diff for.
  ...
  async isIdle() {
    ...
    try {
      ...
      const memActuallyUsed = typeof mem.available === 'number'
        ? mem.total - mem.available
        : (mem.active ?? mem.used);
      const memoryUsage = (memActuallyUsed / mem.total) * 100;
  ...
  async hasReachedDailyLimit() {
    ...
    // Apply merge-rate-adaptive cap: if recent merge rate is bad, halve the
    // daily ceiling so a bad run doesn't fill the queue with low-quality PRs.
    const baseLimit = this.config.maxDailyImprovements;
    const adaptive = await this.getAdaptiveDailyLimit(baseLimit);
    const hasReached = this.dailyImprovementCount >= adaptive;
    if (adaptive !== baseLimit) {
      logger.info(`Daily limit check: ${this.dailyImprovementCount}/${adaptive} (adaptive, base ${baseLimit}), hasReached: ${hasReached}`);
    } else {
      logger.info(`Daily limit check: ${this.dailyImprovementCount}/${baseLimit}, hasReached: ${hasReached}`);
    }
    return hasReached;
  }

  /**
   * Compute the recent merge rate of self-mod PRs and derive an adaptive
   * daily cap. If merge rate < 30%, halve the cap (floor 1). Cached for the
   * lifetime of one check cycle via _mergeRateCache to avoid repeated
   * `gh pr list` calls within the same run.
   *
   * Returns the adaptive limit. Falls back to `baseLimit` on any failure —
   * we never want this telemetry path to block a check.
   */
  async getAdaptiveDailyLimit(baseLimit) {
    try {
      const stats = await this.computeRecentMergeRate();
      if (!stats || stats.total === 0) {
        logger.info(`Merge-rate adaptive: no recent self-mod PRs found; using base limit ${baseLimit}`);
        return baseLimit;
      }
      const ratePct = Math.round(stats.rate * 100);
      logger.info(`Merge-rate adaptive: recent self-mod PRs ${stats.merged}/${stats.total} merged (${ratePct}%), threshold 30%`);
      if (stats.rate < 0.30) {
        const adaptive = Math.max(1, Math.floor(baseLimit / 2));
        logger.warn(`Merge-rate adaptive: rate ${ratePct}% below 30% — halving daily cap to ${adaptive} (from ${baseLimit})`);
        return adaptive;
      }
      return baseLimit;
    } catch (err) {
      logger.warn(`Merge-rate adaptive check failed: ${err.message} — using base limit ${baseLimit}`);
      return baseLimit;
    }
  }

  /**
   * Query gh for the last N (default 30) self-mod PRs (states merged + closed)
   * and return { merged, total, rate }. Only counts PRs whose headRefName
   * starts with auto-improve/ — manual PRs are excluded.
   */
  async computeRecentMergeRate(limit = 30) {
    const now = Date.now();
    if (this._mergeRateCache && (now - this._mergeRateCache.ts) < 5 * 60 * 1000) {
      return this._mergeRateCache.stats;
    }
    const workingDir = this.developmentPath || process.cwd();
    const result = await this.agent.systemExecutor.execute(
      `gh pr list --state all --limit ${limit} --json state,headRefName`,
      { cwd: workingDir, timeout: 10000 }
    );
    if (result.exitCode !== 0) {
      throw new Error(`gh pr list failed: ${result.stderr || 'unknown'}`);
    }
    const prs = JSON.parse(result.stdout || '[]')
      .filter(pr => typeof pr.headRefName === 'string' && pr.headRefName.startsWith('auto-improve/'));
    const total = prs.length;
    const merged = prs.filter(pr => pr.state === 'MERGED').length;
    const rate = total > 0 ? merged / total : 0;
    const stats = { merged, total, rate };
    this._mergeRateCache = { ts: now, stats };
    return stats;
  }
  ...
  async checkForImprovements() {
    ...
    try {
      ...
      // Check daily limit (adaptive: hasReachedDailyLimit consults merge rate)
      ...
      for (const candidate of prioritized) {
        ...
        if (this.isDuplicateCapability(fp)) {
          ...
          // If the candidate came from the GitHub-discovery pipeline, mark its
          // DiscoveredFeature record as rejected so it stops resurfacing every
          // 30 min as the lone surviving candidate (then losing the dedupe race).
          if (candidate.discoveredFeatureId) {
            try {
              const { default: DiscoveredFeature } = await import('../models/DiscoveredFeature.js');
              await DiscoveredFeature.findByIdAndUpdate(candidate.discoveredFeatureId, {
                status: 'rejected',
                rejectionReason: `Duplicate capability fingerprint: ${fp}`
              });
              logger.info(`🗑️ Marked discovered feature ${candidate.discoveredFeatureId} as rejected (dupe fingerprint)`);
            } catch (dbErr) {
              logger.warn(`Failed to mark dupe discovered feature: ${dbErr.message}`);
            }
          }
          continue;
        }
        ...
        if (isDuplicatePR) {
          ...
          if (candidate.discoveredFeatureId) {
            try {
              ...
              await DiscoveredFeature.findByIdAndUpdate(candidate.discoveredFeatureId, {
                status: 'rejected',
                rejectionReason: 'Similar PR already exists for this discovered feature'
              });
              logger.info(`🗑️ Marked discovered feature ${candidate.discoveredFeatureId} as rejected (PR exists)`);
            } catch (dbErr) {
              logger.warn(`Failed to mark dupe-PR discovered feature: ${dbErr.message}`);
            }
          }
          continue;
        }
  ...
  async getStoredGitHubFeatures() {
    try {
      ...
      for (const feature of discoveredFeatures) {
        const enriched = await this.enrichDiscoveredFeature(feature);
        if (!enriched) continue; // dropped by quality gate (and marked skipped in DB)
        improvements.push(enriched);
      }

      logger.info(`🌟 Found ${improvements.length}/${discoveredFeatures.length} discovered features with sufficient context for implementation`);
      return improvements;
  ...
  /**
   * Enrich a DiscoveredFeature with real GitHub context before it reaches the
   * AI prompt and PR-body builders.
   *
   * For `commit_feature` records the on-disk codeSnippets are typically just
   * the commit *message* text — useless as a code reference. This method
   * extracts the commit SHA from the suggestion field, fetches the real diff
   * via `gh api repos/owner/repo/commits/<sha>`, filters to JS/TS/MJS files
   * that fit in AI context, and rewrites `codeSnippets` with actual patch
   * content. It also lifts the commit subject into `description` so the PR
   * body says "migrate Twitter backend from bird CLI to xreach CLI" instead
   * of "Auto-discovered from GitHub project X. Study commit: <sha>".
   *
   * Quality gate: if the diff is unfetchable, too large (>8K total), touches
   * >5 files, or contains no JS/TS code, the feature is marked
   * `kind: 'skip'` with a rationale and excluded — preventing it from
   * resurfacing on every 30-min scan.
   *
  ...
  async enrichDiscoveredFeature(feature) {
    ...
    // Only commit-derived features need diff enrichment. Other types (e.g.
    // code_pattern from a /search/code result) already carry real snippets.
    const isCommitDerived = feature.type === 'commit_feature' ||
      (feature.codeSnippets || []).some(s => /^Commit:\s*[a-f0-9]{7,40}/im.test(s.code || ''));

    if (!isCommitDerived) return baseImprovement;

    // Extract SHA from either the suggestion or the existing codeSnippet header.
    const shaSources = [
      feature.implementation?.suggestion,
      ...(feature.codeSnippets || []).map(s => s.code)
    ].filter(Boolean);
    let sha = null;
    for (const src of shaSources) {
      const m = src.match(/(?:Study commit|Commit):\s*([a-f0-9]{7,40})/i) || src.match(/\b([a-f0-9]{40})\b/);
      if (m) { sha = m[1]; break; }
    }
    if (!sha || !feature.source?.repository) {
      await this._skipDiscoveredFeature(feature._id, 'no_sha_or_repo');
      return null;
    }

    try {
      const diff = await this._fetchCommitDiff(feature.source.repository, sha);
      if (!diff) {
        await this._skipDiscoveredFeature(feature._id, 'github_fetch_failed');
        return null;
      }

      // Quality gate
      if (diff.files.length === 0 || diff.files.length > 5) {
        await this._skipDiscoveredFeature(feature._id, `bad_file_count:${diff.files.length}`);
        return null;
      }
      const codeFiles = diff.files.filter(f => /\.(js|mjs|cjs|ts|tsx)$/i.test(f.filename) && f.patch);
      if (codeFiles.length === 0) {
        await this._skipDiscoveredFeature(feature._id, 'no_code_files');
        return null;
      }
      const totalPatch = codeFiles.reduce((sum, f) => sum + (f.patch?.length || 0), 0);
      if (totalPatch > 8000) {
        await this._skipDiscoveredFeature(feature._id, `patch_too_large:${totalPatch}`);
        return null;
      }

      // Replace placeholder snippets with real diffs
      const realSnippets = codeFiles.map(f => ({
        code: f.patch,
        language: 'diff',
        filePath: f.filename,
        contextNotes: `+${f.additions} -${f.deletions} ${f.status}`
      }));

      // Use commit subject line as description so PR body reads naturally
      const subject = (diff.message || '').split('\n')[0].trim();
      const cleanDescription = subject || feature.description;

      // Extract implementation hint from commit body
      const bodyLines = (diff.message || '').split('\n').slice(1).map(l => l.trim()).filter(Boolean);
      const implementationHint = bodyLines.slice(0, 3).join(' ').slice(0, 400);

      // New capabilities = the modified-file list with their change scale
      const newCapabilities = codeFiles.map(f =>
      ...
      return {
        ...
        implementation: implementationHint || `Apply pattern from ${feature.source.repository}@${sha.slice(0, 7)}`,
        newCapabilities,
        codeSnippets: realSnippets,
        hasCodeSnippets: true,
        sourceCommitUrl: `https://github.com/${feature.source.repository}/commit/${sha}`,
        sourceCommitSha: sha
      };
    } catch (err) {
      logger.debug(`Discovered feature ${feature._id} enrichment error: ${err.message}`);
      await this._skipDiscoveredFeature(feature._id, `enrichment_error:${err.message?.slice(0, 60)}`);
      return null;
    }
  }

  /**
   * Mark a DiscoveredFeature as skipped so findImplementable stops returning
   * it. Sets implementation.kind = 'skip' (rather than status: 'rejected') so
   * the record stays around for future inspection but won't waste a scan slot.
   */
  async _skipDiscoveredFeature(id, reason) {
    try {
      await DiscoveredFeature.findByIdAndUpdate(id, {
        $set: {
          'implementation.kind': 'skip',
          'implementation.kindRationale': `Auto-skipped at enrichment: ${reason}`
        }
      });
      logger.info(`Discovered feature ${id} marked skip: ${reason}`);
    } catch (err) {
      logger.debug(`Failed to mark discovered feature ${id} as skip: ${err.message}`);
    }
  }

  /**
   * Fetch the full diff of a commit via `gh api`. Uses gh CLI rather than
   * raw axios because GIT_PERSONAL_ACCESS_TOKEN is already configured for it
   * and gh handles auth/retries uniformly.
   *
   * Returns { message, files: [{filename, status, additions, deletions, patch}] }
   * or null on failure.
   */
  async _fetchCommitDiff(repoFullName, sha) {
    // Cache so multiple files mapped to the same commit don't repeat the fetch
    if (!this._commitDiffCache) this._commitDiffCache = new Map();
    const cacheKey = `${repoFullName}@${sha}`;
    if (this._commitDiffCache.has(cacheKey)) return this._commitDiffCache.get(cacheKey);

    try {
      const cmd = `gh api repos/${repoFullName}/commits/${sha}`;
      const result = await this.agent.systemExecutor.execute(cmd, { timeout: 15000 });
      if (result.exitCode !== 0 || !result.stdout) {
        this._commitDiffCache.set(cacheKey, null);
        return null;
      }
      ...
      this._commitDiffCache.set(cacheKey, out);
      return out;
    } catch (err) {
      this._commitDiffCache.set(cacheKey, null);
      return null;
...
Pick the single most relevant existing file that should be enhanced.`;
  ...
  /**
   * Generate or update a test file alongside a capability upgrade. Best-effort:
   * any failure here is logged and swallowed by the caller — the code change
   * still goes out as a single-file PR if the test generation breaks.
   *
   * Test path convention: tests/unit/<basename>.test.js. If a test already
  ...
  async generateOrUpdateTestForUpgrade(improvement, targetFile, modifiedCode) {
    ...
    const testRelPath = path.join('tests', 'unit', `${base}.test.js`);
    const testAbsPath = path.join(this.developmentPath, testRelPath);

    let existingTest = null;
    try {
      existingTest = await fs.readFile(testAbsPath, 'utf8');
    } catch {
...
EXISTING TEST FILE (${testRelPath}):
\`\`\`
${existingTest}
\`\`\`
...
- Keep the file under 300 lines.`;
    ...
    // reasoning and returned 0 chars. 16000 plus reasoning_effort=low keeps
    // headroom for the actual code without runaway thinking.
    const response = await this.agent.providerManager.generateResponse(prompt, {
      maxTokens: 16000,
      temperature: 0.2,
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (modifiedCode && modifiedCode.trim() !== originalCode.trim()) {
          ...
          if (changeStats.linesAdded > 0 || changeStats.linesRemoved > 0 || changeStats.linesChanged > 0) {
            logger.info(`✅ AI generated meaningful changes on attempt ${attempt} (+${changeStats.linesAdded} -${changeStats.linesRemoved} ~${changeStats.linesChanged})`);
            return modifiedCode;
          }
          const origLen = originalCode.length;
          const modLen = modifiedCode.length;
          const firstDiffIdx = (() => {
            const limit = Math.min(origLen, modLen);
            for (let i = 0; i < limit; i++) if (originalCode[i] !== modifiedCode[i]) return i;
            return limit;
          })();
          const sample = modifiedCode.substring(Math.max(0, firstDiffIdx - 40), firstDiffIdx + 120).replace(/\n/g, '\\n');
          logger.warn(`⚠️ Attempt ${attempt}: only whitespace-equivalent changes (origLen=${origLen} modLen=${modLen} firstDiffAt=${firstDiffIdx}). Sample: "${sample}"`);
        } else {
          logger.warn(`⚠️ Attempt ${attempt}: AI returned ${!modifiedCode ? 'empty' : 'identical'} response (origLen=${originalCode.length} modLen=${modifiedCode?.length ?? 0})`);
        }
  ...
  async generateAICodeUpgrade(improvement, originalCode, attempt = 1) {
    try {
      ...
      // The prompt asks for the complete modified file. With reasoning models
      // (gpt-5, o-series) the budget must cover BOTH the visible output AND
      // the internal reasoning tokens, which can consume thousands even with
      // reasoning_effort=low/minimal. Observed empirically: a 12.5K-char file
      ...
      const response = await this.agent.providerManager.generateResponse(basePrompt, {
        ...
        // Reasoning models (gpt-5, o-series) burn the budget on internal
...
18. WRONG - Embedding unbounded arrays in MongoDB documents:
   ...
   * Uses set membership on trimmed lines so reordering and indentation
   * shifts don't get reported as added/removed when the content is the
   * same. Whitespace-only changes are excluded from linesChanged.
   */
  analyzeCodeChanges(original, modified) {
    ...
    for (const line of modifiedSet) {
      if (!originalSet.has(line)) linesAdded++;
    }

    let linesRemoved = 0;
    for (const line of originalSet) {
      if (!modifiedSet.has(line)) linesRemoved++;
    }

    // linesChanged: positional edits where trimmed content differs and
    // neither side is empty. Excludes whitespace-only differences.
    let linesChanged = 0;
    const maxLength = Math.max(originalLines.length, modifiedLines.length);
    for (let i = 0; i < maxLength; i++) {
      const o = (originalLines[i] || '').trim();
      const m = (modifiedLines[i] || '').trim();
      if (o && m && o !== m) linesChanged++;
    }
...
Provide only the corrected code line(s).`;
  ...
  async createPullRequest(branchName, improvement) {
    ...
    const tr = improvement.testResults;
    const testLine = improvement.stagingTested && tr
      ? `staging tests: ${tr.success ? 'PASS' : 'FAIL'}${tr.session ? ` (${tr.session.passed}/${tr.session.totalTests})` : ''}`
      : 'staging tests: not run';
    const agentLabel = process.env.AGENT_NAME || 'LANAgent';
    const prBody = `**Why:** ${improvement.description}

**Change:** automated improvement on \`${improvement.file}\`.

**Tests:** ${testLine}. Manual review still required before merge.

<sub>type: ${improvement.type} · priority: ${improvement.priority} · effort: ${improvement.effort} · generated by ${agentLabel} self-mod (staging path)</sub>`;
...
**Tests:** passed on contributing instance (${agentName}). Upstream review required.

<sub>type: ${improvement.type} · priority: ${improvement.priority} · fork: ${provider.owner}/${provider.repo} · contributed by ${agentName} self-mod</sub>`;
  ...
  async createUpgradePullRequest(branchName, upgrade) {
    ...
    const newCapsBlock = Array.isArray(upgrade.newCapabilities) && upgrade.newCapabilities.length > 0
      ? `\n**New capabilities:**\n${upgrade.newCapabilities.map(cap => `- ${cap}`).join('\n')}\n`
      : '';

    const implementationText = (upgrade.implementation && String(upgrade.implementation).trim())
    ...
    const meta = [
      ...
      upgrade.priority ? `priority: ${upgrade.priority}` : null,
      upgrade.effort ? `effort: ${upgrade.effort}` : null,
