...
class RegistryClient extends EventEmitter {
  constructor() {
    ...
    // Heartbeat watchdog: detects silent half-open WebSocket connections.
    // After each heartbeat send, a per-beat timer waits for the ACK. If too many
    // beats in a row miss their ACK, we force-terminate the socket to trigger
    // the existing reconnect path instead of waiting for TCP FIN.
    this.missedHeartbeats = 0;
    this.maxMissedHeartbeats = 2;
    this.heartbeatAckTimeoutMs = HEARTBEAT_INTERVAL - 2000;
  ...
  _onOpen() {
    ...
    // Reset watchdog state before (re)starting heartbeat
   ...
   *
   * EventEmitter rule: emitting `'error'` with NO listeners attached crashes
   * the process. p2pService.js attaches its listeners AFTER the initial
   * registryClient.connect() call, so any ws error that fires during the
   * boot-time connect — or during a reconnect cycle where listeners were
   * transiently removed — would propagate as an uncaught and kill ALICE.
   * Seen live 2026-05-20 when a customer demo was in flight: ws emitted
   * "Opening handshake has timed out" from inside the handshake retry, the
   * unguarded emit crashed Node, PM2 restarted, ~3min outage during the demo.
   *
   * Guarding the emit means errors are always logged, and propagate to
   * listeners when present — but never crash the agent.
   */
  _onError(error) {
  ...
  _startHeartbeat() {
    ...
    this.heartbeatInterval = setInterval(() => {
      if (this.state === 'connected') {
        const sent = this._send({ type: 'heartbeat' });
        if (sent) this._onHeartbeatSent();
      }
    }, HEARTBEAT_INTERVAL);
  }
  ...
  _onHeartbeatSent() {
    ...
    this.heartbeatAckTimer = setTimeout(() => {
      ...
      logger.warn(`P2P Registry: heartbeat ACK missed (${this.missedHeartbeats}/${this.maxMissedHeartbeats})`);
      if (this.missedHeartbeats > this.maxMissedHeartbeats &&
          this.ws && this.ws.readyState === WebSocket.OPEN) {
        logger.warn('P2P Registry: max missed heartbeats exceeded, terminating socket for fast reconnect');
        try { this.ws.terminate(); } catch (e) {
          logger.error('P2P Registry: error terminating socket:', e.message);
        }
      }
    }, Math.max(1000, this.heartbeatAckTimeoutMs));
  }

  _onHeartbeatAck() {
    this._clearHeartbeatAckTimer();
    if (this.missedHeartbeats > 0) {
      logger.debug(`P2P Registry: heartbeat ACK received, clearing missed counter (was ${this.missedHeartbeats})`);
    }
    this.missedHeartbeats = 0;
  }

  _clearHeartbeatAckTimer() {
