...
class P2PService extends EventEmitter {
  ...
  async initialize() {
    ...
    try {
      ...
      // Registry connect is best-effort during boot. If the registry is slow
      // or unreachable, this call can hang indefinitely and block the entire
      // agent startup — including the web interface bind — leaving the agent
      // alive but unreachable. Cap it at 10s; the registryClient will keep
      // retrying in the background via its own reconnect logic.
      try {
        await Promise.race([
          ...
          new Promise((_, reject) => setTimeout(() => reject(new Error('P2P registry connect timeout (10s)')), 10000))
        ]);
      } catch (regErr) {
        logger.warn(`P2P registry connect deferred: ${regErr.message} — boot will continue, registry client will retry in background`);
      }
  ...
  _setupRegistryEvents() {
    ...
    registryClient.on('error', (error) => {
      ...
      // Guard — EventEmitter `'error'` emit with zero listeners synchronously
      // throws and crashes the process. See registryClient.js#_onError for the
