...
class SwapService {
    ...
    async verifySwapBalances(swap) {
        try {
            ...
            // wallet.addresses is an array of {chain,address}, not an object — the
            // old `wallet.addresses.ethereum` lookup silently returned undefined and
            // then contract.balanceOf(undefined) failed with the unhelpful ethers
            // error "unsupported addressable value (argument='target', value=null)".
            const chain = network === 'ethereum' ? 'eth' : network;
            const address = wallet?.addresses?.find(a => a.chain === chain)?.address;
            if (!address) {
                logger.warn(`Skipping swap balance verification: no wallet address for chain '${chain}' (network='${network}')`);
                swap.verificationError = `no wallet address for chain ${chain}`;
                return;
            }
            ...
            logger.error(`Balance verification failed (${swap?.tokenOut} on ${swap?.network}): ${detail}`);
