...
class HardhatService {
     ...
     * Hardhat is installed per-project (not globally into the agent's package.json) to avoid
     * peer-dep conflicts between hardhat-toolbox and @langchain/community/@huggingface/inference.
     */
    async initialize() {
        ...
        try {
            ...
            logger.info('Hardhat service initialized (projects dir ready)');
        } catch (error) {
    ...
    /**
     * Install hardhat + toolbox into a specific project directory.
     * Pinned to hardhat@^2 + toolbox@^5 — hardhat 3 dropped non-interactive `init` and
     * the rest of this service uses hardhat 2's compile/run/test conventions.
     * --legacy-peer-deps bypasses the @huggingface/inference peer-dep conflict
     * that exists in the agent's parent package tree.
     */
    async ensureProjectHardhat(projectPath) {
        try {
            await fs.access(path.join(projectPath, 'node_modules', 'hardhat', 'package.json'));
            return;
        } catch (e) {
            logger.info(`Installing hardhat into ${projectPath}...`);
            await execAsync(
                'npm install --save-dev --legacy-peer-deps hardhat@^2 @nomicfoundation/hardhat-toolbox@^5',
    ...
    async createProject(projectName, template = 'basic') {
        ...
        try {
            ...
            // Seed a minimal package.json so npm install has something to write to
            ...
            // Install hardhat into the project directory (not the agent's main package)
            await this.ensureProjectHardhat(projectPath);

            // Skip `npx hardhat init` — it requires a TTY in hardhat 2.22+ and 3.x
            // (no --no-interactive flag honored). We write the config + contracts/
            // dir manually below, which is everything `init` would have produced
