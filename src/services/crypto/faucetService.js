...
class FaucetService {
    constructor() {
        ...
        this.failureTracker = new Map(); // apiUrl -> { count, suppressUntil }
    }
    ...
    async claimFromApiFaucet(apiUrl, address, network) {
        const suppression = this.failureTracker.get(apiUrl);
        if (suppression && suppression.suppressUntil > Date.now()) {
        ...
        try {
            ...
            if (response.ok && data.success) {
                this.failureTracker.delete(apiUrl);
            ...
            const reason = data.error || data.message
                || `HTTP ${response.status} ${response.statusText || ''} (content-type: ${contentType || 'unknown'})`;
            throw new Error(reason.trim());
        } catch (error) {
            const msg = error.message || String(error) || 'unknown error';
            const tracker = this.failureTracker.get(apiUrl) || { count: 0, suppressUntil: 0 };
            tracker.count += 1;
            const SUPPRESS_AFTER = 3;
            const SUPPRESS_HOURS = 24;
            if (tracker.count >= SUPPRESS_AFTER && tracker.suppressUntil < Date.now()) {
                tracker.suppressUntil = Date.now() + SUPPRESS_HOURS * 60 * 60 * 1000;
                logger.warn(`API faucet ${apiUrl} suppressed for ${SUPPRESS_HOURS}h after ${tracker.count} consecutive failures (last: ${msg})`);
            } else {
                logger.warn(`API faucet claim failed for ${network} (${tracker.count}/${SUPPRESS_AFTER}): ${msg}`);
            }
            this.failureTracker.set(apiUrl, tracker);
