...
class TokenTraderHeartbeatManager {
  ...
  async _executeTick(address, state) {
    // Auto-clean orphaned heartbeats (instance removed but timer still running)
    const trader = strategyRegistry.getTokenTrader(address);
    if (!trader) {
      logger.info(`TokenHeartbeat: ${state.symbol} — no instance found, stopping orphaned heartbeat for ${address.slice(0, 10)}...`);
      await this.stopToken(address);
      return;
    }
