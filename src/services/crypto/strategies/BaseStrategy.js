...
export class BaseStrategy {
    ...
    /**
     * Average duration (ms) of completed round-trips, pairing each
     * sell/closing trade with the most recent prior buy/opening trade
     * on the same network. Returns null when no pairs exist.
     */
    calculateAverageTradeDuration() {
        const history = this.state.performanceHistory || [];
        if (history.length < 2) return null;

        const isOpen = (a) => /buy|open|enter|long/i.test(a || '');
        const isClose = (a) => /sell|close|exit|short/i.test(a || '');

        const opensByNetwork = new Map();
        const durations = [];
        for (const t of history) {
            const ts = t.timestamp instanceof Date ? t.timestamp.getTime() : new Date(t.timestamp).getTime();
            if (!Number.isFinite(ts)) continue;
            const net = t.network || '__';
            if (isOpen(t.action)) {
                if (!opensByNetwork.has(net)) opensByNetwork.set(net, []);
                opensByNetwork.get(net).push(ts);
            } else if (isClose(t.action)) {
                const opens = opensByNetwork.get(net);
                if (opens && opens.length) {
                    durations.push(ts - opens.shift());
                }
            }
        }
    ...
    /**
     * Maximum drawdown over the recorded performanceHistory, computed
     * from the cumulative pnl curve. Returns the largest peak-to-trough
     * drop as an absolute pnl value (not a percentage — strategies don't
     * track an equity baseline). Returns 0 when no drawdown exists.
     */
    ...
    /**
     * Sharpe ratio over daily aggregated pnl from performanceHistory.
     * Annualized assuming 365 trading days for crypto. Risk-free rate
     * is treated as 0 — appropriate for short-horizon strategy comparison
     * where the rf component is dominated by realized variance. Returns
     * null when fewer than 2 days of data exist (variance undefined).
     */
