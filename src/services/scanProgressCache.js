...
/**
 * Pre-populate the cache for a session so subsequent reads hit warm entries.
 * Loads total count, per-status counts (pending/processing/completed/failed),
 * and the pending-entries list. Safe to call before opening a session UI.
 */
export async function warmCacheForSession(ScanProgress, sessionScanId) {
    try {
        await Promise.all([
            getCachedCount(ScanProgress, sessionScanId, null),
            getCachedCount(ScanProgress, sessionScanId, 'pending'),
            getCachedCount(ScanProgress, sessionScanId, 'processing'),
            getCachedCount(ScanProgress, sessionScanId, 'completed'),
            getCachedCount(ScanProgress, sessionScanId, 'failed'),
            getCachedPendingEntries(ScanProgress, sessionScanId)
        ]);
        logger.debug(`scanProgressCache: warmed session ${sessionScanId}`);
    } catch (error) {
        logger.warn(`scanProgressCache: warm failed for ${sessionScanId}: ${error.message}`);
    }
}
