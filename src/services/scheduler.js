...
class TaskScheduler {
  ...
  async initialize(agent) {
    ...
    // Surface Agenda errors instead of letting them become unhandledRejection.
    // Without this, a transient MongoNetworkTimeoutError can wedge the job
    // processor silently — locks get re-acquired every cycle but no handler
    // runs, and the agent appears alive (HTTP responds) while every scheduled
    // job stops firing.
    this.agenda.on('error', (err) => {
      logger.error('[scheduler] Agenda error:', err?.message || err);
    });

    // Stuck-job watchdog: every 5 min, force-unlock jobs whose lock is older
    // than 2× Agenda's lockLifetime (default 10min, so >20min). Recovers from
    // the wedged state where a connection blip left jobs locked-but-never-
    // completing — observed 2026-05-22 when 35 jobs sat locked for 8–10h.
    this.stuckJobWatchdog = safeInterval(async () => {
      const lockLifetimeMs = 10 * 60 * 1000;
      const cutoff = new Date(Date.now() - 2 * lockLifetimeMs);
      const mongoose = (await import('mongoose')).default;
      if (mongoose.connection.readyState !== 1) return;
      const result = await mongoose.connection.collection('scheduled_jobs').updateMany(
        { lockedAt: { $ne: null, $lt: cutoff } },
        { $set: { lockedAt: null } }
      );
      if (result.modifiedCount > 0) {
        logger.warn(`[scheduler] Stuck-job watchdog unlocked ${result.modifiedCount} jobs locked >${(2 * lockLifetimeMs) / 60000}min without completing`);
      }
    }, 5 * 60 * 1000, 'scheduler-stuck-job-watchdog');
...
Respond with ONLY the rephrased message, no explanation:`;
    ...
    this.agenda.define('skynet-lp-staking-autoclaim', async (job) => {
      try {
        ...
        try {
          ...
          const skynetAddress = await SystemSettings.getSetting('skynet_token_address') || '0x8b77CC5c6cB3d846608d9d5Dd03fA406BA03b8F1';
  ...
  async scheduleRecurringJobs() {
    ...
    // day per endpoint and dominating MindSwarm's API quota for our own
    // account. The own-posts/replies scan inside _engagementCycle is further
    // gated to once-per-hour internally.
    await this.agenda.every('15 minutes', 'mindswarm-engagement');
  ...
  async getCryptoStats() {
    try {
      ...
      try {
        ...
        const skynetAddr = '0x8b77CC5c6cB3d846608d9d5Dd03fA406BA03b8F1';
