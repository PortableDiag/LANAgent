  ...
  setupRoutes() {
    ...
    const healthHandler = async (req, res) => {
      try {
        ...
        // Don't let a single stale "critical" report pin /api/health to
        // unhealthy forever. If the diagnostic that produced this report is
        // older than 2× the schedule interval, the report is no longer trust-
        // worthy as a current-state signal — treat it as unknown. The under-
        // lying issue (skipped scheduled runs) is handled by the scheduler
        // watchdog; this just stops external callers from acting on stale data.
        const intervalMs = this.agent.selfDiagnosticsService.config?.autoRunInterval
          ?? (6 * 60 * 60 * 1000);
        const staleThresholdMs = 2 * intervalMs;
        const reportAgeMs = latest?.timestamp
          ? Date.now() - new Date(latest.timestamp).getTime()
          : null;
        const isStale = reportAgeMs !== null && reportAgeMs > staleThresholdMs;

        const health = isStale ? 'stale' : (latest?.overallHealth || 'unknown');
        const isHealthy = isStale || !latest || latest.overallHealth !== 'critical';
    ...
    this.app.get('/api/settings/skynet-token-address', authenticateToken, async (req, res) => {
      try {
        const { SystemSettings } = await import('../../models/SystemSettings.js');
        const value = await SystemSettings.getSetting('skynet_token_address', '0x8b77CC5c6cB3d846608d9d5Dd03fA406BA03b8F1');
