...
router.get('/api/transfers/:transferId/progress', authenticateToken, async (req, res) => {
  try {
    const transfer = await P2PTransfer.getTransferProgress(req.params.transferId);
    if (!transfer) {
      return res.status(404).json({ success: false, error: 'Transfer not found' });
    }
    res.json({ success: true, progress: transfer.getProgressInfo() });
  } catch (error) {
    if (error?.name === 'CastError') {
      return res.status(400).json({ success: false, error: 'Invalid transferId' });
    }
    logger.error('P2P transfer progress API error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});
