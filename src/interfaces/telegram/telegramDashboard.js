...
export class TelegramDashboard extends TelegramInterface {
  ...
  async initialize() {
    ...
    // handlerTimeout defaults to 90s in telegraf — far too short when a single
    // update triggers a large/slow media download+send (e.g. a long YouTube/Rumble
      ...
      try {
        ...
        if (response.requiresApproval) {
          ...
          if (response.type === 'document' && response.path) {
            ...
            // Telegram bots can only SEND files up to 50MB. Anything larger is
            // delivered as download links — a public gateway link (works LAN +
            // remote) plus a LAN link — instead of a futile, hang-prone document
            // upload that previously ate the handler timeout and delivered nothing.
            const buildDownloadLinks = async (filePath, filename) => {
              const lines = [];
              // Public gateway link — works whether the user is remote or on-LAN.
              try {
                const { generateDownloadToken } = await import('../../api/external/services/downloadTokenService.js');
                let agentId = this.agent?.gatewayClient?.agentId;
                if (!agentId) {
                  const { Agent } = await import('../../models/Agent.js');
                  const doc = await Agent.findOne({ name: process.env.AGENT_NAME || 'LANAgent' }).select('erc8004').lean();
                  agentId = doc?.erc8004?.agentId;
                }
                if (agentId) {
                  const token = generateDownloadToken({ filePath, filename, agentId, maxDownloads: 5, expiresInMinutes: 120 });
                  const base = (process.env.GATEWAY_URL || 'https://api.lanagent.net').replace(/\/+$/, '');
                  lines.push(`🌐 ${base}/download/${agentId}/${token}`);
                }
              } catch (linkErr) {
                logger.warn(`Could not build gateway download link: ${linkErr.message}`);
              }
              // LAN link, if the file lives under downloads/.
              try {
                const pathMod = await import('path');
                const downloadsDir = pathMod.join(process.cwd(), 'downloads');
                if (filePath.startsWith(downloadsDir)) {
                  const rel = filePath.slice(downloadsDir.length).replace(/^\//, '');
                  lines.push(`🏠 http://${getServerHost()}:${process.env.AGENT_PORT || 3000}/downloads/${encodeURI(rel)}`);
                }
              } catch { /* ignore LAN link build errors */ }
              return lines;
            };

            const sendDownloadLinks = async (filePath, filename, sizeMB) => {
              const links = await buildDownloadLinks(filePath, filename);
              const safeFilename = (filename || '').replace(/_/g, ' ');
              const sizeStr = sizeMB ? ` (${sizeMB.toFixed(1)}MB)` : '';
              const body = links.length ? links.join('\n') : `📂 Saved on the server at:\n${filePath}`;
              await ctx.reply(`✅ Downloaded "${safeFilename}"${sizeStr}.\nToo large to send inline via Telegram — download it here (links expire in 2h):\n\n${body}`);
            };
            ...
            try {
              ...
              if (sizeMB > 50) {
                logger.warn(`Video too large for Telegram (${sizeMB.toFixed(1)}MB > 50MB) — sending download links`);
                await sendDownloadLinks(response.path, filename, sizeMB);
              } else {
              ...
              // Inline send failed (413/timeout/etc.) — deliver links rather than nothing.
              logger.error('Failed to send video inline, sending download links:', videoErr.message);
              try {
                const pathMod = await import('path');
                const fsStat = await import('fs').then(m => m.promises);
                const filename = response.filename || pathMod.basename(response.path);
                let sizeMB = 0;
                try { sizeMB = (await fsStat.stat(response.path)).size / (1024 * 1024); } catch { /* size optional */ }
                await sendDownloadLinks(response.path, filename, sizeMB);
              } catch (linkErr) {
                logger.error('Failed to deliver download links:', linkErr.message);
                await ctx.reply(`✅ Downloaded, but couldn't send via Telegram. Saved on the server at:\n${response.path}`);
              }
            }
